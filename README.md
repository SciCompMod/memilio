...
The C++ code is divided into two libraries, epidemiology (models, math etc.) and epidemiology_io (IO routines). By standard, epidemiology is built without IO.
...
* Do cmake --build . (without IO) or cmake -DEPI_BUILD_EPI_IO=ON --build . (with IO library)
* Run 

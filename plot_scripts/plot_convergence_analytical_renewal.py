...
def S(timepoint, rho, kappa):
    return rho*np.exp((rho-kappa)*timepoint)


def I(timepoint, rho, kappa):
    return (rho-kappa)*(np.exp((rho-kappa)*timepoint)-np.exp(-kappa*timepoint))


def R(timepoint, rho, kappa):
    return S(timepoint, rho, kappa) - S(0, rho, kappa)-I(timepoint, rho, kappa)


def define_groundtruth(timesteps, t0, tmax, rho, kappa):
    ...
    for timestep in timesteps:
        ...
        for compartment in range(3):
            ...
            if compartment == 0:
                groundtruth_at_timepoints.append([S(timepoint, rho, kappa)
                                                  for timepoint in timepoints])  # np.cosh(timepoint)
            if compartment == 1:
                groundtruth_at_timepoints.append([
                    I(timepoint, rho, kappa) for timepoint in timepoints])  # np.sinh(timepoint)
            if compartment == 2:
                groundtruth_at_timepoints.append([
                    R(timepoint, rho, kappa) for timepoint in timepoints])
...
def main():
    ...
    main_dir = "2026-06-21/analytical_renewal_rho=5.2_kappa=5.1"
    ...
    for dir_index, dir_name in enumerate(sub_dirs):
        ...
        groundtruth = define_groundtruth(timesteps_ide, t0, tmax, rho, kappa)

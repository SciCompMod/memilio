...
section below. For everything else, check out the page on :doc:`ODE-based model <cpp/ode>` usage.
...
The simulation uses an Euler-Maruyama scheme by default, so the initial step size will not change over time.

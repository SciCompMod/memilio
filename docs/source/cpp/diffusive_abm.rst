...
To simulate the model from `t0` to `tmax` with given step size `dt`, a :code:`Simulation` has to be created and advanced until `tmax`, which is done as follows:

...
Our ODE-based models are either implemented as **FlowModel** or as **CompartmentalModel**.
Most models are implemented as **FlowModel**\s. In **FlowModel**\s, flows ``Flow<State1, State2>`` between
**InfectionState**\s **State1** and **State2** are defined. Instead of a standard solution to an ODE-based model, this 
implementation additionally realizes the solution of the transitions or flows between states and directly enables users 
to access new transmissions or hospitalizations at any time point. 
...
- run simulation, t0, tmax (simulation period), initial step size dt
- zu dt: adaptive solution scheme, absolute / relative tolerance 1e-8 --> for details see numerical integrator stuff)
- interpolation to days, miliseconds..
...
:doc:`See Python package <../python/memilio_plot>`




How to: Set up and run a simulation of the ODE-SECIR model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

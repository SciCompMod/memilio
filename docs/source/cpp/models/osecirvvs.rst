...
The ODE-SECIRVVS model is implemented as a :code:`FlowModel`, which defines the derivatives of each flow between compartments. The model follows the same flow pattern as the basic ODE-SECIR model but with three parallel sets of compartments representing different immunity levels.

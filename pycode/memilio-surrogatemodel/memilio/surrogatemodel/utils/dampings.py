...
def calc_factors_active(n_ddays, gamma_pos=0, alpha=-1, p0=0.5, t1_max=-0.3, t1_min=-1.2, t2_max=2, t2_min=-0.5):
    ...
    The h-values are generated by the following rule: 
    1) h is initialized as 0
    2) If h > gamma_pos, we start to reduce the h-value actively by choosing changes delta_h uniformly in 
        the interval (t1_min, t1_max). After each step h is updated according to h = h + delta_h
        This procedure is repeated till h < alpha 
    3) If h < gamma_pos the changes of h, called delta_h are chosen randomly, s.t. delta_h = 0 with 
...
def generate_dampings_withshadowdamp(number_of_dampings, days, min_distance, min_damping_day, number_of_runs):
    ...
    The idea is to draw dampings with a minimum distance while trying to keep

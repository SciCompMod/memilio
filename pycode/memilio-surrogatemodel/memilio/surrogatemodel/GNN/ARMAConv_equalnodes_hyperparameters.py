...
# skip first parameter combination with order = 1 and iteration = 1 as we have this data already
#parameters = parameters[1:]
#parameters[5:]
#parameters = parameters[10:] #starting from (3,3)
df = pd.DataFrame(
    columns=['order', 'iterations',
...
def train_and_evaluate_model(
    ...
    class Net(Model):
        def __init__(self):
            ...
            self.conv1 = ARMAConv(
                512, order=order, iterations=iterations, activation='elu')
            self.conv2 = ARMAConv(
                512, order=order, iterations=iterations, activation='elu')
            self.conv3 = ARMAConv(
                512, order=order, iterations=iterations, activation='elu')
    ...
    df.loc[len(df.index)] = [order, iterations,
    ...
    #    os.mkdir(file_path)
    file_path = '/hpc_data/schm_a45/data_paper/model_evaluation'
...
epochs = 1500
filename = '/GNN_gridsearch_ARMAConv_hyperparameters_3512_21_elu.csv'
#for param in parameters:
#    train_and_evaluate_model(epochs, 0.001, param)
train_and_evaluate_model(epochs, 0.001)

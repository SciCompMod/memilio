...
def remove_confirmed_compartments(dataset_entries, num_groups):
    ...
    :returns: Array that contains the compartmental data without confirmed compartments. 
   """
...
def make_graph(directory, num_regions, countykey_list, models):
    """ Generating graph with one node per region.

    Each node contains the county ID and a osecir model for the county. The edges 
    contain the mobility information between different counties/nodes. 

    :param directory: Directory with mobility data. 
    :param num_regions: Number (int) of counties that should be added to the 
            graph-ODE model. Equals 400 for whole Germany. 
...
def transform_mobility_directory():
    ...
    directory = os.path.join(directory, 'Germany/mobility/')
    ...
    tmd.updateMobility2022(
        directory, mobility_file='commuter_mobility_2022')
    return directory

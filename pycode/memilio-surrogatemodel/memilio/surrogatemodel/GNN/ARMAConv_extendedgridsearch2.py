...
def train_and_evaluate_model(
    ...
    if number_of_layer == 1:
        ...
        class Net(Model):
            ...
            def __init__(self):
                ...
                self.conv5 = layer(number_of_n, activation='relu')
                self.conv6 = layer(number_of_n, activation='relu')
                self.conv7 = layer(number_of_n, activation='relu')
                self.dense = Dense(data.n_labels, activation="linear")
    ...
    def evaluate_r(loader):
        ...
        while step < loader.steps_per_epoch:
            ...
            outs = (
                ...
                tf.reduce_mean(mean_absolute_percentage_error(np.expm1(target), np.expm1(pred))),
                len(target),  # Keep track of batch size
            )
            output.append(outs)
            if step == loader.steps_per_epoch:
    ...
    print("K-Fold ORIGINAL DATA Test Score: {}".format(np.mean(test_rescaled)))  # Use test_rescaled
...
epochs = 1500
filename = '/GNN_gridsearch_ARMA_nodeswithvariance_7512_7.csv'

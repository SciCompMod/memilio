...
df_gridsearch = pd.read_csv(
    '/home/schm_a45/Documents/code3/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_grid_searchdataframe')
# load trained model
...
def lineplot_number_of_days():
    ...
    model60 = tf.keras.models.load_model(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/saved_models/saved_models_secir_simple_60days')
    model90 = tf.keras.models.load_model(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/saved_models/saved_models_secir_simple_90days')
    model120 = tf.keras.models.load_model(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/saved_models/saved_models_secir_simple_120days')
    model150 = tf.keras.models.load_model(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/saved_models/saved_models_secir_simple_150days')

    models = [model60, model90, model120, model150]
    filenames = ["data_secir_simple_60days.pickle", "data_secir_simple_90days.pickle",
                 "data_secir_simple_120days.pickle", "data_secir_simple_150days.pickle"]
...
def lineplot_number_of_days_secir_groups():
    ...
    model30 = tf.keras.models.load_model(os.path.join(
        path_models, 'saved_models_secir_groups_best_LSTM'))
    model60 = tf.keras.models.load_model(os.path.join(
        path_models, 'saved_models_secir_groups_best_LSTM_60days'))
    model90 = tf.keras.models.load_model(os.path.join(
        path_models, 'aved_models_secir_groups_best_LSTM_90days'))
    model120 = tf.keras.models.load_model(os.path.join(
        path_models, 'saved_models_secir_groups_best_LSTM_120days'))
    model150 = tf.keras.models.load_model(os.path.join(
        path_models, 'saved_models_secir_groups_best_LSTM_150days'))
    ...
    for filenames, modelnames, MAPE in zip([filenames_simple, filenames_groups], [modelnames_simple, modelnames_groups], [MAPE_simple, MAPE_groups]):
        ...
        for modelname, filename in zip(modelnames, filenames):
            ...
            model = tf.keras.models.load_model(
                os.path.join(path_models, modelname))
...
def heatmap(df_gridsearch):
    ...
    df_heatmap1 = pd.DataFrame(data=df.loc[(df['model'] == 'Dense')][[
                               'number_of_hidden_layers', 'number_of_neurons', 'kfold_test']])
    df_heatmap1 = df_heatmap1.pivot(
        index='number_of_hidden_layers', columns='number_of_neurons', values='kfold_test')
...
def compartment_error_simple():
    ...
    new_model = tf.keras.models.load_model(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/saved_models/saved_models_secir_simple_150days_w')
    ...
    mape = 100*np.mean(abs((test_labels - pred)/test_labels),
                       axis=0).transpose()

    fig, ((ax1, ax2), (ax3, ax4), (ax5, ax6), (ax7, ax8)) = plt.subplots(
        nrows=4, ncols=2, sharey=False, figsize=(10, 13))
...
def plot_30days_differentmodels():
    ...
    for modelname, filename, name_plot in zip(modelnames_simple, filenames_simple, days):
        ...
        infectionstates = ['Susceptible', 'Exposed', 'InfectedNoSymptoms',
                           'InfectedSymptoms', 'InfectedSevere', 'InfectedCritical', 'Recovered', 'Dead']
        mape = 100*np.mean(abs((test_labels - pred) /
                           test_labels), axis=0).transpose()
    ...
    fig.legend(lines[:(len(days))], line_labels[:len(
        days)], loc='upper center',  bbox_to_anchor=(0.5, -0.05), shadow=True, ncol=3)
    # fig.('MSE and MAPE for compartments', fontsize=16)
...
def plot_inputs():
    ...
    data = pickle.load(file)
    data_splitted = split_data(data['inputs'], data['labels'])
    # reshape to [n_compartments, n_days, n_samples]
    train_inputs = np.transpose(data_splitted['train_inputs'])
    ...
    labels = ['percentile p50', 'percentile p25',
              'percentile p75', 'percentile p05', 'percentile p95']
...
def plot_inputs_withseaborn():
    ...
    intervals = [75, 95]
    colors = ['tab:red', 'tab:green', 'tab:blue', 'tab:red',
              'tab:green', 'tab:blue', 'tab:red', 'tab:green']
...
def plot_input_days():
    df = pd.read_csv(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_grid_search_input_width/dataframe_simple_30days')
    df = df[['input_days', 'kfold_train', 'kfold_test']]

    df_groups = pd.read_csv(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_grid_search_input_width/dataframe_groups30_test')
    ...
    for attribute, measurement in penguin_means.items():
        ...
        rects = ax.bar(x + offset, measurement.round(4),
...
def boxplot_inputs():
    ...
    file_w2 = open(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/data/data_secir_simple_30days_widerinput_3.pickle', 'rb')
...
def plot_results_on_b_w1_w3():
    df_b = pd.read_csv(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_dataframessecir_simple_baselineLSTM_30')
    df_w1 = pd.read_csv(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_dataframessecir_simple_w1LSTM_30_w1')
    ...
    df_w3 = pd.read_csv(
        '/home/schm_a45/Documents/Code/memilio/memilio/pycode/memilio-surrogatemodel/memilio/secir_simple_dataframessecir_simple_w3LSTM_30_w3')
...
def compartment_error_simple_barplot():
    ...
    # file = open(os.path.join(path_data,filename), 'rb')
    filenames = ['data_secir_simple_30days_widerinput_2.pickle',
                 'data_secir_simple_90days_w2.pickle',
                 'data_secir_simple_150days_w2.pickle']
    ...
    modelnames = ['saved_models_secir_simple_30days_w',
                  'saved_models_secir_simple_90days_w',
                  'saved_models_secir_simple_150days_w']
    ...
    for attribute, measurement in compartment_errors.items():
        ...
        offset = width * multiplier
        rects = ax.barh(x + offset, measurement.round(4),
                        width, label=attribute)

        large_bars = [
            p if p > 0.28 else '' for p in measurement.round(4)]  # for MAPE
        small_bars = [
            p if p <= 0.28 else '' for p in measurement.round(4)]  # for MAPE
...
def lineplot_days_simple_and_groups_MAE():
    ...
    for filenames, modelnames, MAE in zip([filenames_simple, filenames_groups], [modelnames_simple, modelnames_groups], [MAE_simple, MAE_groups]):
        ...
        for modelname, filename in zip(modelnames, filenames):
            ...
            mae = np.mean((abs(labels_reversed - pred_reversed)),
                          axis=0).transpose()

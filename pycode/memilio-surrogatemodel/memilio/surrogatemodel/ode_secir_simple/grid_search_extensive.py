...
#from tensorflow.keras.optimizers import Adam, Adagrad, Nadam, RMSprop, SGD
from keras.activations import relu, elu, sigmoid, softmax, tanh
import numpy as np
...
filename_df = "dataframe_grid_search"
...
activations = [relu, elu, sigmoid, softmax]
        ...
        for modelname in models:
                    for act in activations:
                         for opt in optimizers:
                            parameters.append((layer, neuron_number, modelname, act, opt))
...
df_results  = pd.DataFrame(
    columns=['model', 'number_of_hidden_layers', 'number_of_neurons', 'activation', 'optimizer',
...
def train_and_evaluate_model(param, max_epochs):
    ...
    for train_idx, test_idx in zip(train_idxs, test_idxs):
        ...
        if modelname == 'Dense':
            ...
            model = tf.keras.Sequential([
                ...
                tf.keras.layers.Dense(units=neuron_number, activation=activation)])
            ...
            tf.keras.layers.Conv1D(neuron_number, activation=activation,
    ...
    df_results.loc[len(df_results.index)] = [modelname, layer, neuron_number,  activation, optimizer, df.mean()[0] , np.mean(train_losses),
                             np.mean(val_losses),
                             np.mean(test_scores),
                             (elapsed / 60)]
                            # [losses_history_all],
    ...
    file_path = os.path.join(
        ...
        'secir_simple_grid_search_extensive_part2')

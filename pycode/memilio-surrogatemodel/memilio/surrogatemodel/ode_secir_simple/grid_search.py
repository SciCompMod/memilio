...
# we want to use 80% of the dataset for grid serach with cross calidation
# the other 20% will be used for testing purposes afterwards
...
# select 80% of the data for grid serach with CV
grid_search_inputs = data['inputs'][:(80*len(data['inputs']))]
grid_search_labels = data['labels'][:(80*len(data['labels']))]

# save remaining 20 % of data as test data set
test_inputs = data['inputs'][(80*len(data['inputs'])):]
with open(("test_inputs_secir_simple_"+str(label_width)+"_days"), 'wb') as f:
    pickle.dump(test_inputs, f)

test_labels = data['labels'][(80*len(data['labels'])):]
with open(("test_labels_secir_simple_"+str(label_width)+"_days"), 'wb') as f:
...
def train_and_evaluate_model(param, max_epochs):
    ...
    for train_idx, test_idx in zip(train_idxs, test_idxs):
        ...
        train_inputs = tf.gather(
            data['inputs'], indices=train_idx[:(int(0.8*len(train_idx)))])
        train_labels = tf.gather(
            data['labels'], indices=train_idx[:(int(0.8*len(train_idx)))])
        valid_inputs = tf.gather(
            data['inputs'], indices=train_idx[(int(0.8*len(train_idx))):])
        valid_labels = tf.gather(
            data['labels'], indices=train_idx[(int(0.8*len(train_idx))):])

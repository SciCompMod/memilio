...
def run_secir_simple_simulation(days):
   ...
   """
    ...
    # first, initialize the number of infected individuals
    # next, select the number of individuals in the other compartments  based on that value
    model.populations[A0, Index_InfectionState(
        InfectionState.InfectedSymptoms)] = populations * random.uniform(0.0001, 0.05)

    model.populations[A0, Index_InfectionState(
        InfectionState.Exposed)] = model.populations[A0, InfectionState.InfectedSymptoms].value * random.uniform(0.1, 5)

    model.populations[A0, Index_InfectionState(
        InfectionState.InfectedNoSymptoms)] = model.populations[A0, InfectionState.InfectedSymptoms].value * random.uniform(0.1, 5)

    model.populations[A0, Index_InfectionState(
        InfectionState.InfectedSevere)] = model.populations[A0, InfectionState.InfectedSymptoms].value * random.uniform(0.001, 1)

    model.populations[A0, Index_InfectionState(
        InfectionState.InfectedCritical)] = model.populations[A0, InfectionState.InfectedSevere].value * random.uniform(0.001, 1)

    model.populations[A0, Index_InfectionState(
        InfectionState.Dead)] = model.populations[A0, InfectionState.InfectedCritical].value * random.uniform(0.001, 1)

    subtotal = (model.populations[A0, InfectionState.InfectedSymptoms].value
                + model.populations[A0, InfectionState.Exposed].value
                + model.populations[A0,
                                    InfectionState.InfectedNoSymptoms].value
                + model.populations[A0, InfectionState.InfectedSevere].value
                + model.populations[A0, InfectionState.InfectedCritical].value
                + model.populations[A0, InfectionState.Dead].value
                )
    model.populations[A0, Index_InfectionState(
        InfectionState.Recovered)] = random.uniform(0, (populations-subtotal))
...
def generate_data(
   ...
   """
    ...
    if save_data:
        ...
        with open(os.path.join(path, 'data_secir_simple_90days_I_based_100k.pickle'), 'wb') as f:
            pickle.dump(data, f)
    return data

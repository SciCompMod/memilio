...
from memilio.simulation import (AgeGroup, Damping, LogLevel, set_log_level, MigrationParameters)
...
def run_secir_groups_simulation(days, populations):
   ...
   """
    ...
    for data, position in zip(np.asarray(commuter_data).flatten(), positions):

        migration_coefficients = 0.05 * np.ones(model.populations.numel())
        migration_params = MigrationParameters(migration_coefficients)
        graph.add_edge(position[0], position[1], migration_params)
    ...
    # for i in range(400):
    #     for j in range(400):            
    #         migration_coefficients = np.asarray(commuter_data).flatten()[index] * np.ones(model.populations.numel())
    #         migration_params = MigrationParameters(migration_coefficients)
    #         graph.add_edge(i, j, migration_params)
    #         index += 1
    ...
    for result_indx in range(len(results)):
        results[result_indx] = remove_confirmed_compartments(
            np.transpose(results[result_indx].as_ndarray()[1:, :]), 6)
...
def generate_data(
   ...
   """
    ...
    if save_data:
            ...
            all_data['inputs'].append(scaled_inputs)
            all_data['labels'].append(scaled_labels)
...
if __name__ == "__main__":
    ...
    path_data = os.path.join(
        ...
        'data_GNN_nodamp_400pop_30days_2024_withputsetedges')

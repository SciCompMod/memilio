...
path_data = os.path.join(
    ...
    'data/data_GNN_20pop_one_var_damp_30days_1k')
...
mean_contact_pP = sum(
   pop_sum_per_agegroup*contacts_per_agegroup)/sum(pop_sum_per_agegroup)
...
#### add damping: calculate an array of edge weights for each edge #####
# each edge has an array as edge attribute with the length = number of days
# array[0] represents the number of people commutin on day t=0


num_days = 30
all_egde_arrays = []
for damp, day in zip(dampings, damping_days):
    edges_array = []
    ...
    all_egde_arrays.append(edges_array)

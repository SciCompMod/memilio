...
# in this file, we want to find out which type of layer that utilized edge weights is the best 
# we take three layers into condsideration that use edge weights in their calculations: 
# XENetConv, CensNetConv and ECCConv layer (look @homepage from spektral for more details)
# since we have a very densely connected Graph, the computational costs are extremly high for a model with 400 nodes 
# this is why we make an analysis on a subset of nodes, which results in a graph with xxx nodes 
...
new_inputs = np.asarray(
    data_secir['inputs'][:numer_of_nodes]).reshape(
    len_dataset, numer_of_nodes, shape_input_flat)
new_labels = np.asarray(data_secir['labels'][:numer_of_nodes]).reshape(
...
edge_features_o = df.rename_axis('Source')\
    .reset_index()\
    .melt('Source', value_name='Weight', var_name='Target')\
    .reset_index(drop=True)


edge_features = edge_features_o[
    edge_features_o.Weight != 0]  # 157864 edges
...
df_edges = edge_features_o.pivot(index='Source', columns='Target', values='Weight')
...
def train_and_evaluate_model(
    ...
    layer_name, channels, number_of_layer = param
    if layer_name == 'XENetConvBatch':
        layer = XENetConvBatch
    elif layer_name =='CensNetConv':
        layer = CensNetConv
    elif layer_name == 'ECCConv':
        layer = ECCConv
    ...
    class MyDataset(spektral.data.dataset.Dataset):
        def read(self):
            if layer == XENetConvBatch:
                ...
                self.a = adjacency_matrix
                self.e = df_edges.values.reshape(50,50,1)
    ...
    df.loc[len(df.index)] = [layer_name, number_of_layer, channels, np.mean(train_losses),
                             np.mean(val_losses),
                             np.mean(test_scores),
                             (elapsed / 60), 
...
epochs = 2
filename = '/dataframe_layertesting_subset_withedges_test.csv'

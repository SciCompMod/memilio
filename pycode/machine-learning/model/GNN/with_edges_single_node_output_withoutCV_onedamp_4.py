...
# with damping, and we implement the damping by adding the damping day and the damping factor the node features
# and contact matrix 
...
file = open(
    os.path.join(path_data, 'data_secir_age_groups.pickle'),
    'rb')
...
train_contact_matrix = data_secir['damped_matrix']
...
test_contact_matrix = data_secir['damped_matrix']
...
## add damping factor and damping day 
node_features_train_withdamp = []
for graph, damp, day, matrix in zip(node_features_train, train_damping_coeff, train_damping_days, train_contact_matrix):
    for node in graph: 
        node_features_train_withdamp.append(np.append(node, [damp, day, matrix]))

node_features_train_withdamp = np.asarray(node_features_train_withdamp).reshape([node_features_train.shape[0], node_features_train.shape[1],np.asarray(node_features_train_withdamp).shape[1]])


node_features_test = new_inputs_test

## add damping factor and damping day 
node_features_test_withdamp = []
for graph, damp, day, matrix in zip(node_features_test, test_damping_coeff, test_damping_days, test_contact_matrix):
    for node in graph: 
        node_features_test_withdamp.append(np.append(node, [damp, day]))
        
node_features_test_withdamp = np.asarray(node_features_test_withdamp).reshape([node_features_test.shape[0], node_features_test.shape[1],np.asarray(node_features_test_withdamp).shape[1]])
...
#     edges_array = []
#     for weight in edge_features['Weight']:

#         edges_array.append([weight, damp, day])
...
class MyDataset_train(spektral.data.dataset.Dataset):
    def read(self):
        ...
        return [spektral.data.Graph(x=x, y=y, e=self.e, a=self.a) for x, y in zip(node_features_train_withdamp, node_labels_train)]
...
class MyDataset_test(spektral.data.dataset.Dataset):
    def read(self):
        ...
        return [spektral.data.Graph(x=x, y=y, e=self.e, a=self.a) for x, y in zip(node_features_test_withdamp, node_labels_test)]
...
shape_node_features = node_features_train_withdamp.shape[2]
################################################################################
# Build model
################################################################################
class Net(Model):

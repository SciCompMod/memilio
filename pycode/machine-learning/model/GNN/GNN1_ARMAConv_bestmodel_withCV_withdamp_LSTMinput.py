...
#     'data_GNN_400pop_var_5damp_100days_1k')


# file = open(os.path.join(path_data, 'GNN_400pop_5damp.pickle'), 'rb')
...
# file = open(os.path.join(path_data, 'GNN_400pop_5damp_part2.pickle'), 'rb')

# data_secir_part2 = pickle.load(file)

# # for damp = 5 
# data_secir['inputs'] = np.concatenate((data_secir['inputs'], data_secir_part2['inputs'][1:]))
# data_secir['labels'] = np.concatenate((data_secir['labels'], data_secir_part2['labels'][1:]))
# data_secir['damping_coeff'] = np.concatenate((data_secir['damping_coeff'], data_secir_part2['damping_coeff'][1:]))
# data_secir['damping_day'] = np.concatenate((data_secir['damping_day'], data_secir_part2['damping_day'][1:]))
# data_secir['damped_matrix'] = np.concatenate((data_secir['damped_matrix'], data_secir_part2['damped_matrix'][1:]))
...
if len(np.asarray(data_secir['damping_day']).shape) == 2 :
    n_dampings = 1 
else:
    n_dampings =np.asarray(data_secir['damping_day']).shape[2]

input_days =np.asarray(data_secir['inputs']).shape[2] 


inputs_with_damp = np.concatenate((np.asarray(data_secir['inputs']).reshape([n_runs, n_pop, input_days, 48]),
                              (np.repeat(np.asarray(damping_factors).reshape([n_runs,n_pop,n_dampings])[:, :, :,np.newaxis],input_days , axis=2)),
                               (np.repeat(np.asarray(damping_days).reshape([n_runs,n_pop,n_dampings])[:, :, :,np.newaxis],input_days , axis=2)),
                               (np.repeat(np.asarray(contact_matrices).reshape([n_runs,n_pop,36*n_dampings])[:, :, :,np.newaxis],input_days , axis=2)).reshape([n_runs, n_pop, input_days,36*n_dampings])), axis = 3)

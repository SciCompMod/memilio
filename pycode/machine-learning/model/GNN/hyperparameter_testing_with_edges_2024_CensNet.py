    ...
    if number_of_layer == 1: 
        class Net(Model):
            ...
            def call(self, inputs):
                x, a , e= inputs
                a = np.asarray(a)
                e = tf.reshape(e, [e.shape[0], e.shape[1]*e.shape[2], e.shape[3]]) # for censnet

                laplacian, edge_laplacian, incidence = preprocess(a)
                matrix_tuple = [laplacian, edge_laplacian, incidence]
...
epochs = 4
filename = '/dataframe_CensNet_Baseline_test.csv'

...
def plot_icu_on_germany(simulations):
    ...
    plt.savefig(f"{name}/median_icu_germany_nuts3.png", bbox_inches='tight', dpi=dpi)
    ...
    cbar_fig.savefig("colorbar_median_icu_nuts3.png", bbox_inches='tight', dpi=dpi)
...
def run_inference(num_samples=1000):
    ...
    plot_aggregated_over_regions(
        simulations, true_data=divi_data, ax=ax, color=colors["Red"]
    )

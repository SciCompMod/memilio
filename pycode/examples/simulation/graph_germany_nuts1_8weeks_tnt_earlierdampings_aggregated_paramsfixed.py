...
class Simulation:
    ...
    def set_covid_parameters(self, model, t_E, t_ISy, t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob):
        ...
        model.parameters.DeathsPerCritical[mio.AgeGroup(0)] = 0.2
    ...
    def get_graph(self, end_date, scaling_factor, t_E, t_ISy, t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob):
        print("Initializing model...")
        model = Model(self.num_groups)
        self.set_covid_parameters(
            model, t_E, t_ISy, t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob)
    ...
    def run(self, num_days_sim, scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob, export_timeseries):
...
def run_germany_nuts1_simulation(scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob, export_timeseries=False):
    mio.set_log_level(mio.LogLevel.Warning)
    ...
    results = sim.run(num_days_sim, scaling_factor, damping_values, t_E, t_ISy,
                      t_ISev, t_Cr, mu_IH, mu_HU, transmission_prob, export_timeseries)
...
def run_training(num_training_files=20):
    ...
    history = workflow.fit_offline(
        data=trainings_data, epochs=700, batch_size=64, validation_data=validation_data
    )
...
def run_inference(num_samples=1000, on_synthetic_data=False):
    ...
    if os.path.exists(f'{name}/backup/sims_{name}{synthetic}_with_aug.pickle') and os.path.exists(f'{name}/backup/sims_{name}{synthetic}.pickle') and os.path.exists(f'{name}/backup/samples_{name}{synthetic}.pickle'):
        simulations = load_pickle(f'{name}/backup/sims_{name}{synthetic}.pickle')
        simulations_aug = load_pickle(
            f'{name}/backup/sims_{name}{synthetic}_with_aug.pickle')
        samples = load_pickle(f'{name}/backup/samples_{name}{synthetic}.pickle')
        ...
        for i in range(num_samples):  # we only have one dataset for inference here
            result = run_germany_nuts1_simulation(
                ...
                mu_IH=samples['mu_IH'][0, i], mu_HU=samples['mu_HU'][0, i],
        ...
        best_sim = run_germany_nuts1_simulation(
            ...
            mu_IH=samples['mu_IH'][0, sim_idx], mu_HU=samples['mu_HU'][0, sim_idx],

...
name = "nuts0_tnt_newnetworks_8weeks_earlierdampings"
...
def plot_map(values, state_data, map_data, fedstate_data, ax, label, vmin, vmax):
...
def run_inference(num_samples=1000, on_synthetic_data=False):
    ...
    if on_synthetic_data:
        ...
        validation_data['damping_values'] = validation_data['damping_values'].reshape((validation_data['damping_values'].shape[0], -1))
        validation_data = {k: v[0][np.newaxis, ...] if k == 'state0' else v for k, v in validation_data.items()}  # only one dataset
    ...
    if os.path.exists(f'{name}/sims_{name}{synthetic}_with_aug.pickle') and os.path.exists(f'{name}/sims_{name}{synthetic}.pickle') and os.path.exists(f'{name}/samples_{name}{synthetic}.pickle'):
        ...
        best_sim = run_germany_nuts0_simulation(
            scaling_factor=samples['scaling_factor'][0, i],
    ...
    plot_region_fit(
        simulations_aug, region=0, true_data=divi_data, label="Region Aggregated Median (With Aug)", ax=ax, color=colors["Red"]
    )    
    lines, labels = ax.get_legend_handles_labels()
    fig.legend(lines, labels, loc='upper left', bbox_to_anchor=(0.115, 0.95), ncol=1)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(name):
        os.makedirs(name)
    # create_train_data(filename=f'{name}/trainings_data2_{name}.pickle', number_samples=10000)
    # run_training(num_training_files=2)
    run_inference(on_synthetic_data=True, num_samples=1000)

...
name = "spain_nuts0_earlierdampings_biggerinfnet"
...
comunidad_ids = [region_id for region_id in dd.Comunidades.keys()
                 if region_id not in excluded_comunidades]
region_ids = [0]
...
def plot_region_fit(
    ...
    ax.plot(
        x, med, label="Simulation", color=color)
...
def plot_icu_on_spain(simulations, synthetic, with_aug):
    ...
    population = pd.read_json(
        'data/Spain/pydata/spain_current_population.json')
...
def plot_map(values, map_data, fedstate_data, ax, label, vmin, vmax):
    map_data[label] = map_data['ID_Provincia'].map(
        {region_id: values[0] for region_id in provincia_ids})
...
class Simulation:
    ...
    def set_npis(self, params, end_date, damping_values):
        start_damping_1 = DATE_TIME + datetime.timedelta(days=0)
        start_damping_2 = DATE_TIME + datetime.timedelta(days=20)
        start_damping_3 = DATE_TIME + datetime.timedelta(days=40)
    ...
    def get_graph(self, end_date, scaling_factor, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob):
        ...
        path_population_data = os.path.join(
            pydata_dir, "spain_current_population.json")

        print("Setting nodes...")
        mio.osecir.set_node_spain(
            model.parameters,
...
def run_spain_nuts0_simulation(scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob, export_timeseries=False):
    mio.set_log_level(mio.LogLevel.Warning)
...
def load_divi_data():
    ...
    data = pd.read_json(os.path.join(divi_path, "spain_icu_ma7.json"))
...
def create_train_data(filename, number_samples=1000):
    ...
    simulator = bf.simulators.make_simulator(
        [prior, run_spain_nuts0_simulation]
    )
...
def get_workflow():
    ...
    workflow = bf.BasicWorkflow(
        ...
        standardize='all'
        # augmentations={f'state{i}': aug for i in range(len(region_ids))}
    )
...
def run_inference(num_samples=1000, on_synthetic_data=False):
    ...
    if on_synthetic_data:
        ...
        validation_data['damping_values'] = validation_data['damping_values'].reshape(
            (validation_data['damping_values'].shape[0], -1))
        validation_data = {k: v[0][np.newaxis, ...] if k ==
                           'state0' else v for k, v in validation_data.items()}  # only one dataset
    ...
    if os.path.exists(f'{name}/sims_{name}{synthetic}_with_aug.pickle') and os.path.exists(f'{name}/sims_{name}{synthetic}.pickle') and os.path.exists(f'{name}/samples_{name}{synthetic}.pickle'):
        ...
        for i in range(num_samples):  # we only have one dataset for inference here
            result = run_spain_nuts0_simulation(
        ...
        best_sim = run_spain_nuts0_simulation(
            scaling_factor=samples['scaling_factor'][0, i],
    ...
    plot_region_fit(
        simulations_aug, region=0, true_data=divi_data, label="Median", ax=ax, color=colors["Red"]
    )
    lines, labels = ax.get_legend_handles_labels()
    fig.legend(lines, labels, loc='upper left',
               bbox_to_anchor=(0.11, 0.99), ncol=1)
...
if __name__ == "__main__":
    ...
    # run_training(num_training_files=2)

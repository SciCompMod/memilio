...
name = "spain_nuts1_excludedregions"

excluded_provincias = [530, 630, 640, 701, 702]
provincia_ids = [region_id for region_id in dd.Provincias.keys()
                 if region_id not in excluded_provincias]
excluded_comunidades = [53, 63, 64, 70]
no_icu_comunidad = [22]
region_ids = [region_id for region_id in dd.Comunidades.keys() if region_id not in excluded_comunidades]

inference_params = ['scaling_factor', 'damping_values', 't_E', 't_ISy', 't_ISev',
                    't_Cr', 'mu_CR', 'mu_IH', 'mu_HU', 'mu_UD', 'transmission_prob']

summary_vars = [f'comunidad{i}' for i in range(len(region_ids)) if region_ids[i] not in no_icu_comunidad] + ['state']
...
def plot_region_fit(
    ...
    ax.plot(
        x, med, lw=2, label=label or f"{dd.Comunidades[region_ids[region]]}", color=color)
    ...
    ax.set_xlabel("Time")
    ax.set_ylabel("ICU")
    ax.set_title(f"{dd.Comunidades[region_ids[region]]}")
...
def plot_icu_on_spain(simulations, synthetic, with_aug):
    ...
    population = pd.read_json(
        'data/Spain/pydata/comunidades_current_population.json')
    ...
    plt.savefig(f"{name}/median_icu_spain_{name}{synthetic}{with_aug}.png", bbox_inches='tight', dpi=dpi)
...
def plot_map(values, map_data, fedstate_data, ax, label, vmin, vmax):
    map_data[label] = map_data['ID_Provincia'].map(
        {region_id: values[region_ids.index(region_id // 10)] for region_id in provincia_ids})

    map_data['state_id'] = map_data['ID_Provincia'].astype(
        int).map({county: county // 10 for county in dd.Provincias.keys()}).fillna(0).astype(int)
...
def plot_damping_values(damping_values, synthetic):
    ...
    for i, ax in enumerate(axes):
        if i < len(region_ids):
            ...
            ax.set_title(f"{dd.Comunidades[region_ids[i]]}")
            ax.set_xlabel("Time")
    ...
    for i, comunidad in enumerate(region_ids):
        ax.stairs(
...
def best_fit_sim(results, true_data, ax=None):
    rmse_per_region = np.sqrt(
        np.mean((results - true_data[None, :, :])**2, axis=1))
    ...
    ax.plot(x, agg_over_regions[rmse_agg_idx], lw=2,
            label="First rmse then aggregated", color="red")
...
class Simulation:
    ...
    def get_graph(self, end_date, scaling_factor, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob):
        ...
        data_dir_Spain = os.path.join(self.data_dir, "Spain")
        mobility_data_file = os.path.join(
            data_dir_Spain, "mobility", "commuter_mobility_comunidades.txt")
        pydata_dir = os.path.join(data_dir_Spain, "pydata")

        path_population_data = os.path.join(
            pydata_dir, "comunidades_current_population.json")

        print("Setting nodes...")
        mio.osecir.set_nodes_comunidades(
            model.parameters,
    ...
    def run(self, num_days_sim, scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob, export_timeseries):
        ...
        for node_idx in range(mobility_sim.graph.num_nodes):
            ...
            if node.id in no_icu_comunidad:
                results[f'no_icu_comunidad{node_idx}'] = osecir.interpolate_simulation_result(
                    node.property.result)
            else:
                results[f'comunidad{node_idx}'] = osecir.interpolate_simulation_result(
                    node.property.result)

        return results


def run_spain_nuts1_simulation(scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob, export_timeseries=False):
    mio.set_log_level(mio.LogLevel.Warning)
...
def load_divi_data():
    ...
    data = pd.read_json(os.path.join(divi_path, "comunidades_icu_ma7.json"))
    ...
    for i, region_id in enumerate(region_ids):
        if region_id not in no_icu_comunidad:
            divi_dict[f"comunidad{i}"] = divi_data[region_id].to_numpy()[None, :, None]
        else:
            divi_dict[f"no_icu_comunidad{i}"] = np.zeros((1, divi_data.shape[0], 1))
    return divi_dict
...
def create_train_data(filename, number_samples=1000):
    ...
    simulator = bf.simulators.make_simulator(
        [prior, run_spain_nuts1_simulation]
    )
...
def is_comunidad_key(k: str) -> bool:
    return 'comunidad' in k


def apply_aug(d: dict, aug) -> dict:
    return {k: np.clip(aug(v), 0, None) if is_comunidad_key(k) else v for k, v in d.items()}
...
def skip_2weeks(d: dict) -> dict:
    return {k: v[:, 14:, :] if is_comunidad_key(k) else v for k, v in d.items()}


def aggregate_states(d: dict) -> None:
    d["state"] = np.sum([d[f"comunidad{r}"] if region_ids[r] not in no_icu_comunidad else d[f"no_icu_comunidad{r}"] for r in range(len(region_ids))], axis=0)

def region_keys_sorted(d: dict):
    def idx(k):
        # handles "regionN" and "no_icu_regionN"
        return int(k.split("comunidad")[-1])
    return sorted([k for k in d if is_comunidad_key(k)], key=idx)
...
def get_workflow():
    ...
    workflow = bf.BasicWorkflow(
        ...
        standardize='all'
        # augmentations={f'comunidad{i}': aug for i in range(len(region_ids))}
    )
...
def run_inference(num_samples=1000, on_synthetic_data=False):
    ...
    if os.path.exists(f'{name}/sims_{name}{synthetic}_with_aug.pickle') and os.path.exists(f'{name}/sims_{name}{synthetic}.pickle') and os.path.exists(f'{name}/samples_{name}{synthetic}.pickle'):
        ...
        samples = workflow.sample(
            conditions=divi_dict, num_samples=num_samples)
        ...
        samples['damping_values'] = samples['damping_values'].reshape(
            (samples['damping_values'].shape[0], num_samples, len(region_ids), NUM_DAMPING_POINTS))
        results = []
        for i in range(num_samples):  # we only have one dataset for inference here
            result = run_spain_nuts1_simulation(
        ...
        for i in range(num_samples):
            ...
            simulations_aug[i] = np.concatenate(
                [results_aug[key][i] for key in results.keys()], axis=-1)
        ...
        best_sim = run_spain_nuts1_simulation(
            scaling_factor=samples['scaling_factor'][0, i],
            damping_values=samples['damping_values'][0, sim_idx],
            t_E=samples['t_E'][0, sim_idx], t_ISy=samples['t_ISy'][0, sim_idx],
            t_ISev=samples['t_ISev'][0,
                                     sim_idx], t_Cr=samples['t_Cr'][0, sim_idx],
            mu_CR=samples['mu_CR'][0,
                                   sim_idx], mu_IH=samples['mu_IH'][0, sim_idx],
            mu_HU=samples['mu_HU'][0,
                                   sim_idx], mu_UD=samples['mu_UD'][0, sim_idx],
            transmission_prob=samples['transmission_prob'][0,
                                                           sim_idx], export_timeseries=True
        )
    ...
    plot_icu_on_spain(simulations, synthetic, with_aug="")
    plot_icu_on_spain(simulations_aug, synthetic, with_aug="_with_aug")

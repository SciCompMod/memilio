...
def run_secir_groups_simulation():
    ...
    for i in range(num_groups):
        ...
        model.parameters.InfectiousTimeMild[AgeGroup(i)] =  6.  # 4-14  (=R4^(-1))
        model.parameters.SerialInterval[AgeGroup(i)] = 4.2   # 4-4.4 // R_2^(-1)+0.5*R_3^(-1)
        model.parameters.HospitalizedToHomeTime[AgeGroup(i)] = 12.  # 7-16 (=R5^(-1))
        model.parameters.HomeToHospitalizedTime[AgeGroup(i)] = 5.  # 2.5-7 (=R6^(-1))
        model.parameters.HospitalizedToICUTime[AgeGroup(i)] = 2.  # 1-3.5 (=R7^(-1))

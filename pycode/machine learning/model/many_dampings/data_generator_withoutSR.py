...
def run_secir_groups_simulation(days, damping_days, populations):
    ...
    for i in range(num_groups):
        ...
        # Initial number of peaople in each compartment
        # model.populations[AgeGroup(i), State.Exposed] = random.uniform(
        #    0.00025, 0.05) * populations[i]
        # model.populations[AgeGroup(i), State.Carrier] = random.uniform(
        #    0.0001, 0.035) * populations[i]
        # model.populations[AgeGroup(i), State.Infected] = random.uniform(
        #    0.00007, 0.01) * populations[i]
        # model.populations[AgeGroup(
        #    i), State.Hospitalized] = random.uniform(
        #    0.00003, 0.006) * populations[i]
        # model.populations[AgeGroup(i), State.ICU] = random.uniform(
        #    0.00001, 0.002) * populations[i]
        # model.populations[AgeGroup(i), State.Recovered] = random.uniform(
        #    0.002, 0.8) * populations[i]
        #model.populations[AgeGroup(i), State.Dead] = 0
        # model.populations.set_difference_from_group_total_AgeGroup(
        #    (AgeGroup(i), State.Susceptible), populations[i])
...
def generate_data(num_runs, path, input_width, days, number_of_dampings,
   ...
   """
    ...
    for _ in range(num_runs):
        ...
        for i in labels:
            # delete susceptible
            withoutS = np.delete(np.array(i), np.arange(0, i.size, 8))
            # delete recovered
            withoutSR = np.delete(withoutS, np.arange(5, withoutS.size, 7))
            new_labels.append(withoutSR)
    ...
    if save_data:
        ...
        scaled_labels = scaled_labels.transpose().reshape(
            num_runs, np.asarray(data['labels']).shape[1], 36)

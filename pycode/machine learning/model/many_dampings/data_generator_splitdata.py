...
def generate_data(num_runs, path, input_width, number_of_dampings,
                  save_data=True):
   ...
   """
    ...
    days = (input_width + label_width)*number_of_dampings
    # days = damping_days[-1]+label_width for flexible days (difficult on training because output/label_dimension is not the same)
    ...
    for _ in range(num_runs):
        ...
        damping_days[0] = input_width
        for i in range(1, number_of_dampings):
            damping_days[i] = damping_days[(i-1)] + (input_width+label_width)
        ...
        for i in range(0, number_of_dampings):
            data['inputs'].append(
                data_run[(damping_days[i]-input_width):damping_days[i]])

        for i in range(0, number_of_dampings):
            data['labels'].append(
                data_run[damping_days[i]:(damping_days[i]+label_width)])
    ...
    if save_data:
        ...
        scaled_inputs = scaled_inputs.transpose().reshape(
            num_runs * number_of_dampings, input_width, 48)
        ...
        scaled_labels = scaled_labels.transpose().reshape(
            num_runs*number_of_dampings, np.asarray(data['labels']).shape[1], 48)

...
def run_secir_groups_simulation(days, damping_days, populations):
    """
    ...
    correct_damping_matrix = []
    for i in damping_days:
        ...
        model.parameters.ContactPatterns.cont_freq_mat.add_damping(Damping(
            coeffs=(damping), t=i, level=0, type=0))

        correct_damping_matrix.append(
    ...
    return dataset, correct_damping_matrix
...
def generate_data(num_runs, path, input_width, days, number_of_dampings,
                  save_data=True):
   ...
   """
    ...
    for _ in range(num_runs):
        ...
        damping_days[0] = (5)
        # for creating three datasets out od nine dampings
        for i in range(1, 3):
            damping_days[i] = damping_days[(i-1)]+randrange(14, 21)
        ...
        data["inputs"].append(data_run[80:85])
        # data["inputs"].append(data_run[160:165])

        data["labels"].append(data_run[input_width:80])
        data["labels"].append(data_run[85:160])
        # data["labels"].append(data_run[165:])

        data['contact_matrix'].append(damping_matrix[:3])
        data['contact_matrix'].append(damping_matrix[3:6])
        # data['contact_matrix'].append(damping_matrix[6:])

        data['damping_days'].append(damping_days[:3])
        data['damping_days'].append(damping_days[3:6])
        # data['damping_days'].append(damping_days[6:])
    ...
    if save_data:
        ...
        scaled_inputs = scaled_inputs.transpose().reshape(num_runs*2, input_width, 48)
        ...
        scaled_labels = scaled_labels.transpose().reshape(
            num_runs*2, np.asarray(data['labels']).shape[1], 48)
...
if __name__ == "__main__":
    ...
    path_data = os.path.join(
        ...
        'data_groups_with_damping_6damp_split_100k')

    input_width = 5
    days = 190
    num_runs = 100000
    number_of_dampings = 6
    generate_data(num_runs, path_data, input_width,
                  days, number_of_dampings)

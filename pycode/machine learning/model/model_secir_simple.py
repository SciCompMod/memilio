...
def getPartitions(input_width, label_width,
    ...
    input_indices, labels_indices = _getPartitionIndices(data,input_width,label_width)
    if input_width == 1:
        inputs = data.iloc[input_indices]
        labels = data.iloc[labels_indices].iloc[:,:-1]
    else:
        inputs = []
        for val in range(int(input_indices.shape[0] / input_width)):
            start_idx = val * (input_width + label_width) 
            input_idx = np.arange(start_idx , start_idx + input_width, dtype=int)
            inputs.append(data.iloc[input_idx]) # maybe cast to_numpy() before

        labels = []
        for val in range(int(labels_indices.shape[0] / label_width)):
            start_idx = val * (input_width + label_width) + input_width
...
def _getPartitionIndices(data, input_width, label_width):
    n = len(data)
    input_indices = np.arange(0, n, input_width + label_width, dtype=int)
    for input_indx in range(1,input_width):
        input_indices_indx = np.arange(0+input_indx, n, input_width + label_width, dtype=int)
        input_indices = np.concatenate((input_indices, input_indices_indx), axis=None)
    
    label_indices = np.arange(input_width, n,input_width + label_width , dtype=int)
    for label_indx in range(1,label_width):
        label_indices_indx = np.arange(input_width + label_indx, n, input_width + label_width, dtype=int)
        label_indices = np.concatenate((label_indices, label_indices_indx), axis=None)

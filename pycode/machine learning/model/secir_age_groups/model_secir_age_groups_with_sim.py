...
def simulation(model):
    ...
    # copied from data generation for simulation --> more elegant solution needed
    t1 = 5
    t2 = 20
    t3 = 40
    ...
    while i < len(sim_inputs_compartments):
        ...
        first_days = t2-t1
        results = np.append(
            results, (first_output.reshape(1440)[:(first_days*6*8)]))
        ...
        second_days = t3-t2
        results = np.append(
            results, (second_output.reshape(1440)[:(second_days*6*8)]))
        ...
        labels_run = np.asarray(sim_labels[i])
        third_days = int((len(labels_run)/48)+5 - t3)  # 5 = label_width
        results = np.append(
            results, (third_output.reshape(1440)[:(third_days*6*8)]))
        ...
        i += 1
    print('Simuation MAPE: ', mean_percentage_error.mean()*100)

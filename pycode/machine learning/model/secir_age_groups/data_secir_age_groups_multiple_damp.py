...
def run_secir_groups_simulation(days, t2, t3, t4,  populations):
    """
    ...
    damping4 = np.ones((num_groups, num_groups)
                       ) * np.float16(random.uniform(0, 0.5))
    ...
    model.parameters.ContactPatterns.cont_freq_mat.add_damping(Damping(
        coeffs=(damping4), t=t4, level=0, type=0))
    ...
    correct_damping_matrix3 = model.parameters.ContactPatterns.cont_freq_mat.get_matrix_at(
        t3+1)
    correct_damping_matrix4 = model.parameters.ContactPatterns.cont_freq_mat.get_matrix_at(
        t4+1)
    ...
    return dataset,  correct_damping_matrix2, correct_damping_matrix3, correct_damping_matrix4


# create data with only one damping
def run_secir_groups_simulation_simple(days, t1,  populations):
    """
    Runs the c++ secir model using mulitple age groups
    and plots the results
    """
...
def generate_data(num_runs, path, input_width, label_width,
   ...
   """
    ...
    for _ in range(num_runs):
        ...
        data_run,  damping_matrix2, damping_matrix3, damping_matrix4 = run_secir_groups_simulation(
            days, t2, t3, t4,  population[random.randint(0, len(population) - 1)])

        baseline = getBaselineMatrix()

        data["contact_matrix"].append(
            [baseline, damping_matrix2])

        data["contact_matrix"].append(
            [damping_matrix2, damping_matrix3])

        data["contact_matrix"].append(
            [damping_matrix3, damping_matrix4])

        inputs_1 = data_run[:input_width]
        data["inputs"].append(inputs_1)

        inputs_2 = data_run[(t3-input_width):t3]
        data["inputs"].append(inputs_2)

        inputs_3 = data_run[(t4-input_width):t4]
        data["inputs"].append(inputs_3)

        # inputs = data_run[:input_width]
        # data["inputs"].append(inputs)

        # data["labels"].append(data_run[input_width:])
        data['labels'].append(data_run[input_width:(t3-input_width)])
        data['labels'].append(data_run[t3:(t3+label_width)])
        data['labels'].append(data_run[t4:(t4+label_width)])
        # bar.next()

        # add data with no damping at t= 0
        t_new = 5
        ...
        data_run_two, damping_matrix1_new = run_secir_groups_simulation_simple(
            days_new, t_new, population[random.randint(0, len(population) - 1)])

        data["contact_matrix"].append(
            [baseline, damping_matrix1_new])

        inputs_new = data_run_two[:input_width]
        data["inputs"].append(inputs_new)

        # inputs = data_run[:input_width]
        # data["inputs"].append(inputs)

        data["labels"].append(data_run_two[input_width:])
    ...
    if save_data:
        ...
        scaled_inputs = scaled_inputs.transpose().reshape(num_runs*4, input_width, 48)
        ...
        scaled_labels = scaled_labels.transpose().reshape(num_runs*4, label_width, 48)

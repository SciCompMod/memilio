...
df_npis_old = pd.read_csv(
    os.path.join(directory, 'old', 'kr_massnahmen_unterkategorien.csv'),
    sep=',')  # , nrows=numberofcities*1248
print(df_npis_old)
numberofcities = int(len(df_npis_old) / 1248)
print('Number of cities', numberofcities)

list = ['m01a', 'm01b', 'm02a', 'm02b', 'm03', 'm04', 'm05', 'm06', 'm07', 'm08', 'm09',
...
for i in range(numberofcities):
    ...
    for code in list:
        ...
        # initialization of new data frame:
        ...
        df.iloc[i*1248 + counter: i*1248 + counter + numberofcodes,
                6:] = df_npis_new.iloc[0:numberofdays, 6:].T

        #print(df.iloc[i*1248 + counter: i*1248 + counter + numberofcodes])

        counter += numberofcodes
print(df)
...
for i in range(numberofcities):
   ...
   # print('indices', (i)*1248, (i+1)*counter)
    df_lost_subcat = df_lost_subcat.drop(
        df_lost_subcat.index[range(i*(1248-counter), i*(1248-counter) + counter)])
...
for i in range(numberofcities):
    try:
        if pd.testing.assert_frame_equal(pd.DataFrame(df_npis_old_dropped.iloc[i*counter: (i+1)*counter, :]), pd.DataFrame(df_dropped.iloc[i*counter: (i+1)*counter, :]), check_dtype=False) == None:
            nodiffcities.append(df_npis_old_dropped.iloc[i*counter]['kreis'])
            #print('No difference in column', df_npis_old_dropped.columns[i])

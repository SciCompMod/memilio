...
def get_commuter_data(read_data=dd.defaultDict['read_data'],
                      ...
                      ref_year=2022,
                      **kwargs):
    """! Computes DataFrame of commuter movement patterns based on the Federal
    Agency of Work data.
    ...
    @param setup_dict dictionary with necessary values:
        'path': String with datapath where movement files can be found
    ...
    @return df_commuter_movement DataFrame of commuter movement.
        df_commuter_movement[i][j]= number of commuters from county with county-id i to county with county-id j
    In commuter movement files is a cumulative value per county for number of commuters from whole Germany given.
    ...
    if setup_dict == '':
        abs_tol = 100  # maximum absolute error allowed per county movement
        rel_tol = 0.01  # maximum relative error allowed per county movement
    ...
    commuter_movement_files = [[] for _ in range(len(states))]
    ...
    for state_id_file in range(len(states)):
        ...
        commuter_movement_files[state_id_file] = gd.get_file(
            filepath, url, read_data, param_dict, interactive=conf.interactive)
        if not no_raw:
            gd.write_dataframe(
                commuter_movement_files[state_id_file], mobility_dir, filename, 'json')
    ...
    mat_commuter_movement = np.zeros(
    ...
    with progress_indicator.Spinner() as p:
        for file in range(len(commuter_movement_files)):
            p.set_message('Progress '+str(file+1)+'/16')
            commuter_movement_file = commuter_movement_files[file]

            for i in range(commuter_movement_file.shape[0]):
                if (len(str(commuter_movement_file.iloc[i, 0])) == len_county_id
                        and (commuter_movement_file.iloc[i, 0]).isdigit()):
                    checksum = 0  # sum of county movement from, to be checked against sum in document

                    # make zero-filled list of counties explicitly moved to from county considered
                    # 'implicit' movement means 'movement to' which is summed in a larger regional entity and not given in
                    # detail per county
                    counties_movedfrom = []
                    ...
                    current_key = commuter_movement_file.iloc[i, 0]
                    # movement to itself excluded!
                    counties_movedfrom[countykey2govkey[current_key]][
                        countykey2localnumlist[current_key]] = 1

                if not isinstance(commuter_movement_file.iloc[i, 2], float):
                    # removal of nan's, regional keys are stored as strings

                    # check if entry is a digit
                    if (commuter_movement_file.iloc[i, 2]).isdigit():
                        # explicit movement from county to county
                        if len(str(commuter_movement_file.iloc[i, 2])) == len_county_id:
                            # check if entry refers to a specific county, then set matrix value
                            current_row = countykey2numlist[commuter_movement_file.iloc[i, 2]]
                            # TODO
                        ...
                        # here, some counties of the region are stated explicitly and the rest is summed.
                        elif (str(commuter_movement_file.iloc[i, 3]) == 'Übrige Kreise (Regierungsbezirk)' and str(
                                commuter_movement_file.iloc[i, 4]).isdigit()):
                            # remove trailing zeros
                            remaining_counties = str(
                                commuter_movement_file.iloc[i, 2])
                            ...
                            # distribute out-movement relatively to county population where movement comes from
                            for mapped_county in range(
                                len(
                                    gov_county_table
                                    [govkey2numlist[remaining_counties]])):
                        ...
                        elif (commuter_movement_file.iloc[i, 2] in govkey_list and sum(
                                counties_movedfrom[govkey2numlist[commuter_movement_file.iloc[i, 2]]]) == 0):
                            ...
                            for mapped_county in range(
                                len(
                                    ...
                                    [
                                        govkey2numlist
                                        [commuter_movement_file.iloc[i, 2]]])):
                                if counties_movedfrom[govkey2numlist[commuter_movement_file.iloc[i, 2]]][mapped_county] < 1:
                                    # get identifier (0-401) for county key
                                    globindex = countykey2numlist[gov_county_table[
                        ...
                        elif ((str(commuter_movement_file.iloc[i, 3]) == 'Übrige Regierungsbezirke (Bundesland)' and str(
                                commuter_movement_file.iloc[i, 4]).isdigit())
                              or ((commuter_movement_file.iloc[i, 2]).isdigit() and str(
                                commuter_movement_file.iloc[i - 1, 2]).startswith('nan'))
                              or (len(str(commuter_movement_file.iloc[i, 2])) == 2 and
                                  abs(float(commuter_movement_file.iloc[i, 2]) - float(
                                      commuter_movement_file.iloc[i - 1, 2])) == 1)
                              or (len(str(commuter_movement_file.iloc[i, 2])) == 2 and
                                  abs(float(commuter_movement_file.iloc[i, 2]) - float(
                                      commuter_movement_file.iloc[i - 1, 2])) == 2)):

                            # auxiliary key of Bundesland (key translated to int starting at zero)
                            dummy_key = int(
                                commuter_movement_file.iloc[i, 2]) - 1
                ...
                if str(commuter_movement_file.iloc[i, 3]) == 'Einpendler aus dem Bundesgebiet':
                    abs_err = abs(
                        checksum - commuter_movement_file.iloc[i, 4])
                    ...
                    if abs_err < setup_dict['abs_tol'] and abs_err / checksum < setup_dict['rel_tol']:
                        ...
                        gd.default_print('Warning', 'Error in calculations for county ' + str(curr_county_movedto) +
                                         ...
                                         ', correct sum:' + str(commuter_movement_file.iloc[i, 4]))
                        gd.default_print('Debug', 'Absolute error:' + str(abs_err) +
                                         ', relative error:' + str(abs_err / checksum))

            if np.isnan(mat_commuter_movement).any():
                raise gd.DataError(
    ...
    df_commuter_movement = pd.DataFrame(
        data=mat_commuter_movement, columns=countykey_list, index=countykey_list)
    filename = 'movement_bfa_' + \
        str(ref_year) + '_dim' + str(mat_commuter_movement.shape[0])
    gd.write_dataframe(df_commuter_movement, directory, filename, file_format)
    ...
    if 16056 in countykey_list:
        ...
        mat_commuter_movement = np.delete(
            mat_commuter_movement, old_idx, axis=0)
        mat_commuter_movement = np.delete(
            mat_commuter_movement, old_idx, axis=1)

    countykey_list = geoger.get_county_ids()
    df_commuter_movement = pd.DataFrame(
        data=mat_commuter_movement, columns=countykey_list, index=countykey_list)
    commuter_sanity_checks(df_commuter_movement)
    filename = 'movement_bfa_' + str(ref_year)
    gd.write_dataframe(df_commuter_movement, directory, filename, file_format)
    directory = directory.split('pydata')[0] + 'mobility/'
    gd.check_dir(directory)
    gd.write_dataframe(
        df_commuter_movement, directory,
        'commuter_movement_scaled_' + str(ref_year),
        'txt', {'sep': ' ', 'index': False, 'header': False})

    return df_commuter_movement
...
def get_neighbors_mobility(
    ...
    try:
        commuter = gd.get_file(os.path.join(
            directory, "movement_bfa_"+str(ref_year)+"_dim400.json"), read_data=True)

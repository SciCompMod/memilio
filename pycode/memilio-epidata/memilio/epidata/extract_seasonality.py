...
def naive_model(df_r):
    ...
    decompose_result_mult = seasonal_decompose(data, model="multiplicative", period=365)
...
def plot_seasonality(df_r, x,y, ):
    p, cov = optimize.curve_fit(seasonality_func, np.array(df_r.index.dayofyear.to_list()), y)
    plt.figure(figsize=(6, 4))
    plt.scatter(x, y, label='Data',c='b', alpha=0.5)
    plt.plot(x, seasonality_func(np.array(df_r.index.dayofyear.to_list()), p[0], p[1], p[2], p[3],p[4]), label='Fitted function',c='r')
    plt.axhline(y.mean(), c='b')
    print(p)
    y_new = y-seasonality_func(np.array(df_r.index.dayofyear.to_list()), p[0], p[1], p[2], p[3],p[4])+1
    plt.scatter(x, y_new, c='y', alpha=0.3, label='Resolved data')
    plt.fill_between(x, y.mean()+y.std(), y.mean()-y.std(), color='aqua', alpha=0.3, label='Scattering with seasonality')
    plt.axhline(y_new.mean(), c='g')
    plt.fill_between(x, y_new.mean()+y_new.std(), y_new.mean()-y_new.std(), color='lime', alpha=0.2, label='Scattering without Seasonality')
    plt.legend(loc='best')
...
def adjust_r(df_r, filename):
    ...
    if 'state' in filename:
        ...
        df_pop['ID_State']=df_pop['ID_County'].map(geoger.get_countyid_to_stateid_map())
        df_pop = df_pop[['ID_State', 'Population']].groupby('ID_State').sum()
        df_pop.reset_index(drop=False, inplace=True)
        ratio_by_state=dict(zip(df_pop.ID_State, df_pop.Population/pop_total))
        #take mean
...
if __name__ == '__main__':
    ...
    if os.path.exists(filepath):
        ...
        df_r = r.compute_R_eff(iter_ids=geoger.get_state_ids(), state_level = True)
    r_orig = df_r[:]
    ...
    plt.figure()
    plt.scatter(r_orig.Date, np.log(r_orig.R_eff), c='maroon', alpha=0.3, marker='.', label='R_value with seasonality')
    r_orig['R_eff']/=r_orig['Date'].map(data)
    plt.scatter(r_orig.Date, np.log(r_orig.R_eff), c='dodgerblue', alpha=0.3, marker='.', label='R_value without seasonality')
    plt.legend()
    plt.grid(True)

...
# read in the data
bd = pd.read_csv('/Users/saschakorf/Documents/Arbeit/memilio.nosynch/memilio/pycode/memilio-epidata/memilio/epidata/bs.csv', header=None)
number_of_rows_to_transform = 100000
...
bd_long_lat = bd_long_lat.groupby(['loCs', 'laCs', 'loCe', 'laCe']).size().reset_index(name='counts').sort_values('counts', ascending=False)
# take out the trips with less than 10 occurences
bd_long_lat = bd_long_lat[bd_long_lat['counts'] > 10]


bd_long_lat = bd_long_lat.apply(lambda x: transform_coordinates(x['loCs'], x['laCs'], x['loCe'], x['laCe'], x['counts']), axis=1, result_type='expand')
...
bd_long_lat.to_csv('/Users/saschakorf/Documents/Arbeit/memilio.nosynch/memilio/pycode/memilio-epidata/memilio/epidata/bs_lola.csv', index=False)

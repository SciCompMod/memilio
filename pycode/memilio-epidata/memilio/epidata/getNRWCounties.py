...
def main():
    ...
    mobility_matrix_nrw = mobility_matrix.loc[county_indices[4], county_indices[4]]

    gd.write_dataframe(
        mobility_matrix_nrw, directory_mobility, mobility_file + '_nrw', 'txt',
        param_dict={'sep': ' ', 'header': None, 'index': False})
        
    population = pd.read_json(os.path.join(directory_population + population_file + '.json'))
    population_nrw = population.loc[county_indices[4]]
    gd.write_dataframe(population_nrw, directory_population, population_file + '_nrw', 'json')

...
class Test_defaultDict(unittest.TestCase):
    ...
    def test_invert_dict(self):
        ...
        for county in self.test_dict_county.keys():
            ...
            self.assertEqual(self.test_dict_county[county], inv_county[county])
        inv_state = dd.invert_dict(dd.State)
        self.assertDictEqual(self.test_dict_state, inv_state)
        inv_dd = dd.invert_dict(dd.defaultDict)
        for defaultvalue in self.test_dict_county.keys():
            self.assertIn(defaultvalue, inv_county.keys())
            self.assertEqual(self.test_dict_county[defaultvalue], inv_county[defaultvalue])

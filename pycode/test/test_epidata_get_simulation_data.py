...
class TestGetSimulationData(fake_filesystem_unittest.TestCase):
    ...
    def test_get_call_sub_functions(self, mock_vaccine, mock_agep, mock_popul, mock_rki, mock_divi):
        ...
        gsd.get_simulation_data(read_data, out_form, out_folder, no_raw, end_date, fill_dates, make_plot,
                                moving_average, split_berlin, start_date, update_data)

        mock_vaccine.assert_called()
        mock_vaccine.assert_called_with(False, "json", self.path, False)

        mock_agep.assert_called()
        mock_agep.assert_called_with(False, "json", self.path, False)

        mock_popul.assert_called()
        mock_popul.assert_called_with(False, "json", self.path, False)

        mock_rki.assert_called()
        mock_rki.assert_called_with(False, "json", self.path, False, dd.defaultDict['fill_dates'],
                                    dd.defaultDict['make_plot'], dd.defaultDict['moving_average'],
                                    dd.defaultDict['split_berlin'])

        mock_divi.assert_called()
        mock_divi.assert_called_with(False, "json", self.path, False, dd.defaultDict['end_date'],
                                     dd.defaultDict['start_date'], dd.defaultDict['update_data'])

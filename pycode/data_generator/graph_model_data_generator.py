...
def get_county_model(county_data, day, dampings):
    ...
    for i in range(num_groups):
        ...
        model.populations[group, secir.Index_InfectionState(State.Exposed)] = county_data[group_names[i]][day][1]
        model.populations[group, secir.Index_InfectionState(State.Carrier)] = county_data[group_names[i]][day][2]
        model.populations[group, secir.Index_InfectionState(State.Infected)] = county_data[group_names[i]][day][3]
        model.populations[group, secir.Index_InfectionState(State.Hospitalized)] = county_data[group_names[i]][day][4]
        model.populations[group, secir.Index_InfectionState(State.ICU)] = county_data[group_names[i]][day][5]
        model.populations[group, secir.Index_InfectionState(State.Recovered)] = county_data[group_names[i]][day][6]
        model.populations[group, secir.Index_InfectionState(State.Dead)] = county_data[group_names[i]][day][7]
        model.populations.set_difference_from_group_total_AgeGroup((group, secir.Index_InfectionState(State.Susceptible)), county_data[group_names[i]][day][0])
    ...
    #model.parameters.get_contact_patterns().cont_freq_mat[0].baseline = getBaselineMatrix() * 0.05
    #model.parameters.get_contact_patterns().cont_freq_mat[0].minimum = getMinimumMatrix() * 0.05
...
def data_formatting(file_name, data, random_day, dampings, num_sim_days, results):
    ...
    for key in data.keys():
        for group in group_names:
            rounded_compartment_values = [int(item) for item in list(data[key][group][:][random_day])]
...
def single_simulation(data, day, dampings, num_sim_days):
    ...
    for key, index in zip(list_of_keys, range(len(list_of_keys))):
      graph.add_node(index, get_county_model(data[key], day, dampings))
...
def multiple_simulation_organizer(dataset_file_name, num_runs, num_dampings, num_sim_days):
    ...
    for save_circle in range(int(num_runs/secure_save_after_num_runs)):
        ...
        for run in range(secure_save_after_num_runs):
            ...
            formated_sample = data_formatting(dataset_file_name, data, random_day, dampings, num_sim_days, results)
            #add sample to output list
            output.append(formated_sample)

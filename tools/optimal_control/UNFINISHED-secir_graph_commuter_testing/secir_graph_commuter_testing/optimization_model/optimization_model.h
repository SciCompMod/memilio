...
OptimizationModel::create_graph_model() const
{
    ...
    set_all_groups(mio::osecir::RelativeTransmissionNoSymptoms<FP>{}, 1.0); // YES
    ...
    params.get_end_commuter_detection() = std::numeric_limits<FP>::max();
    ...
    std::string mobility_data_file =
        mio::path_join(m_data_directory.string(), "Germany", "mobility", "commuter_mobility_2019.txt");

...
void check_constraint_feasibility(const SecirvvsOptimization& settings)
{
    ...
    std::vector<double> max_path_constraints(settings.num_path_constraints(), 0.0);
    std::vector<double> max_terminal_constraints(settings.num_terminal_constraints(), 0.0);
    ...
    for (std::size_t run = 0; run < settings.num_simulation_runs(); run++) {
        ...
        auto graph_model = settings.optimization_model().get_graph_model<double>(seed);
        ...
        std::vector<double> time_steps =
            make_time_grid<double>(settings.t0(), settings.tmax(), settings.num_intervals());
    ...
    for (size_t i = 0; i < settings.num_path_constraints(); ++i) {
        ...
        std::cout << "  [" << i << "] \"" << constraint.name() << "\": " << max_path_constraints[i] << "\n";
    }
    ...
    for (size_t i = 0; i < settings.num_terminal_constraints(); ++i) {
        ...
        std::cout << "  [" << i << "] \"" << constraint.name() << "\": " << max_terminal_constraints[i] << "\n";
    }
    ...
    for (size_t i = 0; i < settings.num_path_constraints(); ++i) {
        ...
        double value           = max_path_constraints[i];
    ...
    for (size_t i = 0; i < settings.num_terminal_constraints(); ++i) {
        ...
        double value           = max_terminal_constraints[i];
        double max_v           = constraint.max();

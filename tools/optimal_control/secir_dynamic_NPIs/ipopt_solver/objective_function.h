    ...
    const SecirOptimization& settings, const FP* ptr_parameters, size_t n)
{
    ...
    // Track whether each control parameter has been active at least once
    std::vector<bool> threshold_activated(settings.num_control_parameters(), false);
            ...
            for (auto& cp : settings.control_parameters()) {
                ...
                for (auto& d : cp.dampings()) {
                    ...
                    if (inf_rel >= threshold) {
                        threshold_activated[index] = true;
    ...
    for (size_t i = 0; i < threshold_activated.size(); ++i) {
        if (!threshold_activated[i]) {
            // Penalty ensures to set strength of npis with not reached thresholds to zero
            objective += dynamic_NPI_strengths[i] * graph_simulation.get_graph().nodes().size();
        }
    }

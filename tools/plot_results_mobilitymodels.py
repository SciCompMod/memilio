    ...
    for file in range(len(files)):
        ...
        for region in range(number_regions):
            if (len(list(h5file[list(h5file.keys())[region]].keys())) > 3):
                ...
                number_regions = len(list(h5file[list(h5file.keys())[region]].keys())) - 2
                for region in range(number_regions):
                    ...
                    if legendplot[file] in linestyle_dict:
                        for i in range(4):
                            axs[int(i/2), i % 2].plot(dates,
                                                    total[:, i], label=legendplot[file] +" Region "+ str(region), linewidth=1.2, 
    ...
    for file in range(len(files)):
        ...
        for region in range(number_regions):
            if (len(list(h5file[list(h5file.keys())[region]].keys())) > 3):
                ...
                for region_ in range(number_regions):
                    ...
                    if legendplot[file] in linestyle_dict:
                        plt.plot(dates[1:], incidence, linewidth=1.2,
                                linestyle=linestyle_dict[legendplot[file]], color=color_dict[region_])
                    else:
                        plt.plot(dates[1:], incidence, linewidth=1.2)
            else: 
                data = h5file[list(h5file.keys())[region]]
                ...
                if legendplot[file] in linestyle_dict:
                    plt.plot(dates[1:], incidence, linewidth=1.2, 
                            linestyle=linestyle_dict[legendplot[file]], color=color_dict[region])
                else:
                    plt.plot(dates[1:], incidence, linewidth=1.2)

...
def read_groundtruth(data_dir, groundtruth_exponent, gregory_order, finite_difference_order, flows=False):
    ...
    if flows:
        ...
        h5file = h5py.File(os.path.join(
            data_dir, f'result_{model}_dt=1e-{groundtruth_exponent:.0f}_gregoryorder={gregory_order}_finitedifforder={finite_difference_order}.h5'), 'r')
    ...
    for model in models:
        for exponent in exponents_ide:
            if flows:
                ...
                h5file = h5py.File(os.path.join(
                    data_dir, f'result_{model}_dt=1e-{exponent:.0f}_gregoryorder={gregory_order}_finitedifforder={finite_difference_order}.h5'), 'r')
...
def compute_relerror_norm_l2(groundtruth, results, groundtruth_exponent, timesteps_ide, finite_difference_order, flows=False):
    ...
    for i in range(len(results['ide'])):
        ...
        for compartment in range(num_errors):
            ...
            difference = groundtruth['ide'][0][int(scale_timesteps*(num_timepoints -
                                                                    (finite_difference_order + 1)))::int(scale_timesteps)][:, compartment]-results['ide'][i][finite_difference_order + 1:, compartment]
            norm_groundtruth = compute_l2_norm(groundtruth['ide'][0][int(scale_timesteps*(num_timepoints -

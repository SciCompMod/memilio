    ...
    updateModel: (state, action) => {
      ...
      if (state[data.model]) {
        ...
        switch (data.model) {
          ...
          case 'seir':
            model.parameters.find((p) => p.name === data.name).value =
              data.value;
            break;
      ...
      Object.keys(models).forEach((name) => {
        ...
        if (models[name] instanceof Array) {
          model.parameters = JSON.parse(
            JSON.stringify(
              parameters
                .filter((p) => models[name].includes(p.name))
                .map((p) => ({ ...p, value: p.default }))
            )
          );
          ...
          const params = parameters.filter((p) =>
            models[name].parameters.includes(p.name)
          );

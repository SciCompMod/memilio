                                                        ...
                                                        ScalarType saving_exponent = 0.)
{
    ...
    if (!save_dir.empty()) {
        ...
        compressed_compartments = compress_timeseries(result_compartments, saving_exponent);
        auto result             = compressed_compartments.export_csv(fmt::format("{}/lct_result.csv", save_dir));
        auto save_result_status_ode =
            mio::save_result({compressed_compartments}, {0}, num_agegroups,
                             save_dir + "result_ode_dt=1e-" + fmt::format("{:.0f}", ode_exponent) + "_savedt=1e-" +
                                 fmt::format("{:.0f}", saving_exponent) + ".h5");
        ...
        if (!save_result_status_ode) {
            return mio::failure(mio::StatusCode::InvalidValue,
                                "Error occured while saving the LCT simulation results.");
        }
    }
...
int main()
{
    ...
    ScalarType t0_ode                    = 0.;
    ScalarType t0_ide                    = 75.;
    std::vector<ScalarType> init_windows = {75., 10.};
    std::vector<ScalarType> tmax_values  = {t0_ide + 75.};
    ...
    for (std::vector<ScalarType> value_tuple : timeinf_tmax_values) {
        ...
        for (size_t finite_difference_order : finite_difference_orders) {
            ...
            std::string save_dir = fmt::format(
                "../../simulation_results/2026-07-31/convergence_lct_dtode=1e-{}_t0ode={}_timeinf={}_contfreq={}/"
            ...
            auto result_lct =
                simulate_lct(ode_exponent, t0_ode, tmax, time_infected, save_dir, saving_exponent).value();

            // auto compartments_lct = result_lct.value();
                ...
                for (size_t gregory_order : gregory_orders) {
                    ...
                    mio::IOResult<void> result_ide =
                        simulate_ide(ide_exponents, saving_exponent, gregory_order, finite_difference_order,
                                     init_window, t0_ide, tmax, time_infected, save_dir_ide, result_lct);
                }
            }
        }
    }

...
int main()
{
    ...
    // ========================================================================
    // 1. Reference solution with standard lagrangian
    // ========================================================================
    std::cout << "Calculating exact Lagrangian Reference (dt = " << dt_ref << ")..." << std::flush;

    StandardModelLagrangianSecirts lag_model(1, 1);
    ...
    // 2. Phi Setup
    // ========================================================================
    Model<double> phi_model(1);
    setup_parameters(phi_model);

    const auto NC       = 29;
    const auto sys_size = NC + NC * NC; // 29 + 841 = 870

    Eigen::VectorXd initial_totals(NC);
    Eigen::VectorXd initial_commuter(NC);

    for (size_t state = 0; state < NC; ++state) {
        phi_model.populations[{mio::AgeGroup(0), (InfectionState)state}] = base_pop[state];
    ...
    AugmentedPhiSystemSecirts sys(phi_model);

    std::vector<std::string> method_names = {"Phi Euler (RK1)", "Phi RK2", "Phi RK3", "Phi RK4"};

    for (const auto& method_name : method_names) {
        std::cout << "========== Testing " << method_name << " against Lagrangian ==========\n";
        ...
        for (auto dt : dts) {
            Eigen::VectorXd y = y0;
            Eigen::VectorXd k1(sys_size), k2(sys_size), k3(sys_size), k4(sys_size), temp(sys_size);

            for (double t = t0; t < tmax; t += dt) {
                if (t + dt > tmax + 1e-10)
                    break;

                if (method_name == "Phi Euler (RK1)") {
                    sys(y, k1, t);
                    y += dt * k1;
                }
                else if (method_name == "Phi RK2") {
                    sys(y, k1, t);
                    temp = y + (dt * 0.5) * k1;
                    sys(temp, k2, t + 0.5 * dt);
                    y += dt * k2;
                }
                else if (method_name == "Phi RK3") {
                    sys(y, k1, t);
                    temp = y + (dt * 0.5) * k1;
                    sys(temp, k2, t + 0.5 * dt);
                    temp = y + dt * (-k1 + 2.0 * k2);
                    sys(temp, k3, t + dt);
                    y += (dt / 6.0) * (k1 + 4.0 * k2 + k3);
                }
                else if (method_name == "Phi RK4") {
                    sys(y, k1, t);
                    temp = y + (dt * 0.5) * k1;
                    sys(temp, k2, t + 0.5 * dt);
                    temp = y + (dt * 0.5) * k2;
                    sys(temp, k3, t + 0.5 * dt);
                    temp = y + dt * k3;
                    sys(temp, k4, t + dt);
            ...
            Eigen::MatrixXd Phi_final        = Eigen::Map<const Eigen::MatrixXd>(y.tail(NC * NC).data(), NC, NC);
            Eigen::VectorXd current_commuter = Phi_final * initial_commuter;

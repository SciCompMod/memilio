...
void constraint_functions(const ProblemSettings& settings, const FP* parameters, size_t n, FP* constraints, size_t m) {
    ...
    for (size_t controlIndex = 0; controlIndex < settings.numControlIntervals; controlIndex++) {
        ...
        for (size_t substep = 0; substep < settings.controlIntervalResolution; substep++) {
            ...
            for (int state_index = 0; state_index < static_cast<int>(mio::oseair::InfectionState::Count); state_index++) {
            ...
            for (size_t constraintIndex = 0; constraintIndex < settings.numPathConstraints; constraintIndex++) {
                ...
                FP current_value = final_state[state_index];
                max_path_values[constraintIndex] = std::max(max_path_values[constraintIndex], current_value);
            }
        }
    }
    ...
    for (size_t constraintIndex = 0; constraintIndex < settings.numTerminalConstraints; constraintIndex++) {
        ...
        auto state_name = constraint.first;
        int state_index = static_cast<int>(string_to_state(state_name));
        constraints[settings.numPathConstraints + constraintIndex] = model.populations[mio::oseair::InfectionState(state_index)];
    }
}
...
//     FP objective = 0.0;
//     FP total_infections = 0.0;
//     FP total_cost = 0.0;
//     FP total_oscillation_penalty = 0.0;
    
//     // Weighting factors (can be adjusted based on priorities)
//     FP infection_weight = 1.0;
//     FP cost_weight = 0.1;
//     FP oscillation_weight = 0.5;
    
//     // Store previous control values for oscillation penalty
//     std::vector<FP> prev_controls(settings.numControls);
//     for (int i = 0; i < settings.numControls; ++i) {
//         prev_controls[i] = parameters[i];
//     }
...
//         // Calculate oscillation penalty for each control parameter
//         if (controlIndex > 0) {
//             for (int i = 0; i < settings.numControls; ++i) {
//                 FP diff = parameters[controlIndex * settings.numControls + i] - prev_controls[i];
//                 total_oscillation_penalty += diff * diff;
//                 prev_controls[i] = parameters[controlIndex * settings.numControls + i];
//             }
//         }

//         // Calculate costs of measures
//         // Social distancing has economic cost, quarantining has cost, testing has cost but also benefit
//         FP interval_cost = (- socialDistancing) * 10.0 +  // Higher weight for social distancing economic impact
//                           (- quarantined) * 5.0 +         // Cost of quarantining people
//                           (testingRate) * 2.0; // Testing has cost but is beneficial
...
//             // Track infections (both current infected and new cases)
//             FP infected = result.get_last_value()[(int)mio::oseair::InfectionState::Infected];
//             total_infections += infected;
//         }
//     }

//     // Combine all components of the objective
//     objective = infection_weight * total_infections + 
//                 cost_weight * total_cost + 
//                 oscillation_weight * total_oscillation_penalty;

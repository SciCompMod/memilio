...
int main()
{
    ...
    auto sim = mio::make_extended_migration_sim(t0, dt, std::move(g));

    sim.advance(tmax);
    // results node 1
    std::cout << "Results node 1" << std::endl;
    auto interpolated_sim1 =  mio::interpolate_simulation_result(sim.get_graph().nodes()[1].property.base_sim.get_result());

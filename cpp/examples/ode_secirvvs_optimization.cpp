...
private:
    ...
    const int numControls_        = 1; // number of control variables
...
{
    const double N = Secirvvs_NLP::N; // total population of the United States
...
void Secirvvs_NLP::eval_objective_constraints(const std::vector<FP>& x, std::vector<FP>& constraints, FP& objective)
{
    ...
    mio::Simulation sim(model, t0, dt);

    for (int controlIndex = 0; controlIndex < numControlIntervals_; ++controlIndex) {
        sim.get_model().parameters.template get<mio::osecirvvs::TransmissionProbabilityOnContact<FP>>() = x[controlIndex];
        ...
        objective += pcresolution_ * (-x[controlIndex]);
        ...
        for (int i = 0; i < pcresolution_; ++i, ++gridindex) {
            ...
            constraints[gridindex] = sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedNoSymptomsNaiveConfirmed] + sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedNoSymptomsPartialImmunityConfirmed] + sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedNoSymptomsImprovedImmunityConfirmed] +
                                        sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedSymptomsNaiveConfirmed] + sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedSymptomsPartialImmunityConfirmed] + sim.get_result().get_last_value()[(int)mio::osecirvvs::InfectionState::InfectedSymptomsImprovedImmunityConfirmed];
        }
    }
...
{
    mio::unused(n, m);
    // controls order: 1. TransmissionProbabilityOnContact
    for (int i = 0; i < numControlIntervals_; ++i) {
        x_l[i]                            = 0.05; // lower bound of TransmissionProbabilityOnContact
        x_u[i]                            = 1.5; // upper bound of TransmissionProbabilityOnContact
    }

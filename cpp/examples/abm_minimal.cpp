...
int main()
{
    for (int iter = 0; iter < 1; iter++) {
        ...
        for (int iter1 = 0; iter1 < 1000; iter1++) {
            ...
            auto testing_scheme_work = mio::abm::TestingScheme(testing_criteria_work, testing_min_time, start_date,
            ...
            for (auto& person : persons) {
                ...
                if (infection_state != mio::abm::InfectionState::Susceptible)
                    person.add_new_infection(
                        mio::abm::Infection(rng, mio::abm::VirusVariant::Wildtype, person.get_age(),
                                            world.get_global_infection_parameters(), start_date, infection_state));
            }
            ...
            for (auto& person : persons) {
                ...
                if (person.get_age() == mio::abm::AgeGroup::Age15to34 ||
                    person.get_age() == mio::abm::AgeGroup::Age35to59) {
        ...
        LIKWID_MARKER_START("region_2");

        sim.advance(tmax);

        LIKWID_MARKER_STOP("region_2");

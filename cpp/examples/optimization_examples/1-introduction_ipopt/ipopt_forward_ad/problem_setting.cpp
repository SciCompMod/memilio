...
bool TutorialCpp_NLP::eval_grad_f(Ipopt::Index n, const Ipopt::Number* x, bool /*new_x*/, Ipopt::Number* grad_f)
{
    ...
    // Define the AD type (automatic differentiation type) for forward-mode differentiation.
    // It stores both the value of a variable and its derivative.
    // This type allows computations with both the value and its derivative in one object,
    // simplifying the process of computing gradients automatically during function evaluation.
    using ad_t = ad::gt1s<double>::type;

#pragma omp parallel
    {
        // Create a thread-local copy of the decision variables in AD type.
        // Here, `x_ad` holds both the value and derivative for each decision variable.
...
#pragma omp for
        // Compute the gradient in parallel using OpenMP for-loop
...
#pragma omp for
            for (Ipopt::Index column = 0; column < n; ++column) {
                ad::derivative(x_ad[column]) = 1.0;
                constraint_functions(x_ad.data(), static_cast<std::size_t>(n), g_ad.data(),
...
{
    std::cout << "IPOPT with Foward-Mode AD\n";

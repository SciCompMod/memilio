...
Secirvvs_NLP::Secirvvs_NLP(const SecirvvsOptimization& settings)
    ...
    , m_graph_model_double(settings.optimization_model().create_graph_model<double>())
    // forward-mode (tangent-linear) AD model for Jacobian via GT1S
    , m_graph_model_tangent_linear(settings.optimization_model().create_graph_model<ad::gt1s<double>::type>())
    // reverse-mode (adjoint) AD model for gradient via GA1S
    , m_graph_model_adjoint(settings.optimization_model().create_graph_model<ad::ga1s<double>::type>())
{
    ...
    size_t num_graph_nodes = m_graph_model_double.nodes().size();

    m_n = num_control_intervals * num_control_parameters * num_graph_nodes;
    ...
    for (size_t control_interval = 0; control_interval < num_control_intervals; control_interval++) {
        for (size_t control_index = 0; control_index < num_control_parameters; control_index++) {
            ...
            for (size_t node_index = 0; node_index < num_graph_nodes; node_index++) {
                size_t global_idx = idx + node_index * num_control_intervals * num_control_parameters;
                m_x_l[global_idx] = m_settings.control_parameters()[control_index].min();
                m_x_u[global_idx] = m_settings.control_parameters()[control_index].max();
            }
        }
    }
        ...
        for (size_t control_index = 0; control_index < num_control_parameters; control_index++) {
            ...
            for (size_t node_index = 0; node_index < num_graph_nodes; node_index++) {
                ...
                if (m_settings.random_start()) {
                    ...
                    x[global_idx] = dis(gen);
                }
                else {
                    x[global_idx] = 0.5 * (min_val + max_val);
                }
            }
        }
    }
...
bool Secirvvs_NLP::eval_f(Ipopt::Index n, const Ipopt::Number* x, bool /*new_x*/, Ipopt::Number& obj_value)
{
    ...
    obj_value = objective_function(m_graph_model_double, m_settings, x, n);
...
bool Secirvvs_NLP::eval_g(Ipopt::Index n, const Ipopt::Number* x, bool /*new_x*/, Ipopt::Index m, Ipopt::Number* g)
{
    ...
    constraint_function(m_graph_model_double, m_settings, x, n, g, m);
...
bool Secirvvs_NLP::eval_grad_f_forward(Ipopt::Index n, const Ipopt::Number* x, bool /*new_x*/, Ipopt::Number* grad_f)
{
    ...
    PRAGMA_OMP(parallel)
    {
        ...
        for (Ipopt::Index column = 0; column < n; ++column) {
            ...
            ad_t obj_ad                  = objective_function(m_graph_model_tangent_linear, m_settings, x_ad.data(), n);
...
bool Secirvvs_NLP::eval_grad_f_reverse(Ipopt::Index n, const Ipopt::Number* x, bool /*new_x*/, Ipopt::Number* grad_f)
{
    ...
    ad_t obj_ad = objective_function(m_graph_model_adjoint, m_settings, x_ad.data(), n);
                                      ...
                                      Ipopt::Number* values)
{
    ...
    PRAGMA_OMP(parallel)
    {
        ...
        for (Ipopt::Index column = 0; column < n; ++column) {
            ad::derivative(x_ad[column]) = 1.0;
            constraint_function(m_graph_model_tangent_linear, m_settings, x_ad.data(), n, g_ad.data(), m);
                                      ...
                                      Ipopt::Number* values)
{
    ...
    std::vector<ad_t> g_ad(m);
    constraint_function(m_graph_model_adjoint, m_settings, x_ad.data(), n, g_ad.data(), m);
                                     ...
                                     const Ipopt::IpoptData* ip_data, Ipopt::IpoptCalculatedQuantities* ip_cq)
{
    ...
    save_solution(m_graph_model_double, m_settings, n, x, z_L, z_U, m, g, lambda, obj_value);

...
std::shared_ptr<mio::IntegratorCore<FP>> make_integrator(IntegratorType type, FP dt)
{
    FP dt_min  = std::numeric_limits<FP>::min();
    FP dt_max  = dt;
    FP abs_tol = static_cast<FP>(1e-12);
    FP rel_tol = static_cast<FP>(1e-8);

    using namespace boost::numeric::odeint;

    switch (type) {
    case IntegratorType::ExplicitEuler: {
        auto eulerIntegrator = std::make_shared<mio::EulerIntegratorCore<FP>>();
        return eulerIntegrator;
    }
    case IntegratorType::ExplicitCashKarp54: {
        auto cashKarpStepper = std::make_shared<
            mio::ExplicitStepperWrapper<FP, runge_kutta_cash_karp54>
        >();
    ...
    case IntegratorType::ExplicitFehlberg78: {
        auto fehlbergStepper = std::make_shared<
            mio::ExplicitStepperWrapper<FP, runge_kutta_fehlberg78>
        >();
    ...
    case IntegratorType::ControlledCashKarp54: {
        auto cashKarpControlled = std::make_shared<
            mio::ControlledStepperWrapper<FP, runge_kutta_cash_karp54>
        >();
    ...
    case IntegratorType::ControlledFehlberg78: {
        auto fehlbergControlled = std::make_shared<
            mio::ControlledStepperWrapper<FP, runge_kutta_fehlberg78>
        >();

...
// // Utility function to convert Model<FromFP> to Model<ToFP>
// template <typename ToFP, typename FromFP>
// mio::osecirvvs::Model<ToFP> convert_model(const mio::osecirvvs::Model<FromFP>& src)
// {
//     // Create a new model with the same number of age groups
//     mio::osecirvvs::Model<ToFP> dst(src.parameters.get_num_groups());

//     // --- Copy parameters field-by-field ---
//     // ICUCapacity
//     dst.parameters.template get<mio::osecirvvs::ICUCapacity<ToFP>>() =
//         static_cast<ToFP>(src.parameters.template get<mio::osecirvvs::ICUCapacity<FromFP>>());
//     // TestAndTraceCapacity
//     dst.parameters.template get<mio::osecirvvs::TestAndTraceCapacity<ToFP>>() =
//         static_cast<ToFP>(src.parameters.template get<mio::osecirvvs::TestAndTraceCapacity<FromFP>>());
//     // Seasonality
//     dst.parameters.template get<mio::osecirvvs::Seasonality<ToFP>>() =
//         static_cast<ToFP>(src.parameters.template get<mio::osecirvvs::Seasonality<FromFP>>());
//     // DynamicNPIsImplementationDelay
//     dst.parameters.template get<mio::osecirvvs::DynamicNPIsImplementationDelay<ToFP>>() =
//         static_cast<ToFP>(src.parameters.template get<mio::osecirvvs::DynamicNPIsImplementationDelay<FromFP>>());

//     // DailyPartialVaccinations
//     auto& dst_partial = dst.parameters.template get<mio::osecirvvs::DailyPartialVaccinations<ToFP>>();
//     const auto& src_partial = src.parameters.template get<mio::osecirvvs::DailyPartialVaccinations<FromFP>>();
//     dst_partial.resize(src_partial.size());
//     for (size_t i = 0; i < src_partial.size(); ++i) {
//         dst_partial[i] = static_cast<ToFP>(src_partial[i]);
//     }

//     // DailyFullVaccinations
//     auto& dst_full = dst.parameters.template get<mio::osecirvvs::DailyFullVaccinations<ToFP>>();
//     const auto& src_full = src.parameters.template get<mio::osecirvvs::DailyFullVaccinations<FromFP>>();
//     dst_full.resize(src_full.size());
//     for (size_t i = 0; i < src_full.size(); ++i) {
//         dst_full[i] = static_cast<ToFP>(src_full[i]);
//     }

//     // All age-grouped parameters (example for TimeExposed, repeat for all)
//     auto copy_age_group_param = [&](auto tag) {
//         auto& dst_param = dst.parameters.template get<std::decay_t<decltype(tag)>>();
//         const auto& src_param = src.parameters.template get<std::decay_t<decltype(tag)>>();
//         for (mio::AgeGroup ag = 0; ag < src.parameters.get_num_groups(); ++ag) {
//             dst_param[ag] = static_cast<ToFP>(src_param[ag]);
//         }
//     };

//     copy_age_group_param(mio::osecirvvs::TimeExposed<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::TimeInfectedNoSymptoms<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::TimeInfectedSymptoms<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::TimeInfectedSevere<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::TimeInfectedCritical<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::TransmissionProbabilityOnContact<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::RelativeTransmissionNoSymptoms<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::RiskOfInfectionFromSymptomatic<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::MaxRiskOfInfectionFromSymptomatic<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::RecoveredPerInfectedNoSymptoms<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::SeverePerInfectedSymptoms<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::CriticalPerSevere<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::DeathsPerCritical<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducExposedPartialImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducExposedImprovedImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducInfectedSymptomsPartialImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducInfectedSymptomsImprovedImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducInfectedSevereCriticalDeadPartialImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducInfectedSevereCriticalDeadImprovedImmunity<ToFP>{});
//     copy_age_group_param(mio::osecirvvs::ReducTimeInfectedMild<ToFP>{});

//     // ContactPatterns (UncertainContactMatrix)
//     {
//         const auto& src_contacts = src.parameters.template get<mio::osecirvvs::ContactPatterns<FromFP>>();
//         auto& dst_contacts = dst.parameters.template get<mio::osecirvvs::ContactPatterns<ToFP>>();
//         // Copy baseline and minimum matrices for each location
//         for (size_t i = 0; i < src_contacts.get_containers().size(); ++i) {
//             dst_contacts.get_containers()[i].get_baseline() = src_contacts.get_containers()[i].get_baseline().template cast<ToFP>();
//             dst_contacts.get_containers()[i].get_minimum() = src_contacts.get_containers()[i].get_minimum().template cast<ToFP>();
//         }
//         // Dampings are usually scenario-specific, so you may want to clear or copy as needed
//         dst_contacts.get_dampings().clear();
//     }

//     // --- Copy populations ---
//     for (const auto& kv : src.populations) {
//         dst.populations[kv.first] = static_cast<ToFP>(kv.second.value());
//     }
...
//         mio::IOResult<Eigen::MatrixXd> baseline_result = mio::read_mobility_plain(baseline_file_path.string());
//         if (!baseline_result) {
...
//         // contact_matrices[i].update_matrices([&](auto& baseline, auto& minimum) {
//         //     baseline = baseline_result.value().cast<FP>();
//         //     minimum = Eigen::Matrix<FP, Eigen::Dynamic, Eigen::Dynamic>::Zero(
//         //         num_age_groups.get(), num_age_groups.get()
//         //     );
//         // });
//     }
...
//     std::cout<<"Start Graph init"<<std::endl;
...
//     mio::Graph<
//         mio::SimulationNode<FP, mio::Simulation<FP, mio::osecirvvs::Model<FP>>>,
//         mio::MobilityEdge<FP>
//     > graph;

//     graph.add_node(0, model1, problem_settings.t0());
//     graph.add_node(1, model2, problem_settings.t0());

//     graph.nodes()[0].property.get_simulation().set_integrator(
//         std::make_shared<mio::EulerIntegratorCore<FP>>());
//     graph.nodes()[1].property.get_simulation().set_integrator(
//         std::make_shared<mio::EulerIntegratorCore<FP>>());

//     // Allow no mobility along the edges.
//     graph.add_edge(0, 1, Eigen::VectorX<FP>::Constant(4, 0));
//     graph.add_edge(1, 0, Eigen::VectorX<FP>::Constant(17, 0));

//     auto graph_sim_mobility = mio::make_mobility_sim<FP>(
//         problem_settings.t0(), problem_settings.dt(), std::move(graph)
//     );

//     graph_sim_mobility.advance(problem_settings.tmax());

//    std::cout<< graph_sim_mobility.get_graph().nodes()[0].property.get_result().get_last_value()[0] << std::endl;
//    std::cout<< graph_sim_mobility.get_graph().nodes()[1].property.get_result().get_last_value()[0] << std::endl;

//     // mio::Graph<
//     //     mio::SimulationNode<FP, mio::osecirvvs::Simulation<FP>>,
//     //     mio::MobilityEdge<FP>
//     // > graph;

//     // graph.add_node(0, model_group1);
//     // graph.add_node(1, model_group2);
//     // graph.add_edge(0, 1);
//     // // g.add_edge(1, 0, Eigen::VectorX<ScalarType>::Constant((size_t)mio::osecirvvs::InfectionState::Count, 0.1),
//     //            indices_save_edges);

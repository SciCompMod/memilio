...
int main()
{
    ...
    const auto& nodes     = graph_simulation.nodes();

    SecirvvsOptimization settings(graph_simulation, t0, tmax);

    settings.addPathConstraint("Severe", {0.0, 6'000}, nodes[0].id);
    settings.addPathConstraint("Severe", {0.0, 3'000}, nodes[1].id);
    settings.addPathConstraint("Critical", {0.0, 125});

    settings.addTerminalConstraint("Dead", {0.0, 450'000});
    settings.addTerminalConstraint("Dead", {0.0, 150'000}, nodes[0].id);
    settings.addTerminalConstraint("Dead", {0.0, 224'000}, nodes[1].id);
    ...
    int ipopt_verbose         = 5;
    int ipopt_print_frequency = 10;
    bool ipopt_print_timings  = true;
    int ipopt_max_iter        = 300;
    double ipopt_tol          = 1e-4;

    settings.setNumControlIntervals(num_control_intervals);
    settings.setPcResolution(pc_resolution);
    settings.setRandomStart(random_start);
    settings.setIntegratorType(integrator_type);
    settings.setIntegratorResolution(integrator_resolution);
    settings.setAdEvalF(ad_eval_f);
    settings.setAdEvalJac(ad_eval_jac);
    settings.setIpoptVerbose(ipopt_verbose);
    settings.setIpoptPrintFrequency(ipopt_print_frequency);
    settings.setIpoptPrintTimings(ipopt_print_timings);
    settings.setIpoptMaxIter(ipopt_max_iter);
    settings.setIpoptTolerance(ipopt_tol);
...
// int integrator_resolution = 10;   // simulation substeps
...
//     {"Infected", {0.0, INF}},
//     {"Susceptible", {0.0, INF}},
//     {"Infected+Confirmed Severe Critical", {0.0, INF}}
// };
...
// auto graph = initialize_model(t0, tmax);
...
// int main()
// {
//     using Model = mio::osecirvvs::Model<double>;
//     using Simulation = mio::Simulation<double, Model>>;
//     using Node = mio::SimulationNode<double, Simulation>;
//     using Graph = mio::Graph<Node, mio::MobilityEdge<double>>;

//     Graph graph;

//     // initilize the graph with nodes and edges
//     init_graph(graph);

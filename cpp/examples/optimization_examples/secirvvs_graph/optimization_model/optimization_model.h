...
{
public:
    OptimizationModel(const std::filesystem::path& data_dir, double t0, double tmax);
...
OptimizationModel::create_graph_model() const
{
    ...
    mio::MobilityEdge<FP> edge0(
        mio::MobilityParameters<FP>(Eigen::VectorX<FP>::Constant(num_age_groups * num_infection_states, 0.1)));

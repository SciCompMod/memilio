...
void SecirvvsOptimization::check_constraint_feasability()
{
    ...
    auto set_most_restrictive_control = [&](std::vector<std::vector<double>>& parameters, const std::string& name) {
        assert(parameters.size() == num_graph_nodes);
        ...
        for (size_t node_index = 0; node_index < num_graph_nodes; node_index++) {
            assert(parameters[node_index].size() == num_control_parameters() * num_control_intervals());
            for (size_t control_interval = 0; control_interval < num_control_intervals(); control_interval++) {
                parameters[node_index][control_index + control_interval * num_control_parameters()] =
    ...
    std::vector<std::vector<double>> parameters(
        num_graph_nodes, std::vector<double>(num_control_parameters() * num_control_intervals()));
    ...
    set_control_dampings<double>(*this, graph_model, parameters);

    for (auto& node : graph_model.nodes()) {
        node.property.get_simulation().set_integrator(make_integrator<double>(integrator_type(), dt()));
    }

    std::vector<double> time_steps = make_time_grid<double>(t0(), tmax(), num_intervals());
    auto graph_sim_mobility        = mio::make_mobility_sim<double>(t0(), dt(), graph_model);
    ...
    // std::vector<double> path_constraint_values(num_path_constraints(), 0.0);
    // std::vector<double> terminal_constraint_values(num_terminal_constraints(), 0.0);
    // auto graph_model = optimization_graph_model().create_graph_model<double>();

    // auto set_most_restrictive_control = [&](std::vector<double>& parameters, const std::string& name) {
    //     size_t control_index = static_cast<size_t>(string_to_control(name));
    //     for (size_t node_index = 0; node_index < graph_simulation.nodes().size(); node_index++) {
    //         for (size_t control_interval = 0; control_interval < num_control_intervals(); control_interval++) {
    //             parameters[control_index +
    //                        num_control_parameters() * (control_interval + num_control_intervals() * node_index)] =
    //                 control_parameters()[control_index].max();
    //         }
    //     }
    // };

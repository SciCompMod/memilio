...
void SecirvvsOptimization::check_constraint_feasability()
{
    ...
    mio::osecirvvs::Simulation<double> simulation(model, t0(), dt());
    simulation.set_integrator(make_integrator<double>(integrator_type(), dt()));

    update_path_constraint<double>(*this, simulation, path_constraint_values);
    for (size_t interval = 0; interval < num_intervals(); interval++) {
        simulation.advance(time_steps[interval + 1]);
        simulation.get_dt() = dt();
        update_path_constraint<double>(*this, simulation, path_constraint_values);
    }
    update_terminal_constraint<double>(*this, simulation, terminal_constraint_values);

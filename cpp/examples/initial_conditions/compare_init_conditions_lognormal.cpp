...
int main()
{
    ...
    ScalarType t_init_groundtruth = 0.;

    ScalarType t0 = 35.;

    ScalarType t_init_simple         = t0;
    ScalarType t_init_detailed       = t0 - 25.;
    ScalarType t_init_detailed_short = t0 - 15.;
    ...
    std::string save_dir = fmt::format("../../simulation_results/2026-07-29/compare_different_inits_lognorm/"
                                       "nonconst_contacts_tinitgroundtruth={}_tinitdetailed={}_t0={}_tmax={}/",
                                       t_init_groundtruth, t_init_detailed, t0, tmax);
    ...
    mio::IOResult<mio::TimeSeries<ScalarType>> result_ide_groundtruth =
        simulate_ide(ide_exponent, gregory_order, finite_difference_order, t_init_groundtruth, t0, tmax, save_dir,
                     "groundtruth", mio::TimeSeries<ScalarType>((size_t)mio::isir::InfectionState::Count), t0);

    mio::IOResult<mio::TimeSeries<ScalarType>> result_ide_detailed =
        simulate_ide(ide_exponent, gregory_order, finite_difference_order, t_init_detailed, t0, tmax, save_dir,
                     "detailed", result_ide_groundtruth.value(), t0);

    mio::IOResult<mio::TimeSeries<ScalarType>> result_ide_detailed_short =
        simulate_ide(ide_exponent, gregory_order, finite_difference_order, t_init_detailed_short, t0, tmax, save_dir,
                     "detailed_short", result_ide_groundtruth.value(), t0);

    mio::IOResult<mio::TimeSeries<ScalarType>> result_ide_simple =
        simulate_ide(ide_exponent, gregory_order, finite_difference_order, t_init_simple, t0, tmax, save_dir, "simple",
                     result_ide_groundtruth.value(), t0);
}

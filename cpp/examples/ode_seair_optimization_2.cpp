...
void Seair_NLP::eval_objective_constraints(const std::vector<FP>& x, std::vector<FP>& constraints, FP& objective)
{
    ...
    for (int controlIndex = 0; controlIndex < numControlIntervals_; ++controlIndex) {
        sim.get_model().parameters.template get<mio::oseair::SocialDistancing<FP>>() = x[controlIndex];
        sim.get_model().parameters.template get<mio::oseair::Quarantined<FP>>()      = x[controlIndex + numControlIntervals_];
        sim.get_model().parameters.template get<mio::oseair::TestingRate<FP>>()      = x[controlIndex + 2 * numControlIntervals_];
        ...
        for (int i = 0; i < pcresolution_; ++i, ++gridindex) {
            ...
            constraints[gridindex] = sim.get_result().get_last_value()[(int)mio::oseair::InfectionState::Infected];
        }
    }
                                  ...
                                  const Ipopt::IpoptData* ip_data, Ipopt::IpoptCalculatedQuantities* ip_cq)
{
    ...
    std::ofstream outFileResults("SeairOptResult.csv");
    ...
    for (int controlIndex = 0; controlIndex < numControlIntervals_; ++controlIndex) {
        ...
        outFileSocialDistancing << grid[gridindex] << " " << x[controlIndex] << "\n";
        outFileQuarantined << grid[gridindex] << " " << x[controlIndex + numControlIntervals_] << "\n";
        outFileTestingRate << grid[gridindex] << " " << x[controlIndex + 2 * numControlIntervals_] << "\n";

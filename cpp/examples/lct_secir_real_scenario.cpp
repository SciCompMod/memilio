 ...
 */
mio::IOResult<void> set_contact_matrices(const fs::path& data_dir, mio::lsecir::Parameters& params)
{
    ...
    params.get<mio::lsecir::ContactPatterns>() = mio::UncertainContactMatrix(contact_matrices);

    return mio::success();
}
mio::IOResult<void> set_npis(mio::Date start_date, mio::Date end_date, mio::lsecir::Parameters& params)
{
    auto& contacts         = params.get<mio::lsecir::ContactPatterns>();
    ...
    if (start_npi_june < end_date) {
        ...
        v                       = 0.0;
        contact_dampings.push_back(mio::DampingSampling(v, mio::DampingLevel(int(InterventionLevel::Main)),
                                                        mio::DampingType(int(Intervention::SchoolClosure)),
                                                        school_reopen_time, {size_t(ContactLocation::School)},
                                                        Eigen::VectorXd::Constant(1, 1.0))); //school fully reopened
    }
    ...
    if (start_npi_october < end_date) {
        ...
        contact_dampings.push_back(mio::DampingSampling(
            ...
            {size_t(ContactLocation::Home)}, Eigen::VectorXd::Constant(1, 1.0)));
        //physical_distancing_home_school_work_other
        contact_dampings.push_back(
            mio::DampingSampling(v, mio::DampingLevel(int(InterventionLevel::PhysicalDistanceAndMasks)),
...
mio::IOResult<void> simulate(bool save_result = true, bool print_result = false)
{
    ...
    status      = set_npis(start_date, end_date, parameters);
    // maybe the autumn break is missing in end of october.
    ...
    mio::lsecir::InfectionState infectionStates_ode(std::vector<int>((int)mio::lsecir::InfectionStateBase::Count, 1));
    mio::lsecir::Model model_ode(std::move(init_base), infectionStates_ode, std::move(parameters));
    mio::TimeSeries<ScalarType> result_ode = mio::lsecir::simulate(
        0, mio::get_offset_in_days(end_date, start_date), 0.1, model_ode,
        std::make_shared<mio::ControlledStepperWrapper<boost::numeric::odeint::runge_kutta_cash_karp54>>(1e-10, 1e-5, 0,
                                                                                                         0.1));
    // Calculate result without division in subcompartments.
    mio::TimeSeries<ScalarType> populations_ode = model_ode.calculate_populations(result_ode);
    ...
    if (print_result) {
        ...
        mio::lsecir::print_TimeSeries(populations_ode, model_ode.get_heading_CompartmentsBase());
    }
    if (save_result) {
        auto save_result_status = mio::save_result({populations_lct}, {0}, 1, "real_lct_2020_10_15.h5");
        save_result_status      = mio::save_result({populations_ode}, {0}, 1, "real_ode_2020_10_15.h5");
    }
    return mio::success();
}

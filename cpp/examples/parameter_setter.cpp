...
void set_parameters(mio::abm::Parameters& params)
{
    ...
    // go_to_shop() has no age gate of its own (unlike go_to_school()/go_to_work()): it samples a
    // shop trip purely from BasicShoppingRate, which defaults to 1.0 for every age group. City builder
    // never assigns 0-4-year-olds a BasicsShop location, so their rate must be zeroed here, or their
    // draw eventually fires and find_location() asserts on the missing location.
    params.get<mio::abm::BasicShoppingRate>()[age_group_0_to_4] = 0;
}

namespace
{
// ContactRates is a ContactMatrix (Eigen-backed baseline), not a CustomIndexArray,
// so values are copied element-wise into get_baseline() instead of assigned directly.
void assign_contact_rates(mio::abm::Location& loc,
                           const mio::CustomIndexArray<ScalarType, mio::AgeGroup, mio::AgeGroup>& contacts)
{
    auto& baseline     = loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline();
    const int n_groups = static_cast<int>(baseline.rows());
    for (int i = 0; i < n_groups; ++i) {
        for (int j = 0; j < n_groups; ++j) {
            baseline(i, j) = contacts[{mio::AgeGroup(i), mio::AgeGroup(j)}];
        }
    }
}
} // namespace
    ...
    */
    ...
    for (auto& loc : world.get_locations()) {
        ...
        case mio::abm::LocationType::Home:
            assign_contact_rates(loc, contacts_home);
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *=
                1.4; //17 hours //intensity
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 15.0; // Intensity
            break;
        case mio::abm::LocationType::School:
            assign_contact_rates(loc, contacts_school);
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 4.8; //5h
            break;
        case mio::abm::LocationType::Work:
            assign_contact_rates(loc, contacts_work);
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 3.0 * 0.5; // 7h
            break;
        case mio::abm::LocationType::SocialEvent:
            assign_contact_rates(loc, contacts_other);
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *=
                1.2; //aufteilung
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 2.0; // intensity
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 6.0; // 4 hours
            break;
        case mio::abm::LocationType::BasicsShop:
            assign_contact_rates(loc, contacts_other);
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *=
                0.8; //aufteilung
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 0.33; // intensity
            loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= 12.0; // 2 hours
            break;
        default:
            assign_contact_rates(loc, contacts_random);
            break;
        }
    }
}
...
void set_local_parameters_event(mio::abm::Model& world, double contact_rate_multiplier)
{
    ...
    // Every location type receives the same multiplier here, so no per-type switch is needed.
    for (auto& loc : world.get_locations()) {
        loc.get_infection_parameters().get<mio::abm::ContactRates>().get_baseline().array() *= contact_rate_multiplier;
    }
}
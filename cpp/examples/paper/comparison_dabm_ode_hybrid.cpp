...
mio::dabm::Model<SingleWell<mio::osecir::InfectionState>> initialize_abm()
{
    using Model = mio::dabm::Model<SingleWell<mio::osecir::InfectionState>>;

    std::vector<Model::Agent> agents(params::total_population);
    ...
    for (auto& a : agents) {
        ...
        //All agents except one are initially susceptible
        a.status = mio::osecir::InfectionState::Susceptible;
    }
    // One agent is initially exposed
...
mio::IOResult<void> simulate_dabm(std::string result_dir, size_t num_runs)
{

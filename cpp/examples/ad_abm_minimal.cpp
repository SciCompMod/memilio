...
int main()
{
    ...
    auto world = mio::abm::World<FP>(num_age_groups);

    // Set same infection parameter for all age groups. For example, the incubation period is 4 days.
    world.parameters.get<mio::abm::IncubationPeriod<FP>>() = static_cast<FP>(4.);
    ...
    auto test_type             = mio::abm::AntigenTest<FP>();
    auto testing_criteria_work = mio::abm::TestingCriteria();
    auto testing_scheme_work =
        mio::abm::TestingScheme<FP>(testing_criteria_work, testing_min_time, start_date, end_date, test_type, probability);
    ...
    for (auto& person : world.get_persons()) {
        ...
        auto rng = mio::abm::Person<FP>::RandomNumberGenerator(world.get_rng(), person);

...
// template <typename FP>
// using Graph = mio::Graph<EMBModel<FP>, mio::MobilityParameters<FP>>;
...
public:
    ...
    OptimizationModel(boost::filesystem::path data_directory, double t0, double tmax, int num_age_groups = 6)
...
private:
    ...
    double m_t0;
    double m_tmax;
    int m_num_age_groups;
    bool use_county = true;
};
    ...
    for (auto&& contact_location : contact_locations) {
        BOOST_OUTCOME_TRY(auto&& baseline,
                          mio::read_mobility_plain(
                              (m_data_directory / "contacts" / ("baseline_" + contact_location.second + ".txt")).string()));
...
template <typename FP>
mio::IOResult<void> OptimizationModel::set_population_data_germany(Graph<FP>& params_graph, mio::osecirvvs::Parameters<FP>& params)
{  
...
template <typename FP>
mio::IOResult<void> OptimizationModel::set_population_data_county(Graph<FP>& graph, mio::osecirvvs::Parameters<FP>& params)
{  
    ...
    const auto& set_node_function =
        mio::set_nodes<mio::osecirvvs::TestAndTraceCapacity<FP>, mio::osecirvvs::ContactPatterns<FP>,
 ...
 */
template <typename FP>
mio::IOResult<void> OptimizationModel::set_initial_values(Graph<FP>& params_graph)
{
...
template <typename FP>
Graph<FP> OptimizationModel::create_model()
{
    Graph<FP> graph;
    auto out = set_initial_values_germany<FP>(graph);
    // auto graph2 = graph;

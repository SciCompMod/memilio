...
mio::IOResult<MultiRunResults> MultiRunSimulator::run_multi_simulation(const MultiRunConfig& config)
{
    ...
    if (config.event_config.use_panvadere_init) {
        ...
        BOOST_OUTCOME_TRY(auto infections, EventSimulator::initialize_from_panvadere(
            config.event_config.panvadere_file, config.event_config.type));
        ...
        BOOST_OUTCOME_TRY(auto infections, EventSimulator::initialize_from_event_simulation(
            config.event_config, base_world));
        initial_infections = infections;
    }
    ...
    for (int run = 0; run < config.num_runs; ++run) {
        ...
        auto single_result = run_single_simulation_with_infections(
            base_world, initial_infections, k_value, config.simulation_days);
    ...
    if (results.successful_runs > 0) {
        auto final_results = calculate_percentiles(results.all_runs, results.event_type, 
                                                  results.used_panvadere_init, k_value);
...
MultiRunResults MultiRunSimulator::calculate_percentiles(const std::vector<SimulationRunner::SimulationResults>& runs,
                                                        EventType event_type, bool used_panvadere, double k_value)
{
    MultiRunResults results;
    results.event_type = event_type;
    results.used_panvadere_init = used_panvadere;
    results.infection_parameter_k = k_value;
    results.successful_runs = runs.size();
    
    // TODO: Calculate actual percentiles from time series data
    // For now, just take middle results as placeholders
    if (runs.size() >= 3) {
        size_t idx_25 = runs.size() / 4;
        size_t idx_50 = runs.size() / 2;
        size_t idx_75 = (3 * runs.size()) / 4;
        
        results.percentile_25 = runs[idx_25];
        results.percentile_50 = runs[idx_50];
        results.percentile_75 = runs[idx_75];
    }
    
    return results;
}

mio::IOResult<SimulationRunner::SimulationResults> MultiRunSimulator::run_single_simulation_with_infections(
    const mio::abm::World& base_world, 

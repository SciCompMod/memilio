                                         ...
                                         size_t num_agegroups)
{
    ...
    // An age group must not have both GotoSchool and GotoWork set: go_to_school()/go_to_work() only
    // check the age-group flag (no per-person opt-out), so every person in a flagged age group needs
    // a matching location, which create_and_assign_people_to_locations only gives one type per person.
    ...
    const CityParameters::CityInfrastructure infra)
{
    ...
    // No age group may have both AgeGroupGotoSchool and AgeGroupGotoWork set to true (see build_world):
    // go_to_school()/go_to_work() only check the age-group flag, with no per-person opt-out, so every
    // person in a flagged age group must have a matching location. 15-34 only works; only 5-14 goes to
    // school, always covered below regardless of capacity, since AgeGroupGotoSchool requires it.
    int amount_of_60plus_that_work = infra.num_worker - age_vector[2] - age_vector[3];
    ...
    for (auto& person : world.get_persons()) {
        ...
        if (person.get_age() == age_group_5_to_14) {
            // Always assign a school (elementary first, then secondary): AgeGroupGotoSchool requires
            // every 5-14 person to have one, so this must not be gated by infra's planned capacity,
            // which can drift from the realized population.
            if (count_elementary_schools < infra.num_persons_elementary_schools || infra.num_secondary_schools == 0) {
            ...
            // Everyone in these age groups needs a Work location: AgeGroupGotoWork is set for the
            // whole age group, and go_to_work()/find_location() assert if a person has none.
            // Realistic employment rate (infra.num_worker) is modeled via WorkRatio (attendance),
            // not by withholding the location assignment.
    ...
    // infra.num_worker is the employment-rate-scaled target used for city statistics (workplace
    // sizing etc.), not the actual count of assigned Work locations: every 15-34/35-59 person not
    // diverted to school gets one (see above), so workerss is expected to exceed infra.num_worker.

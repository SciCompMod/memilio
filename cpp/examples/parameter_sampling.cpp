...
int main()
{
    ...
    params.get_contact_patterns().get_dampings().push_back(epi::DampingSampling(
        epi::UncertainValue(0.5), epi::DampingLevel(0), epi::DampingType(0), epi::SimulationTime(30.),
        std::vector<size_t>(1, size_t(0)), Eigen::VectorXd::Constant(Eigen::Index(nb_groups), 1.0)));
    params.get_contact_patterns().get_dampings()[0].get_value().set_distribution(

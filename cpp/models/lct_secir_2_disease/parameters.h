...
using ParametersBase =
    ParameterSet<TimeExposed_a, TimeInfectedNoSymptoms_a, TimeInfectedSymptoms_a, TimeInfectedSevere_a,
                 ...
                 TransmissionProbabilityOnContact_b, ContactRate, RelativeTransmissionNoSymptoms_a,
...
public:
    ...
    bool check_constraints() const
    {
        ...
        if (this->get<TimeExposed_a>() < 1.0) {
            log_error("Constraint check: Parameter TimeExposed_a is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeExposed_b>() < 1.0) {
            log_error("Constraint check: Parameter TimeExposed_b is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedNoSymptoms_a>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedNoSymptoms_a is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedNoSymptoms_b>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedNoSymptoms_b is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedSymptoms_a>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedSymptoms_a is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedSymptoms_b>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedSymptoms_b is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedSevere_a>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedSevere_a is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedSevere_b>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedSevere_b is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedCritical_a>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedCritical_a is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TimeInfectedCritical_b>() < 1.0) {
            log_error("Constraint check: Parameter TimeInfectedCritical_b is smaller than {:.4f}", 1.0);
            return true;
        }

        if (this->get<TransmissionProbabilityOnContact_a>() < 0.0 ||
            this->get<TransmissionProbabilityOnContact_a>() > 1.0) {
            log_error("Constraint check: Parameter TransmissionProbabilityOnContact_a smaller {:d} or larger {:d}", 0,
                      1);
            return true;
        }

        if (this->get<TransmissionProbabilityOnContact_b>() < 0.0 ||
            this->get<TransmissionProbabilityOnContact_b>() > 1.0) {
            log_error("Constraint check: Parameter TransmissionProbabilityOnContact_b smaller {:d} or larger {:d}", 0,
                      1);
            return true;
        }

        if (this->get<RelativeTransmissionNoSymptoms_a>() < 0.0 ||
            this->get<RelativeTransmissionNoSymptoms_a>() > 1.0) {
            log_error("Constraint check: Parameter RelativeTransmissionNoSymptoms_a smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<RelativeTransmissionNoSymptoms_b>() < 0.0 ||
            this->get<RelativeTransmissionNoSymptoms_b>() > 1.0) {
            log_error("Constraint check: Parameter RelativeTransmissionNoSymptoms_b smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<RiskOfInfectionFromSymptomatic_a>() < 0.0 ||
            this->get<RiskOfInfectionFromSymptomatic_a>() > 1.0) {
            log_error("Constraint check: Parameter  RiskOfInfectionFromSymptomatic_a smaller {:d} or larger {:d}", 0,
                      1);
            return true;
        }

        if (this->get<RiskOfInfectionFromSymptomatic_b>() < 0.0 ||
            this->get<RiskOfInfectionFromSymptomatic_b>() > 1.0) {
            log_error("Constraint check: Parameter  RiskOfInfectionFromSymptomatic_b smaller {:d} or larger {:d}", 0,
                      1);
            return true;
        }

        if (this->get<RecoveredPerInfectedNoSymptoms_a>() < 0.0 ||
            this->get<RecoveredPerInfectedNoSymptoms_a>() > 1.0) {
            log_error("Constraint check: Parameter RecoveredPerInfectedNoSymptoms_a smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<RecoveredPerInfectedNoSymptoms_b>() < 0.0 ||
            this->get<RecoveredPerInfectedNoSymptoms_b>() > 1.0) {
            log_error("Constraint check: Parameter RecoveredPerInfectedNoSymptoms_b smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<SeverePerInfectedSymptoms_a>() < 0.0 || this->get<SeverePerInfectedSymptoms_a>() > 1.0) {
            log_error("Constraint check: Parameter SeverePerInfectedSymptoms_a smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<SeverePerInfectedSymptoms_b>() < 0.0 || this->get<SeverePerInfectedSymptoms_b>() > 1.0) {
            log_error("Constraint check: Parameter SeverePerInfectedSymptoms_b smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<CriticalPerSevere_a>() < 0.0 || this->get<CriticalPerSevere_a>() > 1.0) {
            log_error("Constraint check: Parameter CriticalPerSevere_a smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<CriticalPerSevere_b>() < 0.0 || this->get<CriticalPerSevere_b>() > 1.0) {
            log_error("Constraint check: Parameter CriticalPerSevere_b smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<DeathsPerCritical_a>() < 0.0 || this->get<DeathsPerCritical_a>() > 1.0) {
            log_error("Constraint check: Parameter DeathsPerCritical_a smaller {:d} or larger {:d}", 0, 1);
            return true;
        }

        if (this->get<DeathsPerCritical_b>() < 0.0 || this->get<DeathsPerCritical_b>() > 1.0) {

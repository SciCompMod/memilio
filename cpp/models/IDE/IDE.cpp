...
/**
    ...
    */
double IdeModel::S_derivative(Eigen::Index idx) const
{
...
const TimeSeries<double>& IdeModel::simulate(int t_max)
{
    while (result.get_last_time() < t_max) {
        ...
        result.get_last_value() =
            Vec::Constant(1, result[idx - 2][0] * exp(dt * (0.5 * 1 / N) *
...
const TimeSeries<double>& IdeModel::calculate_EIR()
{
    ...
    double S, E, I, R;
    for (Eigen::Index i = k; i < num_points; ++i) {
        S = result[i][0];
        E = result[i - l][0] - S;

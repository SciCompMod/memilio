...
void draw_sample_demographics(Model& model)
{
    ...
    for (auto i = AgeGroup(0); i < model.parameters.get_num_groups(); i++) {
        ...
        if (diff > 0) {
            ...
            assert(std::abs(group_total - model.populations.get_group_total(i)) < 1e-8 && "Sanity check.");
        }
...
void draw_sample_infection(Model& model)
{
    ...
    for (auto i = AgeGroup(0); i < model.parameters.get_num_groups(); i++) {
        ...
        model.parameters.get<ExposedFactorPartialImmunity>()[i] =
            model.parameters.get<ExposedFactorPartialImmunity>()[AgeGroup(0)];
        model.parameters.get<ExposedFactorImprovedImmunity>()[i] =
            model.parameters.get<ExposedFactorImprovedImmunity>()[AgeGroup(0)];
        model.parameters.get<InfectedFactorPartialImmunity>()[i] =
            model.parameters.get<InfectedFactorPartialImmunity>()[AgeGroup(0)];
        model.parameters.get<InfectedFactorImprovedImmunity>()[i] =
            model.parameters.get<InfectedFactorImprovedImmunity>()[AgeGroup(0)];
        model.parameters.get<HospitalizedFactorPartialImmunity>()[i] =
            model.parameters.get<HospitalizedFactorPartialImmunity>()[AgeGroup(0)];
        model.parameters.get<HospitalizedFactorImprovedImmunity>()[i] =
            model.parameters.get<HospitalizedFactorImprovedImmunity>()[AgeGroup(0)];
        model.parameters.get<InfectiousTimeFactorImmune>()[i] =
            model.parameters.get<InfectiousTimeFactorImmune>()[AgeGroup(0)];

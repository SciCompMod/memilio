                                    ...
                                    const std::vector<int>& vregion, int num_days)
{
    ...
    for (auto&& vacc_data_entry : vacc_data) {
        ...
        if (it != vregion.end()) {
            ...
            if (date_df ==
                offset_date_by_days(date, -model[region_idx].parameters.template get<ImmunityInterval1>()[age])) {
                model[region_idx].populations[{age, InfectionState::TemporaryImmunity1}] -=
                    vacc_data_entry.num_first_vaccinations_completed;
            }
            if (date_df ==
                offset_date_by_days(date, -model[region_idx].parameters.template get<ImmunityInterval2>()[age])) {
                model[region_idx].populations[{age, InfectionState::TemporaryImmunity2}] -=
                    vacc_data_entry.num_second_vaccinations_completed + vacc_data_entry.num_vaccinations_refreshed +
                    vacc_data_entry.num_vaccinations_refreshed_2;
            }
            ...
            for (size_t d = 0; d < (size_t)num_days + 1; ++d) {
                ...
                auto offset_first_date = offset_date_by_days(date, (int)d - days_until_effective_pi);
                ...
                auto offset_full_date = offset_date_by_days(date, (int)d - days_until_effective_ii);
                if (max_date >= offset_full_date) {
                    // Option 1: considered offset_partial_date is available in input data frame
            ...
            // // also go through the last ImmunityInterval1 and  ImmunityInterval2-days
            // auto max_immunity_interval = std::max(model[region_idx].parameters.template get<ImmunityInterval2>()[age],
            //                                       model[region_idx].parameters.template get<ImmunityInterval1>()[age]);
            // for (size_t d = 0; d < max_immunity_interval; ++d) {
            //     auto offset_first_date = offset_date_by_days(date, (int)d - days_until_effective_pi);
            //     if (max_date >= offset_first_date) {
            //         // Option 1: considered offset_first_date is available in input data frame
            //         if (date_df == offset_first_date) {

                                        ...
                                        bool set_death)
{
    ...
    for (size_t i = 0; i < num_groups; i++) {
        ...
        model.populations[{AgeGroup(i), InfectionState::InfectedNoSymptomsNaive}]          = num_InfectedNoSymptoms[i];
        model.populations[{AgeGroup(i), InfectionState::InfectedNoSymptomsNaiveConfirmed}] = 0;
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsNaive}]            = num_InfectedSymptoms[i];
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsNaiveConfirmed}]   = 0;
        model.populations[{AgeGroup(i), InfectionState::InfectedSevereNaive}]              = num_InfectedSevere[i];
    ...
    for (size_t group = 0; group < num_age_groups; group++) {
        ...
        t_InfectedSymptoms.push_back(static_cast<int>(
            std::round(model.parameters.template get<TimeInfectedSymptoms<ScalarType>>()[(AgeGroup)group] * reduc_t)));
        t_InfectedSevere.push_back(static_cast<int>(
            std::round(model.parameters.template get<TimeInfectedSevere<ScalarType>>()[(AgeGroup)group])));
        t_InfectedCritical.push_back(static_cast<int>(
            std::round(model.parameters.template get<TimeInfectedCritical<ScalarType>>()[(AgeGroup)group])));
        ...
        ScalarType hosp_fac_part_immune =
            model.parameters
                .template get<ReducInfectedSevereCriticalDeadPartialImmunity<ScalarType>>()[(AgeGroup)group];
        ...
        mu_C_R.push_back(
            (1 -
             inf_fac_part_immune / exp_fac_part_immune *
                 (1 - model.parameters.template get<RecoveredPerInfectedNoSymptoms<ScalarType>>()[(AgeGroup)group])));
        mu_I_H.push_back(hosp_fac_part_immune / inf_fac_part_immune *
                         model.parameters.template get<SeverePerInfectedSymptoms<ScalarType>>()[(AgeGroup)group]);
        // transfer from H to U, D unchanged.
        mu_H_U.push_back(icu_fac_part_immune / hosp_fac_part_immune *
    ...
    for (size_t i = 0; i < num_groups; i++) {
        ...
        model.populations[{AgeGroup(i), InfectionState::InfectedNoSymptomsPartialImmunity}] = num_InfectedNoSymptoms[i];
        model.populations[{AgeGroup(i), InfectionState::InfectedNoSymptomsPartialImmunityConfirmed}] = 0;
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsPartialImmunity}] = num_InfectedSymptoms[i];
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsPartialImmunityConfirmed}] = 0;
        model.populations[{AgeGroup(i), InfectionState::InfectedSeverePartialImmunity}] = num_InfectedSevere[i];
        // Only set the number of ICU patients here, if the date is not available in the data.
        if (!is_divi_data_available(date)) {
            model.populations[{AgeGroup(i), InfectionState::InfectedCriticalPartialImmunity}] = num_icu[i];
        }
    }
    // }
    if (std::accumulate(num_InfectedSymptoms.begin(), num_InfectedSymptoms.end(), ScalarType(0.0),
    ...
    for (size_t group = 0; group < num_age_groups; group++) {
        ...
        ScalarType reduc_immune_hosp =
            model.parameters
                .template get<ReducInfectedSevereCriticalDeadImprovedImmunity<ScalarType>>()[(AgeGroup)group];
        ...
        mu_C_R.push_back(
            (1 -
             reduc_immune_inf / reduc_immune_exp *
                 (1 - model.parameters.template get<RecoveredPerInfectedNoSymptoms<ScalarType>>()[(AgeGroup)group])));
        mu_I_H.push_back(reduc_immune_hosp / reduc_immune_inf *
                         model.parameters.template get<SeverePerInfectedSymptoms<ScalarType>>()[(AgeGroup)group]);
        // transfer from H to U, D unchanged.
        mu_H_U.push_back(reduc_immune_icu / reduc_immune_hosp *
                         model.parameters.template get<CriticalPerSevere<ScalarType>>()[(AgeGroup)group]);
    }
    ...
    for (size_t i = 0; i < num_groups; i++) {
        ...
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsImprovedImmunity}] = num_InfectedSymptoms[i];
        model.populations[{AgeGroup(i), InfectionState::InfectedSymptomsImprovedImmunityConfirmed}] = 0;
        model.populations[{AgeGroup(i), InfectionState::InfectedSevereImprovedImmunity}] = num_InfectedSevere[i];
        // Only set the number of ICU patients here, if the date is not available in the data.
        if (!is_divi_data_available(date)) {
            model.populations[{AgeGroup(i), InfectionState::InfectedCriticalImprovedImmunity}] = num_icu[i];
        }
    }
...
IOResult<void> set_confirmed_cases_data(const mio::VectorRange<Node<Model<ScalarType>>>& model, const std::string& path,
                                        Date date, const std::vector<ScalarType>& scaling_factor_inf, bool set_death)
{
    ...
    for (size_t region_idx = 0; region_idx < model.size(); ++region_idx) {
        BOOST_OUTCOME_TRY(set_confirmed_cases_data(model[region_idx].property, vcase_data[region_idx],
                                                   model[region_idx].id, date, scaling_factor_inf, set_death));
    }
    return success();
}
                                   ...
                                   const int region, const std::vector<ScalarType>& num_rec)
{
    ...
    for (auto i = AgeGroup(0); i < num_groups; i++) {
        ...
        ScalarType S_v =
            std::min(model.parameters.template get<DailyFullVaccinations<ScalarType>>()[{i, SimulationDay(0)}] +
                         num_rec[size_t(i)],
                     num_population[size_t(i)]);
        ScalarType S_pv =
            std::max(model.parameters.template get<DailyPartialVaccinations<ScalarType>>()[{i, SimulationDay(0)}] -
        ...
        if (num_population[size_t(i)] - S_pv - S_v < 0.0) {
            log_warning("Number of vaccinated persons greater than population in county {}, age group {}.", region,
                        size_t(i));
            S   = 0.0;
        ...
        ScalarType denom_C =
            1 / (S + S_pv * model.parameters.template get<ReducExposedPartialImmunity<ScalarType>>()[i] +
                 S_v * model.parameters.template get<ReducExposedImprovedImmunity<ScalarType>>()[i]);
        ScalarType denom_I =
            1 / (S + S_pv * model.parameters.template get<ReducInfectedSymptomsPartialImmunity<ScalarType>>()[i] +
                 S_v * model.parameters.template get<ReducInfectedSymptomsImprovedImmunity<ScalarType>>()[i]);
        ScalarType denom_HU =
            1 /
            (S + S_pv * model.parameters.template get<ReducInfectedSevereCriticalDeadPartialImmunity<ScalarType>>()[i] +
             S_v * model.parameters.template get<ReducInfectedSevereCriticalDeadImprovedImmunity<ScalarType>>()[i]);
        ...
        model.populations[{i, InfectionState::InfectedNoSymptomsPartialImmunityConfirmed}] =
            S_pv * model.populations[{i, InfectionState::InfectedNoSymptomsPartialImmunityConfirmed}] * denom_C;
        model.populations[{i, InfectionState::InfectedNoSymptomsImprovedImmunityConfirmed}] =
            S_v * model.populations[{i, InfectionState::InfectedNoSymptomsImprovedImmunityConfirmed}] * denom_C;
        ...
        model.populations[{i, InfectionState::InfectedSeverePartialImmunity}] =
            S_pv * model.parameters.template get<ReducInfectedSevereCriticalDeadPartialImmunity<ScalarType>>()[i] *
        ...
        for (auto j = Index<InfectionState>(0); j < InfectionState::Count; ++j) {
            if (model.populations[{i, j}] < 0) {
                log_warning("Compartment at age group {}, infection state {}, is negative: {}", size_t(i), size_t(j),
                            model.populations[{i, j}] / num_population[size_t(i)]);
            }
        }
    }
                                   ...
                                   const std::string& path_rki, Date date)
{
    ...
    for (size_t region_idx = 0; region_idx < model.size(); ++region_idx) {
        BOOST_OUTCOME_TRY(auto&& num_rec, compute_confirmed_cases_data_fix_recovered(vcase_data[region_idx],
                                                                                     model[region_idx].id, date, 14.));
        BOOST_OUTCOME_TRY(set_population_data(model[region_idx].property, vnum_population[region_idx],
                                              model[region_idx].id, num_rec));
    }
    return success();
}

    ...
    const std::vector<std::vector<double>>& vmu_U_D, const std::vector<double>& scaling_factor_inf)
{
    ...
    return read_confirmed_cases_data(
        rki_data, vregion, date, vnum_exp, vnum_car, vnum_inf, vnum_hosp, vnum_icu, vnum_death, vnum_rec, vnum_timm_i,
        vt_car_to_rec, vt_car_to_inf, vt_exp_to_car, vt_inf_to_rec, vt_inf_to_hosp, vt_hosp_to_rec, vt_hosp_to_icu,
        vt_icu_to_dead, vt_icu_to_rec, vt_imm_interval_i, vmu_C_R, vmu_I_H, vmu_H_U, vmu_U_D, scaling_factor_inf);
}
    ...
    const std::vector<double>& scaling_factor_inf)
{
    ...
    auto vnum_inf_timm_i   = vnum_inf;
    auto vnum_rec_timm_i   = vnum_rec;
    auto vnum_exp_timm_i   = vnum_exp;
    auto vnum_hosp_timm_i  = vnum_hosp;
    auto vnum_death_timm_i = vnum_death;
    auto vnum_icu_timm_i   = vnum_icu;
    ...
    for (auto&& entry : rki_data) {
        ...
        if (it != vregion.end()) {
            ...
            auto& num_inf_timm_i   = vnum_inf_timm_i[region_idx];
            auto& num_rec_timm_i   = vnum_rec_timm_i[region_idx];
            auto& num_hosp_timm_i  = vnum_hosp_timm_i[region_idx];
            auto& num_death_timm_i = vnum_death_timm_i[region_idx];
            auto& num_icu_timm_i   = vnum_icu_timm_i[region_idx];
            ...
            if (entry.date == offset_date_by_days(date, 0 - t_imm_interval_i[age])) {
                num_inf_timm_i[age] += mu_I_H[age] * scaling_factor_inf[age] * entry.num_confirmed;
                num_inf_timm_i[age] += (1 - mu_I_H[age]) * scaling_factor_inf[age] * entry.num_confirmed;
                num_rec_timm_i[age] += entry.num_confirmed;
            }
            // -R4
            if (entry.date == offset_date_by_days(date, -t_inf_to_rec[age] - t_imm_interval_i[age])) {
                num_inf_timm_i[age] -= (1 - mu_I_H[age]) * scaling_factor_inf[age] * entry.num_confirmed;
            }
            // -R6
            if (entry.date == offset_date_by_days(date, -t_inf_to_hosp[age] - t_imm_interval_i[age])) {
                num_inf_timm_i[age] -= mu_I_H[age] * scaling_factor_inf[age] * entry.num_confirmed;
                num_hosp_timm_i[age] += mu_I_H[age] * mu_H_U[age] * scaling_factor_inf[age] * entry.num_confirmed;
                num_hosp_timm_i[age] += mu_I_H[age] * (1 - mu_H_U[age]) * scaling_factor_inf[age] * entry.num_confirmed;
            }
            // -R6 - R7
            if (entry.date ==
                offset_date_by_days(date, -t_inf_to_hosp[age] - t_hosp_to_icu[age] - t_imm_interval_i[age])) {
                num_hosp_timm_i[age] -= mu_I_H[age] * mu_H_U[age] * scaling_factor_inf[age] * entry.num_confirmed;
                if (read_icu) {
                    num_icu_timm_i[age] +=
                        mu_I_H[age] * mu_H_U[age] * mu_U_D[age] * scaling_factor_inf[age] * entry.num_confirmed;
            ...
            // -R6 - R5
            if (entry.date ==
                offset_date_by_days(date, -t_inf_to_hosp[age] - t_hosp_to_rec[age] - t_imm_interval_i[age])) {
                num_hosp_timm_i[age] -= mu_I_H[age] * (1 - mu_H_U[age]) * scaling_factor_inf[age] * entry.num_confirmed;
            }
            // -R10 - R6 - R7 // - T_I^H - T_H^U - T_U^D
            if (entry.date == offset_date_by_days(date, -t_inf_to_hosp[age] - t_hosp_to_icu[age] - t_icu_to_dead[age] -
                                                            t_imm_interval_i[age])) {
    ...
    for (size_t region_idx = 0; region_idx < vregion.size(); ++region_idx) {
        ...
        for (size_t i = 0; i < num_groups; i++) {
            ...
            num_rec_timm_i[i] -= num_inf_timm_i[i] / scaling_factor_inf[i];
            num_rec_timm_i[i] -= num_hosp_timm_i[i] / scaling_factor_inf[i];
            num_rec_timm_i[i] -=
                num_icu_timm_i[i] /
                scaling_factor_inf[i]; // TODO: this has to be adapted for scaling_factor_inf != 1 or != ***_icu
            num_rec_timm_i[i] -= num_death_timm_i[i] / scaling_factor_inf[i];

            num_timm_i[i] = num_rec[i] - num_rec_timm_i[i];
    ...
    for (auto&& vacc_data_entry : vacc_data) {
        ...
        if (it != vregion.end()) {
            ...
            for (size_t d = 0; d < (size_t)num_days + 1; ++d) {
                ...
                // else {
                //     log_error("Situation not defined yet but also not used.");
                // }
                ...
                //         model[region_idx].parameters.template get<DailyPartialVaccination>()[{age, SimulationDay(d)}] -=
                //             (days_plus - 1) * vacc_data_entry.num_second_vaccinations_completed;
                //     }
                //     else if (date_df == max_date) {
                //         model[region_idx].parameters.template get<DailyPartialVaccination>()[{age, SimulationDay(d)}] +=
                //             days_plus * vacc_data_entry.num_second_vaccinations_completed;
                //     }
                // }
                ...
                // else {
                //     log_error("Situation not defined yet");
                // }

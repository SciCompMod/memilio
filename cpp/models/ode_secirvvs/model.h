            ...
            for (auto i = AgeGroup(0); i < num_groups; ++i) {
                FP new_transmission =
                    ...
                    share_new_variant * base_infectiousness[i] *
                        this->get_model().parameters.template get<InfectiousnessNewVariant<FP>>()[i].value();
                this->get_model().parameters.template get<TransmissionProbabilityOnContact<FP>>()[i].value() = new_transmission;
            }
        }
    }
...
private:
    ...
    std::pair<FP, SimulationTime<FP>> m_dynamic_npi = {-std::numeric_limits<double>::max(), SimulationTime(0)};
};

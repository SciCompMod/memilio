...
std::vector<Model> ensemble_params_percentile(const std::vector<std::vector<Model>>& ensemble_params, double p)
{
    ...
    for (size_t node = 0; node < num_nodes; node++) {
        ...
        for (auto i = AgeGroup(0); i < AgeGroup(num_groups); i++) {
            ...
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<ExposedFactorPartialImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<ExposedFactorImprovedImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<InfectedFactorPartialImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<InfectedFactorImprovedImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<HospitalizedFactorPartialImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<HospitalizedFactorImprovedImmunity>()[i];
                });
            param_percentil(
                node, [i](auto&& model) -> auto& {
                    return model.parameters.template get<InfectiousTimeFactorImmune>()[i];
                });
            param_percentil(

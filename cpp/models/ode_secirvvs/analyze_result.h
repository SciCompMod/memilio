...
std::vector<Model> ensemble_params_percentile(const std::vector<std::vector<Model>>& ensemble_params, FP p)
{
    ...
    for (size_t node = 0; node < num_nodes; node++) {
        ...
        for (auto i = AgeGroup(0); i < AgeGroup(num_groups); i++) {
            ...
            param_percentil(node, [i](auto&& model) -> auto& {
                return model.parameters.template get<VaccinationGap<FP>>()[i];
            });
            param_percentil(node, [i](auto&& model) -> auto& {
                return model.parameters.template get<DaysUntilEffectivePartialImmunity<FP>>()[i];
            });
            param_percentil(node, [i](auto&& model) -> auto& {
                return model.parameters.template get<DaysUntilEffectiveImprovedImmunity<FP>>()[i];
            });

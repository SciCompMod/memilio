...
IOResult<void> read_confirmed_cases_data(
    ...
    const std::vector<std::vector<int>>& vt_car_to_rec, std::vector<std::vector<double>>& vnum_timm_i,
    ...
    const std::vector<std::vector<int>>& vt_imm_interval_i, const std::vector<std::vector<double>>& vmu_C_R,
...
IOResult<void> read_confirmed_cases_data(
    ...
    const std::vector<std::vector<int>>& vt_icu_to_rec, const std::vector<std::vector<int>>& vt_imm_interval_i,
                                        ...
                                        const std::vector<double>& scaling_factor_inf)
{
    ...
    std::string path_immunity_data = "/localdata1/code/memilio/data/Germany/immunity_population.txt";
    BOOST_OUTCOME_TRY(immunity_population, read_immunity_population(path_immunity_data, num_age_groups));
    mio::unused(immunity_population);
    ...
    for (size_t county = 0; county < model.size(); county++) {
        ...
        for (size_t group = 0; group < num_age_groups; group++) {
            ...
            t_imm_interval1[county].push_back(
                static_cast<int>(model[county].parameters.template get<ImmunityInterval1>()[(AgeGroup)group]));
            t_imm_interval2[county].push_back(
                static_cast<int>(model[county].parameters.template get<ImmunityInterval2>()[(AgeGroup)group]));
    ...
    BOOST_OUTCOME_TRY(read_confirmed_cases_data(
        rki_data, region, date, num_exp, num_car, num_inf, num_hosp, num_icu, num_death, num_rec, num_timm1,
        t_car_to_rec, t_car_to_inf, t_exp_to_car, t_inf_to_rec, t_inf_to_hosp, t_hosp_to_rec, t_hosp_to_icu,
        t_icu_to_dead, t_icu_to_rec, t_imm_interval1, mu_C_R, mu_I_H, mu_H_U, mu_U_D, scaling_factor_inf));
    ...
    for (size_t county = 0; county < model.size(); county++) {
        ...
        for (size_t i = 0; i < num_groups; i++) {
            ...
            ;
            //+ model[county].parameters.template get<DailyPartialVaccination>()[{AgeGroup(i), SimulationDay(0)}]; //
            // Recovered are set later in the population initialization
    ...
    BOOST_OUTCOME_TRY(read_confirmed_cases_data(
        rki_data, region, date, num_exp, num_car, num_inf, num_hosp, num_icu, num_death, num_rec, num_timm2,
        t_car_to_rec, t_car_to_inf, t_exp_to_car, t_inf_to_rec, t_inf_to_hosp, t_hosp_to_rec, t_hosp_to_icu,
        t_icu_to_dead, t_icu_to_rec, t_imm_interval2, mu_C_R, mu_I_H, mu_H_U, mu_U_D, scaling_factor_inf));
    ...
    for (size_t county = 0; county < model.size(); county++) {
        ...
        for (size_t i = 0; i < num_groups; i++) {
            ...
            model[county].populations[{AgeGroup(i), InfectionState::CarrierPartialImmunity}]      = num_car[county][i];
            model[county].populations[{AgeGroup(i), InfectionState::InfectedPartialImmunity}]     = num_inf[county][i];
            model[county].populations[{AgeGroup(i), InfectionState::HospitalizedPartialImmunity}] = num_hosp[county][i];
            model[county].populations[{AgeGroup(i), InfectionState::TemporaryImmunity2}] =
                num_timm2[county][i] * immunity_population[i][2];
            //+model[county].parameters.template get<DailyFullVaccination>()[{AgeGroup(i), SimulationDay(0)}];
            ;
        }
        // }
        if (std::accumulate(num_inf[county].begin(), num_inf[county].end(), 0.0) == 0) {
                                   ...
                                   const std::vector<int>& vregion, Date date)
{
    ...
    std::string path_immunity_data =
        "/localdata1/code/memilio/data/Germany/immunity_population.txt"; //"data/Germany/immunity_population.txt";
    ...
    for (size_t region = 0; region < vregion.size(); region++) {
        if (std::accumulate(num_population[region].begin(), num_population[region].end(), 0.0) > 0) {
            ...
            for (auto i = AgeGroup(0); i < num_groups; i++) {
                ...
                double S    = num_population[region][size_t(i)] * immunity_population[size_t(i)][0];
                double S_pi = num_population[region][size_t(i)] * immunity_population[size_t(i)][1];
                double S_fi = num_population[region][size_t(i)] - S - S_pi;

                double denom_E =
                    1 / (S + S_pi * model[region].parameters.template get<ExposedFactorPartialImmunity>()[i] +
                         S_fi * model[region].parameters.template get<ExposedFactorImprovedImmunity>()[i]);
                double denom_C = 1 / (S + S_pi + S_fi);
                double denom_I =
                    1 / (S + S_pi * model[region].parameters.template get<InfectedFactorPartialImmunity>()[i] +
                         S_fi * model[region].parameters.template get<InfectedFactorImprovedImmunity>()[i]);
                double denom_HU =
                    1 / (S + S_pi * model[region].parameters.template get<HospitalizedFactorPartialImmunity>()[i] +
                         S_fi * model[region].parameters.template get<HospitalizedFactorImprovedImmunity>()[i]);
                ...
                model[region].populations[{i, InfectionState::ExposedPartialImmunity}] =
                    S_pi * model[region].parameters.template get<ExposedFactorPartialImmunity>()[i] *
                ...
                model[region].populations[{i, InfectionState::CarrierPartialImmunity}] =
                    S_pi * model[region].populations[{i, InfectionState::CarrierPartialImmunity}] * denom_C;
                model[region].populations[{i, InfectionState::CarrierImprovedImmunity}] =
                    S_fi * model[region].populations[{i, InfectionState::CarrierImprovedImmunity}] * denom_C;
                ...
                model[region].populations[{i, InfectionState::InfectedPartialImmunity}] =
                    S_pi * model[region].parameters.template get<InfectedFactorPartialImmunity>()[i] *
                ...
                model[region].populations[{i, InfectionState::HospitalizedPartialImmunity}] =
                    S_pi * model[region].parameters.template get<HospitalizedFactorPartialImmunity>()[i] *
                ...
                model[region].populations[{i, InfectionState::TemporaryImmunity1}] =
                    std::min(double(model[region].populations[{i, InfectionState::TemporaryImmunity1}]),
                             double(S_pi - model[region].populations[{i, InfectionState::ExposedPartialImmunity}] -
                                    model[region].populations[{i, InfectionState::CarrierPartialImmunity}] -
                                    model[region].populations[{i, InfectionState::InfectedPartialImmunity}] -
                                    model[region].populations[{i, InfectionState::HospitalizedPartialImmunity}] -
                                    model[region].populations[{i, InfectionState::ICUPartialImmunity}]));
                ...
                model[region].populations[{i, InfectionState::TemporaryImmunity2}] =
                    std::min(double(model[region].populations[{i, InfectionState::TemporaryImmunity2}]),
                             double(S_fi - model[region].populations[{i, InfectionState::ExposedImprovedImmunity}] -
                                    model[region].populations[{i, InfectionState::CarrierImprovedImmunity}] -
                                    model[region].populations[{i, InfectionState::InfectedImprovedImmunity}] -
                                    model[region].populations[{i, InfectionState::HospitalizedImprovedImmunity}] -
                                    model[region].populations[{i, InfectionState::ICUImprovedImmunity}] -
                ...
                model[region].populations[{i, InfectionState::Recovered}] =
                    ...
                    S_fi - (model[region].populations[{i, InfectionState::CarrierImprovedImmunity}] +
                            model[region].populations[{i, InfectionState::InfectedImprovedImmunity}] +
                            model[region].populations[{i, InfectionState::HospitalizedImprovedImmunity}] +
                            model[region].populations[{i, InfectionState::ICUImprovedImmunity}] +
                            model[region].populations[{i, InfectionState::Dead}] +
                ...
                model[region].populations[{i, InfectionState::Recovered}] = std::min(
                    S + S_pi + S_fi, std::max(0.0, double(model[region].populations[{i, InfectionState::Recovered}])));

                model[region].populations[{i, InfectionState::SusceptiblePartialImmunity}] =
                    std::max(0.0, S_pi - model[region].populations[{i, InfectionState::ExposedPartialImmunity}] -
                                      model[region].populations[{i, InfectionState::CarrierPartialImmunity}] -
                                      model[region].populations[{i, InfectionState::InfectedPartialImmunity}] -
                                      model[region].populations[{i, InfectionState::HospitalizedPartialImmunity}] -
                                      model[region].populations[{i, InfectionState::ICUPartialImmunity}] -
                ...
                // std::cout << model[region].populations[{i, InfectionState::TemporaryImmunity1}] << " und "
                //           << model[region].populations[{i, InfectionState::TemporaryImmunity2}] << std::endl;

                // for (auto j = Index<InfectionState>(0); j < InfectionState::Count; ++j) {
                //     std::cout << "Compartment " << size_t(j) << "hat Wert " << model[region].populations[{i, j}]
                //               << std::endl;
                // }
                                                   ...
                                                   double scaling_factor_icu, int num_days)
{
    ...
    for (size_t day = 0; day <= static_cast<size_t>(num_days); day++) {
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            ...
            num_rec_uv, num_timm1, t_car_to_rec_uv, t_car_to_inf_uv, t_exp_to_car_uv, t_inf_to_rec_uv, t_inf_to_hosp_uv,
            t_hosp_to_rec_uv, t_hosp_to_icu_uv, t_icu_to_dead_uv, t_icu_to_rec_uv, t_imm_interval1, mu_C_R_uv,
            mu_I_H_uv, mu_H_U_uv, mu_U_D_uv, scaling_factor_inf));
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            ...
            num_timm2, t_car_to_rec_pv, t_car_to_inf_pv, t_exp_to_car_pv, t_inf_to_rec_pv, t_inf_to_hosp_pv,
            t_hosp_to_rec_pv, t_hosp_to_icu_pv, t_icu_to_dead_pv, t_icu_to_rec_pv, t_imm_interval2, mu_C_R_pv,
            mu_I_H_pv, mu_H_U_pv, mu_U_D_pv, scaling_factor_inf));
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            ...
            num_timm2, t_car_to_rec_fv, t_car_to_inf_fv, t_exp_to_car_fv, t_inf_to_rec_fv, t_inf_to_hosp_fv,
            t_hosp_to_rec_fv, t_hosp_to_icu_fv, t_icu_to_dead_fv, t_icu_to_rec_fv, t_imm_interval2, mu_C_R_fv,
            mu_I_H_fv, mu_H_U_fv, mu_U_D_fv, scaling_factor_inf));
        ...
        for (size_t county = 0; county < region.size(); county++) {
            if (std::accumulate(num_population[county].begin(), num_population[county].end(), 0.0) > 0) {
                for (size_t age = 0; age < num_age_groups; age++) {
                    ...
                    double S_pv = std::max(
                        model[county].parameters.template get<DailyPartialVaccination>()[{AgeGroup(age),
                                                                                          SimulationDay(day)}] -
                            model[county]
                                .parameters.template get<DailyFullVaccination>()[{AgeGroup(age), SimulationDay(day)}],
                    ...
                    extrapolated_rki[county][day]((size_t)InfectionState::CarrierPartialImmunity + age_group_offset) =
                    ...
                    extrapolated_rki[county][day]((size_t)InfectionState::InfectedPartialImmunity + age_group_offset) =
                        S_pv * model[county].parameters.template get<InfectedFactorPartialImmunity>()[AgeGroup(age)] *
                    ...
                    extrapolated_rki[county][day]((size_t)InfectionState::SusceptiblePartialImmunity +
                        ...
                        std::max(0.0, S_pv -
                                          ...
                                          extrapolated_rki[county][day](
                                              (size_t)InfectionState::InfectedPartialImmunity + age_group_offset) -
                                          extrapolated_rki[county][day](
...
IOResult<void> read_input_data_county(std::vector<Model>& model, Date date, const std::vector<int>& county,
                                      ...
                                      const std::string& dir, int num_days, int vacc_campaign_szenario,
                                      bool export_time_series = false)
{
    ...
    std::string path_cases = path_join(dir, "cases_all_county_age_ma7.json");
    BOOST_OUTCOME_TRY(details::set_confirmed_cases_data(model, path_cases, county, date, scaling_factor_inf));

                                        ...
                                        const std::vector<double>& scaling_factor_inf)
{
    ...
    BOOST_OUTCOME_TRY(read_confirmed_cases_data(
        rki_data, region, date, num_Exposed, num_InfectedNoSymptoms, num_InfectedSymptoms, num_InfectedSevere, num_icu, num_death, num_rec, 
    ...
    for (size_t county = 0; county < model.size(); county++) {
        ...
        for (size_t group = 0; group < num_age_groups; group++) {
            ...
            t_InfectedNoSymptoms[county].push_back(
                static_cast<int>(reduc_t * 2 * (model[county].parameters.template get<IncubationTime>()[(AgeGroup)group] -
                                      model[county].parameters.template get<SerialInterval>()[(AgeGroup)group])));
            t_Exposed[county].push_back(
                static_cast<int>(2 * model[county].parameters.template get<SerialInterval>()[(AgeGroup)group] -
                                 model[county].parameters.template get<IncubationTime>()[(AgeGroup)group]));
    ...
    for (size_t region = 0; region < vregion.size(); region++) {
        if (std::accumulate(num_population[region].begin(), num_population[region].end(), 0.0) > 0) {
            ...
            for (auto i = AgeGroup(0); i < num_groups; i++) {
                ...
                double denom_HU =
                    1 / (S + S_pv * model[region].parameters.template get<ReducInfectedSevereCriticalDeadPartialImmunity>()[i] +
                                                   ...
                                                   double scaling_factor_icu, int num_days)
{
    ...
    for (size_t day = 0; day <= static_cast<size_t>(num_days); day++) {
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            rki_data, region, date, num_Exposed_uv, num_InfectedNoSymptoms_uv, num_InfectedSymptoms_uv, num_InfectedSevere_uv, dummy_icu, num_death_uv,
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            rki_data, region, date, num_Exposed_pv, num_InfectedNoSymptoms_pv, num_InfectedSymptoms_pv, num_InfectedSevere_pv, dummy_icu, dummy_death, dummy_rec,
        ...
        BOOST_OUTCOME_TRY(details::read_confirmed_cases_data(
            rki_data, region, date, num_Exposed_fv, num_InfectedNoSymptoms_fv, num_InfectedSymptoms_fv, num_InfectedSevere_fv, dummy_icu, dummy_death, dummy_rec,
        ...
        for (size_t county = 0; county < region.size(); county++) {
            if (std::accumulate(num_population[county].begin(), num_population[county].end(), 0.0) > 0) {
                for (size_t age = 0; age < num_age_groups; age++) {
                    ...
                    double denom_C = 
                        1 / (S + S_pv * model[county].parameters.template get<ReducExposedPartialImmunity>()[AgeGroup(age)] +
                            S_v * model[county].parameters.template get<ReducExposedImprovedImmunity>()[AgeGroup(age)]);
                    double denom_I =
                        1 /
                        (S +
                    ...
                    double denom_HU =
                        ...
                        (S +
                         S_pv *
                             model[county].parameters.template get<ReducInfectedSevereCriticalDeadPartialImmunity>()[AgeGroup(age)] +
                         S_v * model[county]

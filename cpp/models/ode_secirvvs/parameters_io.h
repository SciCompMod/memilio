                                        ...
                                        const std::vector<double>& scaling_factor_inf, bool set_death = false)
{
    ...
    for (size_t region_idx = 0; region_idx < vregion.size(); ++region_idx) {
        BOOST_OUTCOME_TRY(set_confirmed_cases_data(model[region_idx], vcase_data[region_idx], vregion[region_idx], date, scaling_factor_inf, set_death));
    }
    return success();
}
...
*/
template <class FP>
IOResult<void> set_population_data(mio::VectorRange<Model<FP>>& model, const std::string& path, const std::string& path_rki,
                                   const std::vector<int>& vregion, Date date)
{
    ...
    for (size_t region_idx = 0; region_idx < vregion.size(); ++region_idx) {
        BOOST_OUTCOME_TRY(set_population_data(model[region_idx], vnum_population[region_idx], vregion[region_idx], vnum_rec[region_idx]));
    }
    return success();
}

                                 ...
                                 ScalarType scale_confirmed_cases)
{
    ...
    const int num_age_groups = ConfirmedCasesDataEntry::age_group_names.size();
        ...
        for (auto&& entry : rki_data) {
            ...
            if ((offset >= min_offset_needed) && (offset <= max_offset_needed)) {
                ...
                if ((offset >= min_offset_needed) && (offset <= max_offset_needed)) {
                    ...
                    // Biggest index for which the entry is needed.
                    idx_needed_last =
                        Eigen::Index(std::min(std::ceil((offset - model.m_transitions.get_time(0) + 1) / dt),
                                              double(model.m_transitions.get_num_time_points() - 1)));

                    int INStISi = model.get_transition_flat_index(
                    ...
                    for (Eigen::Index i = idx_needed_first; i <= idx_needed_last; i++) {
                        ...
                        if (offset == int(std::floor(time_idx - dt))) {
                            model.m_transitions[i][INStISi] -= (1 - (time_idx - dt - std::floor(time_idx - dt))) *
                                                               scale_confirmed_cases * entry.num_confirmed;
                        }
                        if (offset == int(std::ceil(time_idx - dt))) {
                            model.m_transitions[i][INStISi] -= (time_idx - dt - std::floor(time_idx - dt)) *
                                                               scale_confirmed_cases * entry.num_confirmed;
                        }
                    }
                    ...
                    if (offset ==
                        ...
                        model.m_populations[0][Di] +=
                            (1 - (-mean_InfectedSymptomsToInfectedSevere[group] -
                                  mean_InfectedSevereToInfectedCritical[group] - mean_InfectedCriticalToDead[group] -
                                  std::floor(-mean_InfectedSymptomsToInfectedSevere[group] -
                                             mean_InfectedSevereToInfectedCritical[group] -
                                             mean_InfectedCriticalToDead[group]))) *
                    ...
                    if (offset ==
                        std::ceil(-mean_InfectedSymptomsToInfectedSevere[group] -
                                  mean_InfectedSevereToInfectedCritical[group] - mean_InfectedCriticalToDead[group])) {
                        model.m_populations[0][Di] +=
                            (-mean_InfectedSymptomsToInfectedSevere[group] -
                             mean_InfectedSevereToInfectedCritical[group] - mean_InfectedCriticalToDead[group] -
                ...
                if (!max_offset_needed_avail) {
                    ...
                    return failure(StatusCode::OutOfRange,
                                   path + ", necessary range of dates does not exist in RKI data.");
                }
                if (!min_offset_needed_avail) {
                    std::string min_date_string = std::to_string(min_date.day) + "." + std::to_string(min_date.month) +
                                                  "." + std::to_string(min_date.year);
                ...
                Eigen::Index index_shift_mean =
                    Eigen::Index(std::round(mean_InfectedNoSymptomsToInfectedSymptoms[group] / dt));
                int EtINSi = model.get_transition_flat_index(
                ...
                index_shift_mean = Eigen::Index(std::round(
                    (mean_ExposedToInfectedNoSymptoms[group] + mean_InfectedNoSymptomsToInfectedSymptoms[group]) / dt));
                int StEi =
                    model.get_transition_flat_index(Eigen::Index(InfectionTransition::SusceptibleToExposed), group);

...
private:
                              ...
                              Eigen::Ref<Eigen::VectorX<FP>> dydt) const
    {
        ...
        // To split the outflow correctly, we need the proportion of people infected with each disease which only depends
        // on the disease-specific parameters (not on e.g. the contact patterns or the seasonality), calculated in the interact function
                  ...
                  Eigen::Ref<Eigen::VectorX<FP>> dydt, double* part_a, double* part_b, int relevant_disease) const
    {
        ...
        FP contact_patterns = params.template get<ContactPatterns<FP>>().get_cont_freq_mat().get_matrix_at(
        ...
        // the proportion of people getting infected with disease a and b.
        *part_a += infection_a_per_person;
        *part_b += infection_b_per_person;

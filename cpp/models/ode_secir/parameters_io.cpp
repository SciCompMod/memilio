    ...
    const double scaling_factor_inf)
{
    ...
    for (size_t region_idx = 0; region_idx < vregion.size(); ++region_idx) {
        ...
        auto try_fix_constraints = [region](double& value, double error, auto str) {
            if (value < error) {
                //this should probably return a failure
                //but the algorithm is not robust enough to avoid large negative values and there are tests that rely on it
                log_error("{:s} is {:.4f} for region {:d}, exceeds expected negative value.", str, value, region);
                value = 0.0;
            }
            else if (value < 0) {
                log_info("{:s} is {:.4f} for region {:d}, automatically corrected", str, value, region);
                value = 0.0;
            }
        };

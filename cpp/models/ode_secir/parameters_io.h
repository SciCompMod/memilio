...
*/
template <class Model>
IOResult<void> export_input_data_county_timeseries_one_age_group(
    ...
    const std::string& divi_data_path, const std::string& confirmed_cases_path, const std::string& population_data_path)
{
    ...
    for (size_t county = 0; county < model.size(); county++) {
        for (size_t group = 0; group < ConfirmedCasesDataEntry::age_group_names.size(); group++) {
            ...
            t_InfectedNoSymptoms[county].push_back(static_cast<int>(
                std::round(2 * (model[county].parameters.template get<IncubationTime>()[AgeGroup(0)] -
                                model[county].parameters.template get<SerialInterval>()[AgeGroup(0)]))));
            t_Exposed[county].push_back(
                static_cast<int>(std::round(2 * model[county].parameters.template get<SerialInterval>()[AgeGroup(0)] -
                                            model[county].parameters.template get<IncubationTime>()[AgeGroup(0)])));
            t_InfectedSymptoms[county].push_back(static_cast<int>(
                std::round(model[county].parameters.template get<TimeInfectedSymptoms>()[AgeGroup(0)])));
            t_InfectedSevere[county].push_back(
                static_cast<int>(std::round(model[county].parameters.template get<TimeInfectedSevere>()[AgeGroup(0)])));
            t_InfectedCritical[county].push_back(static_cast<int>(
            ...
            mu_C_R[county].push_back(
                model[county].parameters.template get<RecoveredPerInfectedNoSymptoms>()[AgeGroup(0)]);
            mu_I_H[county].push_back(model[county].parameters.template get<SeverePerInfectedSymptoms>()[AgeGroup(0)]);
            mu_H_U[county].push_back(model[county].parameters.template get<CriticalPerSevere>()[AgeGroup(0)]);
            mu_U_D[county].push_back(model[county].parameters.template get<DeathsPerCritical>()[(AgeGroup)0]);

            sum_mu_I_U[county] += model[county].parameters.template get<CriticalPerSevere>()[AgeGroup(0)] *
                                  model[county].parameters.template get<SeverePerInfectedSymptoms>()[AgeGroup(0)];
            mu_I_U[county].push_back(model[county].parameters.template get<CriticalPerSevere>()[AgeGroup(0)] *
                                     model[county].parameters.template get<SeverePerInfectedSymptoms>()[AgeGroup(0)]);
        }
    }

    std::vector<TimeSeries<double>> rki_data(region.size(),
                                             TimeSeries<double>::zero(num_days, (size_t)InfectionState::Count));
    ...
    for (size_t t = 0; t < static_cast<size_t>(num_days); ++t) {
        ...
        for (size_t i = 0; i < region.size(); i++) {
            rki_data[i][t]((size_t)InfectionState::Exposed) =
                std::accumulate(num_Exposed[i].begin(), num_Exposed[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedNoSymptoms) =
                std::accumulate(num_InfectedNoSymptoms[i].begin(), num_InfectedNoSymptoms[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedNoSymptomsConfirmed) = std::accumulate(
                num_InfectedNoSymptomsConfirmed[i].begin(), num_InfectedNoSymptomsConfirmed[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedSymptoms) =
                std::accumulate(num_InfectedSymptoms[i].begin(), num_InfectedSymptoms[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedSymptomsConfirmed) =
                std::accumulate(num_InfectedSymptomsConfirmed[i].begin(), num_InfectedSymptomsConfirmed[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedSevere) =
                std::accumulate(num_InfectedSevere[i].begin(), num_InfectedSevere[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::InfectedCritical) = scaling_factor_icu * num_icu[i];
            rki_data[i][t]((size_t)InfectionState::Recovered) =
                std::accumulate(num_rec[i].begin(), num_rec[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::Dead) =
                std::accumulate(num_death[i].begin(), num_death[i].end(), 0.0);
            rki_data[i][t]((size_t)InfectionState::Susceptible) =
                std::accumulate(num_population[i].begin(), num_population[i].end(), 0.0) -
                std::accumulate(num_Exposed[i].begin(), num_Exposed[i].end(), 0.0) -
                std::accumulate(num_InfectedNoSymptoms[i].begin(), num_InfectedNoSymptoms[i].end(), 0.0) -
                std::accumulate(num_InfectedNoSymptomsConfirmed[i].begin(), num_InfectedNoSymptomsConfirmed[i].end(),
                                0.0) -
                std::accumulate(num_InfectedSymptoms[i].begin(), num_InfectedSymptoms[i].end(), 0.0) -
                std::accumulate(num_InfectedSymptomsConfirmed[i].begin(), num_InfectedSymptomsConfirmed[i].end(), 0.0) -
                std::accumulate(num_InfectedSevere[i].begin(), num_InfectedSevere[i].end(), 0.0) -
                std::accumulate(num_rec[i].begin(), num_rec[i].end(), 0.0) -
                std::accumulate(num_death[i].begin(), num_death[i].end(), 0.0) -
                rki_data[i][t]((size_t)InfectionState::InfectedCritical);
        }
        date = offset_date_by_days(date, 1);
    }
    auto num_groups = (int)(size_t)model[0].parameters.get_num_groups();
 ...
 */
template <class Model>
IOResult<void> read_input_data_one_age_group(std::vector<Model>& model, Date date, const std::vector<int>& node_ids,
                                             const std::vector<double>& scaling_factor_inf, double scaling_factor_icu,
                                             ...
                                             bool export_time_series = false)
{
    ...
    BOOST_OUTCOME_TRY(details::set_confirmed_cases_data_one_age_group(
        model, path_join(data_dir, "confirmed_cases.json"), node_ids, date, scaling_factor_inf));
    BOOST_OUTCOME_TRY(details::set_population_data(model, path_join(data_dir, "population_data.json"), node_ids, true));
    ...
    if (export_time_series) {
        ...
        log_warning("Exporting time series of extrapolated real data. This may take some minutes. "
                    "For simulation runs over the same time period, deactivate it.");
        BOOST_OUTCOME_TRY(export_input_data_county_timeseries_one_age_group(
            model, data_dir, node_ids, date, scaling_factor_inf, scaling_factor_icu, num_days,
            path_join(data_dir, "critical_cases.json"), path_join(data_dir, "confirmed_cases.json"),
            path_join(data_dir, "population_data.json")));
    }

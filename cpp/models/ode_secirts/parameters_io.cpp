    ...
    const std::vector<ScalarType>& scaling_factor_inf, const size_t layer)
{
    ...
    for (auto&& entry : case_data) {
        ...
        if (entry.date ==
            offset_date_by_days(date, -timeInfectedSymptoms - t_InfectedSevere[age] - t_InfectedCritical[age])) {
                                        ...
                                        const std::vector<std::vector<ScalarType>>& immunity_population)
{
    ...
    for (size_t group = 0; group < num_age_groups; group++) {
        ...
        reduc_t_Infected.push_back(model.parameters.template get<ReducTimeInfectedMild<ScalarType>>()[(AgeGroup)group]);
    ...
    for (size_t i = 0; i < num_groups; i++) {
        ...
        model.populations[{AgeGroup(i), InfectionState::InfectedSeverePartialImmunity}] =
            immunity_population[1][i] *
            model.parameters.template get<ReducInfectedSevereCriticalDeadPartialImmunity<ScalarType>>()[(AgeGroup)i] *
        ...
        if (!is_divi_data_available(date)) {
            model.populations[{AgeGroup(i), InfectionState::InfectedCriticalPartialImmunity}] =
                immunity_population[1][i] *
                model.parameters
                    .template get<ReducInfectedSevereCriticalDeadPartialImmunity<ScalarType>>()[(AgeGroup)i] *
    ...
    for (size_t i = 0; i < num_groups; i++) {
        ...
        model.populations[{AgeGroup(i), InfectionState::InfectedSevereImprovedImmunity}] =
            immunity_population[2][i] *
            model.parameters.template get<ReducInfectedSevereCriticalDeadImprovedImmunity<ScalarType>>()[(AgeGroup)i] *
            denom_I_Sev_Cr[i] * num_InfectedSevere[i];
        // Only set the number of ICU patients here, if the date is not available in the data.
        if (!is_divi_data_available(date)) {
            model.populations[{AgeGroup(i), InfectionState::InfectedCriticalImprovedImmunity}] =
                immunity_population[2][i] *
                model.parameters
                    .template get<ReducInfectedSevereCriticalDeadImprovedImmunity<ScalarType>>()[(AgeGroup)i] *
                denom_I_Sev_Cr[i] * num_icu[i];
        }
                                   ...
                                   const int region, const std::vector<std::vector<ScalarType>>& immunity_population)
{
    ...
    for (auto i = AgeGroup(0); i < num_groups; i++) {
        ...
        model.populations[{i, InfectionState::SusceptibleImprovedImmunity}] =
            std::max(0.0, SII - (model.populations[{i, InfectionState::ExposedImprovedImmunity}] +
        ...
        model.populations[{i, InfectionState::SusceptiblePartialImmunity}] =
            std::max(0.0, SPI - model.populations[{i, InfectionState::ExposedPartialImmunity}] -
                                   ...
                                   const std::vector<std::vector<ScalarType>>& immunity_population)
{
    ...
    for (size_t region_idx = 0; region_idx < model.size(); ++region_idx) {
        BOOST_OUTCOME_TRY(set_population_data(model[region_idx].property, num_population[region_idx],
                                              model[region_idx].id, immunity_population));
    }
    return success();
}
                                            ...
                                            const mio::regions::de::EpidataFilenames& epidata_filenames)
{
    ...
    for (int t = 0; t <= num_days; ++t) {
        ...
        BOOST_OUTCOME_TRY(read_input_data(model, offset_day, scaling_factor_inf, scaling_factor_icu, num_days,
                                          immunity_population, epidata_filenames));

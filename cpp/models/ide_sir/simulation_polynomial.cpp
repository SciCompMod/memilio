...
void SimulationPolynomial::advance_single(ScalarType tmax)
{
    ...
    while (m_model->populations.get_last_time() < tmax - 1e-10) {
        ...
        m_model->approx_single_integral(m_dt, t0_index);
    }
}

void SimulationPolynomial::advance_double(ScalarType tmax)
{
    // Get index of t0, i.e. index of last time point of given initial values.
    size_t t0_index = m_model->populations.get_num_time_points() - 1;
    ...
    while (m_model->populations.get_last_time() < tmax - 1e-10) {
        ...
        // Compute Susceptibles.
        m_model->approx_double_integral(m_dt, t0_index);
    }
}

...
#ifndef IDE_SIR_SIMULATION_POLY_H
#define IDE_SIR_SIMULATION_POLY_H
...
public:
     ...
     */
    SimulationPolynomial(ModelPolynomial const& model, ScalarType dt)
        : m_model(std::make_unique<ModelPolynomial>(model))
        , m_dt(dt)
    {
        assert(m_dt > 0);
    }
     ...
     */
    void advance_single(ScalarType tmax);

    void advance_double(ScalarType tmax);
     ...
     */
    const ModelPolynomial& get_model() const
    {
        return *m_model;
    }
...
private:
    std::unique_ptr<ModelPolynomial> m_model; ///< Unique pointer to the Model simulated.

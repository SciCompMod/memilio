...
#ifndef IDE_SIR_SIMULATION_POLY_EXT_H
#define IDE_SIR_SIMULATION_POLY_EXT_H
...
class SimulationPolynomialExtended
{
...
public:
     ...
     */
    SimulationPolynomialExtended(ModelPolynomialExtended const& model, ScalarType dt)
        : m_model(std::make_unique<ModelPolynomialExtended>(model))
        , m_dt(dt)
    {
        assert(m_dt > 0);
    }
     ...
     */
    void advance_single(ScalarType tmax, ScalarType alpha);

    void advance_double(ScalarType tmax, ScalarType alpha);
     ...
     */
    const ModelPolynomialExtended& get_model() const
    {
        return *m_model;
    }
     ...
     * @brief returns the simulation model used in simulation.
     */
    ModelPolynomialExtended& get_model()
    {
        return *m_model;
    }
...
private:
    std::unique_ptr<ModelPolynomialExtended> m_model; ///< Unique pointer to the Model simulated.

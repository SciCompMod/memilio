...
#ifndef IDE_SIR_SIMULATION_SMOOTHERCOS_H
#define IDE_SIR_SIMULATION_SMOOTHERCOS_H
...
public:
     ...
     */
    SimulationSmootherCos(ModelSmootherCos const& model, ScalarType dt)
        : m_model(std::make_unique<ModelSmootherCos>(model))
        , m_dt(dt)
    {
        assert(m_dt > 0);
    }
     ...
     */
    const ModelSmootherCos& get_model() const
    {
        return *m_model;
    }
...
private:
    std::unique_ptr<ModelSmootherCos> m_model; ///< Unique pointer to the Model simulated.

...
*/
#ifndef SEIRS_MODEL_H
#define SEIRS_MODEL_H
...
using Flows = TypeList<Flow<InfectionState::Susceptible, InfectionState::Exposed>,
                       Flow<InfectionState::Susceptible, InfectionState::ExposedIsolated>,
                       Flow<InfectionState::Exposed,     InfectionState::Infected>,
                       Flow<InfectionState::Exposed,     InfectionState::ExposedIsolated>,
                       Flow<InfectionState::ExposedIsolated, InfectionState::Infectedisolated>,
                       Flow<InfectionState::Infected,    InfectionState::Recovered>,
                       Flow<InfectionState::Infected,    InfectionState::Infectedisolated>,
                       Flow<InfectionState::Infectedisolated, InfectionState::Recovered>,
...
public:
                   ...
                   Eigen::Ref<Eigen::VectorX<FP>> flows) const override
    {
        ...
        for (auto i : make_index_range(age_groups)) {
            // flow to infected in unit time
            test_and_trace_required += (1.0 / params.template get<TimeExposed<FP>>()[i]) *
                                       y[this->populations.get_flat_index({i, InfectionState::Exposed})];
        }
        ...
        for (auto i : make_index_range(age_groups)) {
            ...
            const size_t EIi = this->populations.get_flat_index({i, InfectionState::ExposedIsolated});
            const size_t Ii  = this->populations.get_flat_index({i, InfectionState::Infected});
            const size_t IIi = this->populations.get_flat_index({i, InfectionState::Infectedisolated});
            ...
            for (auto j : make_index_range(age_groups)) {
                ...
                const size_t EIj = this->populations.get_flat_index({j, InfectionState::ExposedIsolated});
                const size_t Ij  = this->populations.get_flat_index({j, InfectionState::Infected});
                const size_t IIj = this->populations.get_flat_index({j, InfectionState::Infectedisolated});
                const size_t Rj  = this->populations.get_flat_index({j, InfectionState::Recovered});

                const ScalarType Nj    = pop[Sj] + pop[Ej] + pop[Ij] + pop[Rj] + pop[EIj] + pop[IIj];
                ...
                // hidden contagion
                const auto flow_to_E = coeffStoE * y[Si] * pop[Ij];

                flows[Base::template get_flat_flow_index<InfectionState::Susceptible, InfectionState::Exposed>(i)] +=
                    flow_to_E * (1.0 - params.template get<ShareContagionsIsolated<FP>>()[i]);
                flows[Base::template get_flat_flow_index<InfectionState::Susceptible, InfectionState::ExposedIsolated>(
                    i)] += flow_to_E * params.template get<ShareContagionsIsolated<FP>>()[i];
            }
            // exposed to infected
            ...
            // exposed isolated to infected isolated
            flows[Base::template get_flat_flow_index<InfectionState::ExposedIsolated, InfectionState::Infectedisolated>(
                i)] = (1.0 / params.template get<TimeExposed<FP>>()[i]) * y[EIi];

            const auto tnt_capacity_utilization =
            ...
            // infected isolated to recovered
            flows[Base::template get_flat_flow_index<InfectionState::Infectedisolated, InfectionState::Recovered>(i)] =
                (1.0 / params.template get<TimeInfected<FP>>()[i]) * y[IIi];

            // waning immunity
            flows[Base::template get_flat_flow_index<InfectionState::Recovered, InfectionState::Susceptible>(i)] =
                (1.0 / params.template get<TimeImmunity<FP>>()[i]) * y[Ri];
        }
    }
    ...
    */
    IOResult<ScalarType> get_reproduction_number(size_t t_idx, const mio::TimeSeries<ScalarType>& y)
    {
        ...
        for (auto i = AgeGroup(0); i < AgeGroup(num_groups); i++) {
            ...
            ScalarType T_Ei                      = params.template get<mio::oseirs::TimeExposed<ScalarType>>()[i];
            ScalarType T_Ii                      = params.template get<mio::oseirs::TimeInfected<ScalarType>>()[i];

        ...
        for (auto age_i = AgeGroup(0); age_i < n_agegroups; age_i++) {
            for (auto age_j = AgeGroup(0); age_j < n_agegroups; age_j++) {
                ...
                for (auto region = Region(0); region < n_regions; region++) {
                    flows[get_flat_flow_index<InfectionState::Susceptible, InfectionState::Exposed>({region, age_i})] +=
                        pop[population.get_flat_index({region, age_j, InfectionState::Infected})];
                    flows[get_flat_flow_index<InfectionState::Susceptible, InfectionState::Exposed>({region, age_i})] *=
            ...
            for (auto region = Region(0); region < n_regions; region++) {
                flows[get_flat_flow_index<InfectionState::Exposed, InfectionState::Infected>({region, age_i})] =
                    (1.0 / params.get<TimeExposed>()) *
                ...
                // flows[get_flat_flow_index<InfectionState::Exposed, InfectionState::Infected>({region, age_i})] = 0.;
                // flows[get_flat_flow_index<InfectionState::Exposed, InfectionState::Infected>({region, age_i})] = 0.;
            }
        }
    }

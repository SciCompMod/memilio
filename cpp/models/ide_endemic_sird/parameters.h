#ifndef IDE_END_SIRD_PARAMS_H
#define IDE_END_SIRD_PARAMS_H
...
/**************************************************
* Define Parameters of the IDE-END-SIRD model *
**************************************************/
...
struct TransitionProbabilities {
    ...
    static Type get_default()
    {
        ...
        probs[Eigen::Index(InfectionTransition::SusceptibleToInfected)] = 1;
        return Type(probs);
    }
...
using ParametersBase =
    ParameterSet<TransitionDistributions, TransitionProbabilities, ContactPatterns, TransmissionProbabilityOnContact,
                 RiskOfInfectionFromSymptomatic, NaturalBirthRate, NaturalDeathRate>;
...
public:
    ...
    bool check_constraints() const
    {
        ...
        // The TransitionProbabilitie SusceptibleToInfected should be 1.
        if (!floating_point_equal(
                this->get<TransitionProbabilities>()[static_cast<int>(InfectionTransition::SusceptibleToInfected)], 1.0,
                1e-14)) {
        ...
        if (!floating_point_equal(
                this->get<TransitionProbabilities>()[static_cast<int>(InfectionTransition::InfectedToDead)] +
                    this->get<TransitionProbabilities>()[static_cast<int>(InfectionTransition::InfectedToRecovered)],
                1.0, 1e-14)) {
            log_error("Constraint check: Sum of transition probability for InfectedToDead and "
                      "InfectedToRecovered not equal to {:d}",
                      1);
            return true;
        }

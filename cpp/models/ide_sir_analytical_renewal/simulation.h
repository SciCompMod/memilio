...
class SimulationAnalyticalRenewal
{
...
public:
     ...
     */
    SimulationAnalyticalRenewal(ModelAnalyticalRenewal const& model, ScalarType dt)
        : m_model(std::make_unique<ModelAnalyticalRenewal>(model))
        , m_dt(dt)
    {
        assert(m_dt > 0);
    }
     ...
     */
    const ModelAnalyticalRenewal& get_model() const
    {
        return *m_model;
    }
     ...
     * @brief returns the simulation model used in simulation.
     */
    ModelAnalyticalRenewal& get_model()
    {
        return *m_model;
    }
...
private:
    std::unique_ptr<ModelAnalyticalRenewal> m_model; ///< Unique pointer to the Model simulated.

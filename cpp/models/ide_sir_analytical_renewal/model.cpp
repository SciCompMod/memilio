...
#include "ide_sir_analytical_renewal/parameters.h"
#include "ide_sir_analytical_renewal/infection_state.h"
#include "ide_sir_analytical_renewal/gregory_weights.h"
...
namespace isir
{
ModelAnalyticalRenewal::ModelAnalyticalRenewal(TimeSeries<ScalarType>&& populations_init, size_t gregory_order,
...
void ModelAnalyticalRenewal::set_transitiondistribution_vector(ScalarType dt, ScalarType tmax)
{
    m_transitiondistribution_vector = std::vector<ScalarType>(size_t(std::ceil(tmax / dt)) + 1, 0.);

    for (size_t i = 0; i <= size_t(std::ceil(tmax / dt)); i++) {
...
ScalarType ModelAnalyticalRenewal::sum_part1_weight(size_t n, size_t j)
{
    assert(n > 0);
    assert(j < n);
...
ScalarType ModelAnalyticalRenewal::sum_part2_weight(size_t n, size_t j)
{
    assert(n > 0);
    assert(j <= n);
...
ScalarType ModelAnalyticalRenewal::fixed_point_function(ScalarType susceptibles, ScalarType dt)
{
    ...
    for (size_t j = 0; j <= current_time_index; j++) {
        ...
        sum += gregory_weight * m_transitiondistribution_vector[current_time_index - j] * relevant_susceptibles;
    }

    return parameters.get<TransmissionProbabilityOnContact>().eval(0.) *
               m_transitiondistribution_vector[current_time_index] +
           parameters.get<TransmissionProbabilityOnContact>().eval(0.) * dt * sum;
}

size_t ModelAnalyticalRenewal::compute_S(ScalarType s_init, ScalarType dt, ScalarType tol, size_t max_iterations)
{
...
void ModelAnalyticalRenewal::compute_S_deriv(ScalarType dt, size_t time_point_index)
{
...
void ModelAnalyticalRenewal::compute_S_deriv(ScalarType dt)
{
...
void ModelAnalyticalRenewal::compute_I_and_R(ScalarType dt, size_t time_point_index)
{
    ...
    for (size_t j = 0; j <= time_point_index; j++) {

        ScalarType gregory_weight;
        if (j < switch_weights_index) {
            gregory_weight = sum_part1_weight(time_point_index, j);
        }
        else {
            gregory_weight = sum_part2_weight(time_point_index, j);
        }
...
void ModelAnalyticalRenewal::compute_I_and_R(ScalarType dt)
{

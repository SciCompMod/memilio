...
#ifndef LCT_SIR_MODEL_H
#define LCT_SIR_MODEL_H
...
private:
                              ...
                              Eigen::Ref<Eigen::VectorX<FP>> dydt) const
    {
        ...
        size_t Ii_first_index = first_index_group + LctStateGroup::template get_first_index<InfectionState::Infected>();

        // Calculate derivative of the Susceptible compartment.
        interact<Group, 0>(pop, y, t, dydt);

        // Calculate derivative of the Infected compartment.
        dydt[Ii_first_index] = -dydt[first_index_group];
        for (size_t subcomp = 0; subcomp < LctStateGroup::template get_num_subcompartments<InfectionState::Infected>();
             subcomp++) {
            // Variable flow stores the value of the flow from one subcompartment to the next one.
            // Ii_first_index + subcomp is always the index of a (sub-)compartment of Exposed and Ii_first_index
            // + subcomp + 1 can also be the index of the first (sub-)compartment of Recovered.
            flow = (FP)LctStateGroup::template get_num_subcompartments<InfectionState::Infected>() *
                   (1 / params.template get<TimeInfected<FP>>()[Group]) * y[Ii_first_index + subcomp];
            // Subtract flow from dydt[Ei_first_index + subcomp] and add to next subcompartment.
            dydt[Ii_first_index + subcomp] -= flow;
            dydt[Ii_first_index + subcomp + 1] = flow;
        }
                  ...
                  Eigen::Ref<Eigen::VectorX<FP>> dydt) const
    {
        ...
        infected_2 =
            pop.segment(first_index_group2 + LctStateGroup2::template get_first_index<InfectionState::Infected>(),
                        LctStateGroup2::template get_num_subcompartments<InfectionState::Infected>())
                .sum();
        // Size of the Subpopulation Group2.
        FP N_2          = pop.segment(first_index_group2, LctStateGroup2::Count).sum();
        const FP divN_2 = (N_2 < Limits<FP>::zero_tolerance()) ? 0.0 : 1.0 / N_2;
        dydt[Si_1] += -y[Si_1] * divN_2 * params.template get<TransmissionProbabilityOnContact<FP>>()[Group1] *
                      ...
                      (params.template get<RiskOfInfectionFromSymptomatic<FP>>()[Group2] * infected_2);

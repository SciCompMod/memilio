...
public:
    ...
    */
    IOResult<FP> get_reproduction_number(size_t t_idx, const mio::TimeSeries<FP>& y)
    {
        ...
        constexpr size_t num_infected_compartments = 3;
        ...
        for (auto i = AgeGroup(0); i < AgeGroup(num_groups); i++) {
            ...
            V((size_t)i + 2 * num_groups, (size_t)i + num_groups) = -prob_die / T_Ii;
            V((size_t)i + 2 * num_groups, (size_t)i + 2 * num_groups) = 1.0 / T_Bi;
        }

    ...
    */
    ...
    bool check_constraints() const
    {
        for (auto i = AgeGroup(0); i < AgeGroup(m_num_groups); ++i) {
            ...
            if (FP(this->template get<IncubationPeriod<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
                log_error("Constraint check: Parameter IncubationPeriod of age group {:.0f} smaller than {:.4f}",
                          (size_t)i, 0);
                return true;
            }

            if (FP(this->template get<InfectedNoSymptomsToSymptoms<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
            ...
            if (FP(this->template get<InfectedNoSymptomsToRecovered<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
            ...
            if (FP(this->template get<InfectedSymptomsToRecovered<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
                log_error(
            ...
            if (FP(this->template get<InfectedSymptomsToSevere<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
                log_error("Constraint check: Parameter InfectedSymptomsToSevere of age group {:.0f} smaller than {:d}",
                          (size_t)i, 0);
                return true;
            }

            if (FP(this->template get<SevereToCritical<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
                log_error("Constraint check: Parameter SevereToCritical of age group {:.0f} smaller than {:d}",
                          (size_t)i, 0);
                return true;
            }

            if (FP(this->template get<SevereToRecovered<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0) {
            ...
            if (FP(this->template get<DetectInfection<FP>>()[{VirusVariant::Wildtype, i}]) < 0.0 ||
                FP(this->template get<DetectInfection<FP>>()[{VirusVariant::Wildtype, i}]) > 1.0) {
        ...
        if (FP(this->template get<MaskProtection<FP>>()[MaskType::Community]) < 0.0 ||
            FP(this->template get<MaskProtection<FP>>()[MaskType::Community]) > 1.0) {
            log_error(
        ...
        if (FP(this->template get<MaskProtection<FP>>()[MaskType::FFP2]) < 0.0 ||
            FP(this->template get<MaskProtection<FP>>()[MaskType::FFP2]) > 1.0) {
            log_error("Constraint check: Parameter MaskProtection for MaskType FFP2 is smaller {:d} or larger {:d}", 0,
                      1);
            return true;
        }

        if (FP(this->template get<MaskProtection<FP>>()[MaskType::Surgical]) < 0.0 ||
            FP(this->template get<MaskProtection<FP>>()[MaskType::Surgical]) > 1.0) {

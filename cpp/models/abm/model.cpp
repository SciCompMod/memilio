...
Person& Model::add_person(const LocationId id, AgeGroup age)
{
...
void Model::movement(TimePoint t, TimeSpan dt)
{
    PRAGMA_OMP(parallel for)
    for (auto i = size_t(0); i < m_persons.size(); ++i) {
        auto&& person     = m_persons[i];
        ...
        auto try_movement_rule = [&](auto rule) -> bool {
            ...
            if (m_testing_strategy.run_strategy(personal_rng, *person, target_location, t)) {
                if (target_location != current_location &&
                    ...
                    if (wears_mask) {
                        person->move_to(target_location);
                    }
                    return true;
                }
            }
        ...
        while (m_trip_list.get_current_index() < num_trips &&
            ...
            if (!person->is_in_quarantine(t, parameters) && person->get_infection_state(t) != InfectionState::Dead) {
                ...
                if (m_testing_strategy.run_strategy(personal_rng, *person, target_location, t)) {
                    ...
                    person->move_to(target_location, trip.trip_mode);
                }
            }
...
void Model::use_movement_rules(bool param)
{
    m_use_movement_rules = param;
}

bool Model::use_movement_rules() const
{

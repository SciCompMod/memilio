...
*/
#ifndef ODESEIRMETAPOPFAST_MODEL_H
#define ODESEIRMETAPOPFAST_MODEL_H
...
// using MyMatrix = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
...
class Model : public FlowModel<FP, InfectionState, mio::Populations<FP, InfectionState, AgeGroup, mio::regions::Region>,
                               Parameters<FP>, Flows>
{

    using Base = FlowModel<FP, InfectionState, mio::Populations<FP, InfectionState, AgeGroup, mio::regions::Region>,
                           Parameters<FP>, Flows>;
...
public:
    ...
    Model(int num_regions, int num_agegroups)
        : Base(Populations({InfectionState::Count, AgeGroup(num_agegroups), Region(num_regions)}),
               ParameterSet(AgeGroup(num_agegroups), Region(num_regions)))
    {
    }
    ...
    {
        mio::timing::AutoTimer<"get_flows_inside"> my_timer7;
        using enum InfectionState;
        const auto& params                         = this->parameters;
        const auto& population                     = this->populations;
        const Eigen::MatrixXd& commuting_strengths = params.template get<CommutingStrengths<FP>>();
        const size_t n_age_groups                  = (size_t)params.get_num_agegroups();
        const size_t n_regions                     = (size_t)params.get_num_regions();

        mio::timing::AutoTimer<"compute_flows"> my_timer6;
        Eigen::MatrixXd infected_pop(n_age_groups, n_regions);

        {
            mio::timing::AutoTimer<"set_inf_pop"> my_timer;
            for (size_t age_i = 0; age_i < n_age_groups; age_i++) {
                for (size_t region_n = 0; region_n < n_regions; region_n++) {
                    infected_pop(age_i, region_n) =
                        pop[population.get_flat_index({InfectionState::Infected, AgeGroup(age_i), Region(region_n)})];
                }
            }
        }

        Eigen::MatrixXd infectious_share_per_region(n_age_groups, n_regions);
        {
            mio::timing::AutoTimer<"compute_infectious_share_per_region"> my_timer2;
            infectious_share_per_region = infected_pop * commuting_strengths;
        }

        {
            mio::timing::AutoTimer<"normalize_infectious_share_per_region"> my_timer3;
            for (size_t age_i = 0; age_i < n_age_groups; age_i++) {
                for (size_t region_n = 0; region_n < n_regions; region_n++) {
                    infectious_share_per_region(age_i, region_n) /=
                        params.template get<PopulationAfterCommuting<FP>>()[{AgeGroup(age_i), Region(region_n)}];
                }
            }
        }
        Eigen::MatrixXd infections_due_commuting(n_age_groups, n_regions);

        {
            mio::timing::AutoTimer<"compute_infections_due_commuting"> my_timer4;
            infections_due_commuting = infectious_share_per_region * commuting_strengths.transpose();
        }

        {
            mio::timing::AutoTimer<"compute_other_compartments"> my_timer5;

            PRAGMA_OMP(parallel for)
            for (size_t region = 0; region < n_regions; region++) {
                for (size_t age_i = 0; age_i < n_age_groups; age_i++) {
                    for (size_t age_j = 0; age_j < n_age_groups; age_j++) {
                        const size_t Ejn = population.get_flat_index({Exposed, AgeGroup(age_j), Region(region)});
                        const size_t Ijn = population.get_flat_index({Infected, AgeGroup(age_j), Region(region)});
                        const size_t Rjn = population.get_flat_index({Recovered, AgeGroup(age_j), Region(region)});
                        const size_t Sjn = population.get_flat_index({Susceptible, AgeGroup(age_j), Region(region)});
                        ...
                        flows[Base::template get_flat_flow_index<Susceptible, Exposed>(
                            {AgeGroup(age_i), Region(region)})] +=
                            (pop[Ijn] * divNjn + infections_due_commuting(age_j, region)) * coeffStoE *
                            y[population.get_flat_index({Susceptible, AgeGroup(age_i), Region(region)})];
                    }
                    flows[Base::template get_flat_flow_index<Exposed, Infected>({AgeGroup(age_i), Region(region)})] =
                        y[population.get_flat_index({Exposed, AgeGroup(age_i), Region(region)})] /
                        params.template get<TimeExposed<FP>>()[{AgeGroup(age_i), Region(region)}];
                    flows[Base::template get_flat_flow_index<Infected, Recovered>({AgeGroup(age_i), Region(region)})] =
                        y[population.get_flat_index({Infected, AgeGroup(age_i), Region(region)})] /
                        params.template get<TimeInfected<FP>>()[{AgeGroup(age_i), Region(region)}];
                }
            }
        }
    }

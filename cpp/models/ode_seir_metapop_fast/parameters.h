...
*/
#ifndef ODESEIRMETAPOPFAST_PARAMETERS_H
#define ODESEIRMETAPOPFAST_PARAMETERS_H
...
struct TransmissionProbabilityOnContact {
    using Type = CustomIndexArray<UncertainValue<FP>, AgeGroup, Region>;
    static Type get_default(AgeGroup size_age, Region size_region)
    {
        return Type({size_age, size_region}, 1.0);
    }
...
struct TimeExposed {
    ...
    static Type get_default(AgeGroup size_age, Region size_region)
    {
        return Type({size_age, size_region}, 5.2);
    }
    static std::string name()
    {
        return "TimeExposed";
...
struct TimeInfected {
    ...
    static Type get_default(AgeGroup size_age, Region size_region)
    {
        return Type({size_age, size_region}, 6.0);
    }
    static std::string name()
    {
        return "TimeInfected";
...
struct ContactPatterns {
    ...
    static Type get_default(AgeGroup size, Region)
    {
...
struct CommutingStrengths {
    ...
    static Type get_default(AgeGroup, Region size)
    {
        return Type((size_t)size, (size_t)size);
    }
    static std::string name()
    {
...
struct PopulationAfterCommuting {
    using Type = Populations<FP, AgeGroup, Region>;
    static Type get_default(AgeGroup size_agegroups, Region size_regions)
    {
        return Type({size_agegroups, size_regions}, 0.);
    }
    static std::string name()
    {
...
{
public:
    Parameters(AgeGroup num_agegroups, Region num_regions)
        : ParametersBase<FP>(num_agegroups, num_regions)
    ...
    bool apply_constraints()
    {
        ...
        for (auto i = AgeGroup(0); i < AgeGroup(m_num_agegroups); i++) {
            for (auto j = Region(0); j < Region(m_num_regions); j++) {
                if (this->template get<TimeExposed<FP>>()[{i, j}] < tol_times) {
                    log_warning(
                        ...
                        this->template get<TimeExposed<FP>>()[{i, j}], tol_times);
                    this->template get<TimeExposed<FP>>()[{i, j}] = tol_times;
                    corrected                                     = true;
                }
                if (this->template get<TimeInfected<FP>>()[{i, j}] < tol_times) {
                    log_warning(
                        "Constraint check: Parameter TimeInfected changed from {} to {}. Please note that "
                        "unreasonably small compartment stays lead to massively increased run time. Consider to cancel "
                        "and reset parameters.",
                        this->template get<TimeInfected<FP>>()[{i, j}], tol_times);
                    this->template get<TimeInfected<FP>>()[{i, j}] = tol_times;
                    corrected                                      = true;
                }
                if (this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}] < 0.0 ||
                    this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}] > 1.0) {
                    log_warning("Constraint check: Parameter TransmissionProbabilityOnContact changed from {} to {} ",
                                this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}], 0.0);
                    this->template get<TransmissionProbabilityOnContact<FP>>() = 0.0;
                    corrected                                                  = true;
                }

                if (this->template get<PopulationAfterCommuting<FP>>()[{i, j}] <= 0.0) {
                    log_warning(
                        ...
                        this->template get<PopulationAfterCommuting<FP>>()[{i, j}], 1.0);
                    this->template get<PopulationAfterCommuting<FP>>()[{i, j}] = 1.0;
                    corrected                                                  = true;
                }
            }
        }
        if ((this->template get<CommutingStrengths<FP>>().rowwise().sum() -
             Eigen::VectorXd::Ones((size_t)this->get_num_regions()))
                    .cwiseAbs()
                    .maxCoeff() > 1e-10 ||
            this->template get<CommutingStrengths<FP>>().minCoeff() < 0.0 ||
            this->template get<CommutingStrengths<FP>>().maxCoeff() > 1.0) {
            ...
            for (auto j = Region(0); j < Region(m_num_regions); j++) {
                if (this->template get<TimeExposed<FP>>()[{i, j}] < tol_times) {
                    log_error(
                        ...
                        "unreasonably small compartment stays lead to massively increased run time. Consider to cancel "
                        "and reset parameters.",
                        this->template get<TimeExposed<FP>>()[{i, j}], 0.0);
                    return true;
                }
                if (this->template get<TimeInfected<FP>>()[{i, j}] < tol_times) {
                    log_error(
                        ...
                        "unreasonably small compartment stays lead to massively increased run time. Consider to cancel "
                        "and reset parameters.",
                        this->template get<TimeInfected<FP>>()[{i, j}], 0.0);
                    return true;
                }
                if (this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}] < 0.0 ||
                    this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}] > 1.0) {
                    log_error("Constraint check: Parameter TransmissionProbabilityOnContact {} smaller {} or "
                              "greater {:.4f}",
                              this->template get<TransmissionProbabilityOnContact<FP>>()[{i, j}], 0.0, 1.0);
                    return true;
                }

                if (this->template get<PopulationAfterCommuting<FP>>()[{i, j}] <= 0.0) {
                    log_error("Constraint check: Parameter PopulationAfterCommuting {} smaller or equal {}",
                              this->template get<PopulationAfterCommuting<FP>>()[{i, j}], 0.0);
                    return true;
                }
            }
        }

...
{
    auto set_distribution = [dev_rel](UncertainValue& v, double min_val = 0.001, double max_val = std::numeric_limits<double>::max()) {
        v.set_distribution(ParameterDistributionNormal(std::min(max_val, std::max(min_val, (1 - dev_rel * 2.6) * v)),
                                                       std::min(max_val, std::max(min_val, (1 + dev_rel * 2.6) * v)), std::min(max_val, std::max(min_val, double(v))),
                                                       dev_rel * v));
    };

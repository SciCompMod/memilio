                           ...
                           const std::string& filename)
{
    ...
    for (auto& result : results) {
        ...
        MEMILIO_H5_CHECK(county_group.id, StatusCode::UnknownError, "Group could not be created (" + group_name + ")");
...
IOResult<std::vector<SecirSimulationResult>> read_result(const std::string& filename, int nb_groups)
{
    ...
    for (auto& name : group_names) {
        ...
        MEMILIO_H5_CHECK(H5Dread(dataset_total.id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, total_values.data()),

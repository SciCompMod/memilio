...
/**
     ...
     */
IOResult<void>
read_rki_data(std::string const& path, std::vector<int> const& vregion, Date date,
                                       ...
                                       int num_days)
{
    ...
    for (size_t county = 0; county < model.size(); county++) {
        for (size_t group = 0; group < ConfirmedCasesDataEntry::age_group_names.size(); group++) {
            ...
            mu_C_R[county].push_back(
                model[county].parameters.template get<AsymptomaticCasesPerInfectious>()[AgeGroup(group)]);
    ...
    for (size_t j = 0; j < static_cast<size_t>(num_days); j++) {
        std::vector<std::vector<double>> num_inf(
        ...
        std::vector<std::vector<double>> num_rec(
            model.size(), std::vector<double>(ConfirmedCasesDataEntry::age_group_names.size(), 0.0));
        std::vector<std::vector<double>> num_exp(
            model.size(), std::vector<double>(ConfirmedCasesDataEntry::age_group_names.size(), 0.0));
        std::vector<std::vector<double>> num_car(
            model.size(), std::vector<double>(ConfirmedCasesDataEntry::age_group_names.size(), 0.0));
        std::vector<std::vector<double>> num_hosp(
            model.size(), std::vector<double>(ConfirmedCasesDataEntry::age_group_names.size(), 0.0));
        ...
        for (size_t i = 0; i < region.size(); i++) {
            for (size_t age = 0; age < ConfirmedCasesDataEntry::age_group_names.size(); age++) {
                rki_data[i][j]((size_t)InfectionState::Exposed + (size_t)InfectionState::Count * age) = num_exp[i][age];

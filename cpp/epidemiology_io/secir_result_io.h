                           ...
                           const std::string& filename)
{
    ...
    EPI_H5_CHECK(file.id, StatusCode::FileNotFound, filename);
    for (auto& result : results) {
        ...
        EPI_H5_CHECK(county_group.id, StatusCode::UnknownError, "Group could not be created (" + group_name + ")");

        const int n_data    = static_cast<int>(result.get_num_time_points());
        const int n_compart = (int)ModelType::Count;
        ...
        EPI_H5_CHECK(dspace_t.id, StatusCode::UnknownError, "Time DataSpace could not be created.");
        H5DataSet dset_t{
            H5Dcreate(county_group.id, "Time", H5T_NATIVE_DOUBLE, dspace_t.id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)};
        EPI_H5_CHECK(dset_t.id, StatusCode::UnknownError, "Time DataSet could not be created (Time).");
        auto values_t = std::vector<double>(result.get_times().begin(), result.get_times().end());
        EPI_H5_CHECK(H5Dwrite(dset_t.id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, values_t.data()),
        ...
        for (int group = 0; group < nb_groups + 1; ++group) {
            ...
            EPI_H5_CHECK(dspace_values.id, StatusCode::UnknownError, "Values DataSpace could not be created.");
            ...
            EPI_H5_CHECK(dset_values.id, StatusCode::UnknownError, "Values DataSet could not be created.");

            EPI_H5_CHECK(H5Dwrite(dset_values.id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
 ...
 */
inline herr_t store_group_name(hid_t loc_id, const char* name, const H5L_info_t* linfo, void* opdata)
{
    unused(linfo);
    hid_t group;
...
{
    const int nb_compart = (int)ModelType::Count;
    ...
    EPI_H5_CHECK(H5Literate(file.id, H5_INDEX_NAME, H5_ITER_INC, NULL, &store_group_name, &group_names),
    ...
    for (auto& name : group_names) {
        ...
        EPI_H5_CHECK(dataset_t.id, StatusCode::UnknownError, "Time DataSet could not be read.");

        // dataset dimensions
        H5DataSpace dataspace_t{H5Dget_space(dataset_t.id)};
        EPI_H5_CHECK(dataspace_t.id, StatusCode::UnknownError, "Time DataSpace could not be read.");
        ...
        EPI_H5_CHECK(H5Dread(dataset_t.id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, time.data()),
        ...
        for (int i = 0; i < nb_groups; ++i) {
            ...
            EPI_H5_CHECK(dataset_values.id, StatusCode::UnknownError, "Values DataSet could not be read.");

            //read data space dimensions
            H5DataSpace dataspace_values{H5Dget_space(dataset_values.id)};
            EPI_H5_CHECK(dataspace_values.id, StatusCode::UnknownError, "Values DataSpace could not be read.");
        ...
        EPI_H5_CHECK(dataset_total.id, StatusCode::UnknownError, "Totals DataSet could not be read.");

        //read data space dimensions
        H5DataSpace dataspace_total{H5Dget_space(dataset_total.id)};
        EPI_H5_CHECK(dataspace_total.id, StatusCode::UnknownError, "Totals DataSpace could not be read.");
        ...
        EPI_H5_CHECK(H5Dread(dataset_total.id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, total_values.data()),

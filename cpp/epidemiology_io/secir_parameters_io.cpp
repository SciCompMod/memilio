            ...
            if (it != vregion.end()) {
                ...
                if (it_age != age_names.end() - 1) {
                    ...
                    if (date_df == offset_date_by_days(date, 0)) {
                        ...
                        num_inf[age] += (1 - mu_I_H[age]) * scaling_factor_inf[age] * root[i]["Confirmed"].asDouble();
                        num_rec[age] += root[i]["Confirmed"].asDouble();
                    }
                    if (date_df == offset_date_by_days(date, days_surplus)) {
                        num_car[age] -= scaling_factor_inf[age] * root[i]["Confirmed"].asDouble();
                    }
                    // +R3
                    if (date_df == offset_date_by_days(date, +t_car_to_inf[age] + days_surplus)) {
                        num_car[age] += scaling_factor_inf[age] * root[i]["Confirmed"].asDouble();
                        num_car[age] +=
                            mu_C_R[age] / (1 - mu_C_R[age]) * scaling_factor_inf[age] * root[i]["Confirmed"].asDouble();
                        num_exp[age] -=
                            1 / (1 - mu_C_R[age]) * scaling_factor_inf[age] * root[i]["Confirmed"].asDouble();
                    }
                    // R2 - R9

...
void set_dynamic_NPIs(const SecirOptimization& settings, mio::osecir::Model<FP>& model,
                      const std::vector<FP>& dynamic_NPI_strengths)
{
    mio::DynamicNPIs<FP>& dynamic_npis = model.parameters.template get<mio::osecir::DynamicNPIsInfectedSymptoms<FP>>();

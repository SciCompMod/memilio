...
void check_constraint_feasibility(const SecirOptimization& settings)
{
    std::vector<double> dynamic_NPI_strength(settings.num_control_parameters());
    ...
    for (const auto& cp : settings.control_parameters()) {
        for (const auto& d : cp.dampings()) {
            dynamic_NPI_strength[index] = 1.0;
            index++;
        }
    }
    ...
    for (auto& node : graph_model.nodes()) {
        set_dynamic_NPIs<double>(settings, node.property.get_simulation().get_model(), dynamic_NPI_strength);
    }

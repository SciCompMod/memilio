...
public:
    OptimizationModel(const boost::filesystem::path& data_directory, size_t simulation_days, size_t num_age_groups,
                      double npis_duration, double npis_interval, double npis_base_value);
...
OptimizationModel::create_graph_model() const
{
    ...
    params.template get<mio::osecirvvs::DailyPartialVaccinations<FP>>().resize(
        mio::SimulationDay(simulation_days() + 1));
    params.template get<mio::osecirvvs::DailyFullVaccinations<FP>>().resize(mio::SimulationDay(simulation_days() + 1));

    size_t daily_vaccinations = 10;
    for (size_t age_group = 0; age_group < num_age_groups; age_group++) {
        for (size_t current_day = 0; current_day <= simulation_days(); current_day++) {
    ...
    set_all_groups(mio::osecirvvs::TimeExposed<FP>{}, 2.67);
    ...
    params.template get<mio::osecirvvs::DynamicNPIsInfectedSymptoms<FP>>() = dynamic_npis;
    ...
    model.parameters.get_end_commuter_detection()   = 400.0;
    model.parameters.get_commuter_nondetection()    = 1.0;
    ...
    graph_simulation.add_node(1, model);

    mio::MobilityEdge<FP> edge0(
        mio::MobilityParameters<FP>(Eigen::VectorX<FP>::Constant(num_age_groups * num_infection_states, 0.001)));
    mio::MobilityEdge<FP> edge1(
        mio::MobilityParameters<FP>(Eigen::VectorX<FP>::Constant(num_age_groups * num_infection_states, 0.001)));

    graph_simulation.add_edge(0, 1, edge0);
    graph_simulation.add_edge(1, 0, edge1);

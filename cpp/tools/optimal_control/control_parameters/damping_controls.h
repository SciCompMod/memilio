...
mio::DampingSampling<FP> make_damping_from_control_parameter(mio::SimulationTime<FP> time, const ControlParameter& control_parameter, FP value)
{

    return mio::DampingSampling<FP>(control_parameter.effectiveness() * value, control_parameter.damping().get_level(), control_parameter.damping().get_type(), time, control_parameter.damping().get_matrix_indices(), control_parameter.damping().get_group_weights());
}

template <typename FP, class OptimizationSettings>
void set_control_dampings(const OptimizationSettings& settings, typename OptimizationSettings::template ModelTemplate<FP>& model,
                          const std::vector<FP>& control_parameters)
{
    ...
    mio::UncertainContactMatrix<FP>& contacts = model.parameters.template get<typename OptimizationSettings::OptimizationModelType::template ContactPatterns<FP>>();

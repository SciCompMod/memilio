...
public:
    ...
    get_graph_model(unsigned int seed) const;
...
private:
    ...
    template <typename FP>
    mio::Graph<mio::SimulationNode<FP, mio::osecirvvs::Simulation<FP>>, mio::MobilityEdge<FP>>
    create_graph_model() const;

    template <typename FP>
    void apply_uncertainty(
        unsigned int seed,
        mio::Graph<mio::SimulationNode<FP, mio::osecirvvs::Simulation<FP>>, mio::MobilityEdge<FP>>& graph_model) const;
...
template <typename FP>
auto OptimizationModel::get_graph_model(unsigned int seed) const
    -> mio::Graph<mio::SimulationNode<FP, mio::osecirvvs::Simulation<FP>>, mio::MobilityEdge<FP>>
{
...
template <typename FP>
void OptimizationModel::apply_uncertainty(
    unsigned int seed,
    mio::Graph<mio::SimulationNode<FP, mio::osecirvvs::Simulation<FP>>, mio::MobilityEdge<FP>>& graph_model) const
{
    std::mt19937 rng(seed);

    std::uniform_real_distribution<double> TransmissionProbabilityOnContact_dist(0.07, 0.12);
    std::uniform_real_distribution<double> pop_mult_dist(0.75, 1.5);

    double v1 = TransmissionProbabilityOnContact_dist(rng);
    double v2 = pop_mult_dist(rng);

    for (auto& node : graph_model.nodes()) {
        auto& node_simulation = node.property.get_simulation();
        auto& node_model      = node_simulation.get_model();
        auto& params          = node_model.parameters;

        mio::TimeSeries<FP>& simulation_result      = node_simulation.get_result();
        Eigen::Ref<Eigen::VectorX<FP>> latest_state = simulation_result.get_last_value();

        auto set_all_groups = [&](auto Tag, FP value) {
            auto& age_group_params = params.template get<decltype(Tag)>();
            for (size_t age_group = 0; age_group < 6; age_group++) {
                age_group_params[mio::AgeGroup(age_group)] = value;
            }
        };

        set_all_groups(mio::osecirvvs::TransmissionProbabilityOnContact<FP>{}, v1);

        // Uncertain initial population
        for (mio::AgeGroup age = 0; age < node_model.parameters.get_num_groups(); ++age) {
            for (auto& state : query_infection_states("InfectedNoSymptoms")) {
                mio::Index<mio::AgeGroup, mio::osecirvvs::InfectionState> idx{age, state};
                FP value  = node_model.populations[idx];
                FP factor = static_cast<FP>(v2);

                node_model.populations[idx] = value * factor;
                latest_state[static_cast<size_t>(state) +
                             static_cast<size_t>(age) * static_cast<size_t>(mio::osecirvvs::InfectionState::Count)] =
                    value * factor;
            }
        }

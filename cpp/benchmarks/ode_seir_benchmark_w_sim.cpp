...
struct DtOverride {
    ...
    DtOverride(ScalarType new_dt, ScalarType new_t_max)
    {
...
static void bench_matrix_phi_reconstruction_rk4_dt(::benchmark::State& state)
{
    DtOverride ov(mio::benchmark_mio::dt_sweep_values[state.range(2)], SWEEP_T_MAX);
    bench_matrix_phi_reconstruction_rk4(state);
}
...
BENCHMARK(bench_matrix_phi_reconstruction_rk4_dt)
    ->Apply(register_dt_sweep)
    ->Name("matrix_phi_reconstruction(RK4)/dt")
    ->Unit(::benchmark::kMicrosecond);

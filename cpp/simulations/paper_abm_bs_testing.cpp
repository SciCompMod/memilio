    ...
    for (auto& loc : world.get_locations()) {
        ...
        case mio::abm::LocationType::Event:
            ...
            loc.get_infection_parameters().get<mio::abm::ContactRates>().array() *=
                8 * 4; //3 hours with special intensivity
            break;
        default:
                                   ...
                                   mio::TimeSeries<ScalarType> sim_det, int max_num_days, mio::Date start_date)
{
    ...
    return (1.00 * rmse_dead) + (0.1 * rmse_icu) + (0.01 * 0.01 * 0.1 * rmse_conf);
}
...
int main(int argc, char** argv)
{
    ...
    if (run_grid_search) {
        ...
        std::vector<double> grid_boundaries = {1.85, 3.06, 0.425};
        ...
        std::vector<std::vector<double>> parameters = {{2.025}, {2.66}, {0.4}, {0.031}, {7.125}, {10.0}, {0.5}};

...
df_abb = pd.read_excel(
    'simulations/datensatzbeschreibung_massnahmen.xlsx', sheet_name='DSB BL')

#read in the matrix which tells us which measure is active on which day
df_measure_matrix = pd.read_csv(
    r'/Users/saschakorf/Documents/Arbeit.nosynch/memilio/memilio/cpp/simulations/maßnahmen_oberkategorien_bundesländer.csv')



name_bundesland = "Niedersachsen"
...
df_measure_matrix = df_measure_matrix[df_measure_matrix['bundesland'] == name_bundesland]
#also delete all files with different date
df_measure_matrix = df_measure_matrix[(df_measure_matrix['datum']>= start_date) & (df_measure_matrix['datum'] <= end_date)]
...
f = open('measures_active_days_bl.txt', 'w')
# for every measure
for measure in df_measure_matrix.axes[1][4:]:
    # count the days it was active
    days = df_measure_matrix[measure].sum()
    # get the dates it was active on in one list
    dates = df_measure_matrix['datum'][df_measure_matrix[measure] == 1].tolist()

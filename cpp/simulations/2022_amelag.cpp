...
mio::IOResult<void> set_covid_parameters(mio::osecirts::Parameters<double>& params)
{
    ...
    //probabilities
    double fac_variant                                 = 1.9; //https://doi.org/10.7554/eLife.78933
    ...
    const double reducExposedPartialImmunityMin = 0.50; //0.569; // doi.org/10.1136/bmj-2022-071502
    const double reducExposedPartialImmunityMax = 0.50; // 0.637; // doi.org/10.1136/bmj-2022-071502
...
mio::IOResult<void> run(const std::string data_dir, std::string res_dir, const int num_runs, const int num_days)
{
    ...
    auto end_date         = mio::Date(2024, 3, 1);
...
int main()
{
    ...
    std::string results_dir       = mio::path_join(memilio_dir, "results/amelag");

    const auto num_days = 350;

...
mio::IOResult<void> set_covid_parameters(mio::osecirts::Parameters<double>& params)
{
    ...
    //probabilities
    double fac_variant                                 = 1.05; //https://doi.org/10.7554/eLife.78933
    ...
    const auto val_TI = 60.0;

    const double ImmunityInterval1Min = val_TI; ///https://doi.org/10.1016/S1473-3099(22)00801-5
    const double ImmunityInterval1Max = val_TI;
    ...
    const double ImmunityInterval2Min = val_TI; // https://doi.org/10.1016/S1473-3099(22)00801-5
    const double ImmunityInterval2Max = val_TI;
...
set_nodes(const mio::osecirts::Parameters<FP>& params, mio::Date start_date, mio::Date end_date,
          ...
          FP tnt_capacity_factor, int num_days = 0, bool export_time_series = false, bool rki_age_groups = true,
          bool masks = false, bool ffp2 = false)
{
    ...
    for (size_t node_idx = 0; node_idx < nodes.size(); ++node_idx) {
        ...
        for (int t_idx = 0; t_idx < num_days; ++t_idx) {
            ...
            for (auto j = mio::AgeGroup(0); j < params.get_num_groups(); j++) {
                mobility_model.parameters.template get<mio::osecirts::DailyPartialVaccination<double>>()[{j, t}] = 0;
                mobility_model.parameters.template get<mio::osecirts::DailyFullVaccination<double>>()[{j, t}]    = 0;
                // mobility_model.parameters.template get<mio::osecirts::DailyBoosterVaccination>()[{j, t}] = 0;
            }
        }
 ...
 * @returns Currently generates no errors.
 */
mio::IOResult<void> set_npis(mio::Date start_date, mio::Date end_date, mio::osecirts::Parameters<double>& params,
                             ScalarType fact)
{
    auto& contacts         = params.get<mio::osecirts::ContactPatterns<double>>();
    ...
    auto contacts_at_home = [=](mio::SimulationTime t, auto min, auto max) {
    ...
    auto school_closure = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
        return mio::DampingSampling<double>(v, mio::DampingLevel(int(InterventionLevel::Main)),
    ...
    auto home_office = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
        return mio::DampingSampling<double>(v, mio::DampingLevel(int(InterventionLevel::Main)),
    ...
    auto social_events = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
        return mio::DampingSampling<double>(v, mio::DampingLevel(int(InterventionLevel::Main)),
    ...
    auto social_events_work = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
        return mio::DampingSampling<double>(v, mio::DampingLevel(int(InterventionLevel::Main)),
                                            mio::DampingType(int(Intervention::GatheringBanFacilitiesClosure)), t,
    ...
    auto physical_distancing_home_school = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
    ...
    auto physical_distancing_work_other = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
        return mio::DampingSampling<double>(v, mio::DampingLevel(int(InterventionLevel::PhysicalDistanceAndMasks)),
                                            mio::DampingType(int(Intervention::PhysicalDistanceAndMasks)), t,
    ...
    auto senior_awareness = [=](mio::SimulationTime t, auto min, auto max) {
        auto v = mio::UncertainValue<double>();
        assign_uniform_distribution(v, min, max);
    ...
    auto start_octoberfest = mio::Date(2022, 9, 17);
    if (start_octoberfest < end_date) {
        auto start_day_october = mio::SimulationTime(mio::get_offset_in_days(start_octoberfest, start_date));
        contact_dampings.push_back(contacts_at_home(start_day_october, fact, fact));
        contact_dampings.push_back(school_closure(start_day_october, fact, fact));
        contact_dampings.push_back(home_office(start_day_october, fact, fact));
        contact_dampings.push_back(social_events(start_day_october, fact, fact));
        contact_dampings.push_back(social_events_work(start_day_october, fact, fact));
        contact_dampings.push_back(physical_distancing_home_school(start_day_october, fact, fact));
        contact_dampings.push_back(physical_distancing_work_other(start_day_october, fact, fact));
        contact_dampings.push_back(senior_awareness(start_day_october, fact, fact));
    }

    auto end_octoberfest = mio::Date(2022, 10, 3);
    if (end_octoberfest < end_date) {
        auto end_day_october = mio::SimulationTime(mio::get_offset_in_days(end_octoberfest, start_date));
        contact_dampings.push_back(contacts_at_home(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(school_closure(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(home_office(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(social_events(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(social_events_work(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(physical_distancing_home_school(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(physical_distancing_work_other(end_day_october, 0.0, 0.0));
        contact_dampings.push_back(senior_awareness(end_day_october, 0.0, 0.0));
    }
    return mio::success();
}
...
mio::IOResult<void> run(const std::string data_dir, std::string res_dir, const int num_runs, const int num_days)
{
    ...
    auto start_date = mio::Date(2022, 9, 15);
    auto end_date   = mio::Date(2022, 12, 20);
    // const int num_runs  = 12;
    constexpr bool masks  = false;
    constexpr bool ffp2   = false;
    ...
    for (auto& node : params_graph.nodes()) {
        if (node.id == 9162) {
            // set NPIs
            auto npi_status = set_npis(start_date, end_date, node.property.base_sim.parameters, -0.5);
            std::cout << "NPIs set for Munich" << std::endl;
        }
    }
...
int main()
{
    ...
    std::string results_dir       = mio::path_join(memilio_dir, "results/octoberfest1_05_npi_05/secirts");

    const auto num_days = 83;
    const auto num_runs = 70;

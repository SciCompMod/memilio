...
mio::IOResult<void> set_covid_parameters(mio::osecir::Parameters& params)
{
    ...
    const double severePerInfectedSymptomsMin[]       = {0.006, 0.006, 0.015, 0.049, 0.05, 0.01};
    const double severePerInfectedSymptomsMax[]       = {0.009, 0.009, 0.023, 0.074, 0.05, 0.01};
                                                         ...
                                                         const fs::path& data_dir, double sclaling_infected)
{
    ...
    auto scaling_factor_icu      = std::vector<double>(size_t(params.get_num_groups()), 2.4);

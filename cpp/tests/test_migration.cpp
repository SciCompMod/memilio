...
{
    auto deriv_f1 = [] (auto&& y, auto&& t, auto&& ydt) { ydt[0] = cos(t); };
    auto deriv_f2 = [] (auto&& y, auto&& t, auto&& ydt) { ydt[0] = -sin(t); };
    auto migration_f = [] (auto&& idx, auto&& t, auto&& matrix) { matrix = Eigen::MatrixXd::Identity(2,2); };
    ...
    EXPECT_FLOAT_EQ(t_group.back(), tmax);
    ...
    for (size_t j = 0; j < v_single1.back().size(); j++)
    {

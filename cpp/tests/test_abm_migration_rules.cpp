...
TEST(TestMigrationRules, school_return)
{
    ...
    ASSERT_EQ(mio::abm::go_to_school(p_child, t, dt, mio::abm::Parameters(6)), mio::abm::LocationType::Home);
}
...
TEST(TestMigrationRules, go_shopping)
{
    ...
    ASSERT_EQ(mio::abm::go_to_shop(p_home, t_night, dt, mio::abm::Parameters(6)), mio::abm::LocationType::Home);

    ScopedMockDistribution<testing::StrictMock<MockDistribution<mio::ExponentialDistribution<double>>>>
        mock_exponential_dist;
    EXPECT_CALL(mock_exponential_dist.get_mock(), invoke).Times(1).WillOnce(testing::Return(0.01));
    ASSERT_EQ(mio::abm::go_to_shop(p_home, t_weekday, dt, mio::abm::Parameters(6)), mio::abm::LocationType::BasicsShop);
}
...
TEST(TestMigrationRules, go_event)
{
    ...
    ASSERT_EQ(mio::abm::go_to_event(p_work, t_weekday, dt, mio::abm::Parameters(6)), mio::abm::LocationType::Work);
    ASSERT_EQ(mio::abm::go_to_event(p_home, t_night, dt, mio::abm::Parameters(6)), mio::abm::LocationType::Home);

    ScopedMockDistribution<testing::StrictMock<MockDistribution<mio::ExponentialDistribution<double>>>>
        mock_exponential_dist;
    EXPECT_CALL(mock_exponential_dist.get_mock(), invoke).Times(1).WillOnce(testing::Return(0.01));
    ASSERT_EQ(mio::abm::go_to_event(p_home, t_weekday, dt, mio::abm::Parameters(6)),
              mio::abm::LocationType::SocialEvent);

    EXPECT_CALL(mock_exponential_dist.get_mock(), invoke).Times(1).WillOnce(testing::Return(0.01));
    ASSERT_EQ(mio::abm::go_to_event(p_home, t_saturday, dt, mio::abm::Parameters(6)),
              mio::abm::LocationType::SocialEvent);
}
...
TEST(TestMigrationRules, recover)
{
    ...
    ASSERT_EQ(mio::abm::return_home_when_recovered(p_rec, t, dt, mio::abm::Parameters(6)),
              mio::abm::LocationType::Home);
    ASSERT_EQ(mio::abm::return_home_when_recovered(p_inf, t, dt, mio::abm::Parameters(6)),
              mio::abm::LocationType::Hospital);
}

TEST(TestMigrationRules, dead)
{
    auto icu    = mio::abm::Location(mio::abm::LocationType::ICU, 0, 6);
    auto t      = mio::abm::TimePoint(12346);
    auto dt     = mio::abm::hours(1);
    auto p_dead = make_test_person(icu, AGE_GROUP_60_TO_79, mio::abm::InfectionState::Dead, t);

    ASSERT_EQ(mio::abm::get_buried(p_dead, t, dt, mio::abm::Parameters(6)), mio::abm::LocationType::Cemetery);
}

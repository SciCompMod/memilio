...
{ // Copy and move (assignment) are defined in base class StateAgeFunction and are equal for all derived classes, therefore test for one Special Member.
    ...
    // move
    mio::GammaSurvivalFunction<double> gamma3(std::move(gamma2));
    ...
    // copy assignment
    mio::GammaSurvivalFunction<double> gamma4 = gamma3;
    ...
    // move assignment
    mio::GammaSurvivalFunction<double> gamma5 = std::move(gamma4);
    ...
    mio::LognormSurvivalFunction<double> logn(0.1, 3.3, 0.5);
    ...
    mio::ErlangDensity<double> erl(2, 0.7);
...
TEST(TestStateAgeFunction, testSettersAndGettersForParameters)
{
    ...
    mio::GammaSurvivalFunction<double> gamma(testvalue_before, testvalue_before, -1);
...
TEST(TestStateAgeFunction, testGetSupportMax)
{
    ...
    mio::GammaSurvivalFunction<double> gamma(1.0, 1, 0.5);
    ...
    mio::LognormSurvivalFunction<double> logn(0.1, 1.0, 0.5);
    ...
    mio::ErlangDensity<double> erl(2, 0.5);
...
{ // Same as testSpecialMember for Wrapper.
    ScalarType dt = 0.5;
    mio::GammaSurvivalFunction<double> gamma(1., 2., 3.);
    ...
    mio::LognormSurvivalFunction<double> logn(0.1, 1, 0.5);
...
TEST(TestStateAgeFunction, testComparisonOperator)
{
    ...
    mio::GammaSurvivalFunction<double> gamma(0.5, 0, 1);
    mio::GammaSurvivalFunction<double> gamma2(0.5, 0, 1);
    mio::GammaSurvivalFunction<double> gamma3(2, 0, 1);
    mio::GammaSurvivalFunction<double> gamma4(0.5, 1.5, 1);
    mio::GammaSurvivalFunction<double> gamma5(0.5, 0, 2);
    mio::LognormSurvivalFunction<double> logn(0.5, 0, 1);
...
TEST(TestStateAgeFunction, testGamma)
{
    ...
    for (ScalarType r : rate) {
        mio::GammaSurvivalFunction<double> gamma(1, 0, 1. / r);

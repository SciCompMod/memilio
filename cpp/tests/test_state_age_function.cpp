...
TEST(TestStateAgeFunction, testSpecialMember)
{
    ...
    EXPECT_EQ(smoothcos.get_mean(dt), smoothcos2.get_mean(dt));
    ...
    EXPECT_EQ(smoothcos.get_mean(dt), smoothcos3.get_mean(dt));
    ...
    EXPECT_EQ(smoothcos.get_mean(dt), smoothcos4.get_mean(dt));
    ...
    EXPECT_EQ(smoothcos.get_mean(dt), smoothcos5.get_mean(dt));
...
TEST(TestStateAgeFunction, testGetMean)
{
    ScalarType dt = 0.5;

    // Test get_mean for all derived classes as this method can be overridden.
    // Always check that the average is still calculated correctly after setting the parameter object of a StateAgeFunction.
    // Check ExponentialDecay.
    mio::ExponentialDecay expdecay(1.0);
    EXPECT_NEAR(expdecay.get_mean(dt), 1., 1e-14);
    expdecay.set_parameter(2.0);
    EXPECT_NEAR(expdecay.get_mean(dt), 0.5, 1e-14);

    // Check ConstantFunction.
    mio::ConstantFunction constfunc(1.0);
    EXPECT_NEAR(constfunc.get_mean(dt), 1.0, 1e-14);
    constfunc.set_parameter(2.0);
    EXPECT_NEAR(constfunc.get_mean(dt), 2.0, 1e-14);

    // Check SmootherCosine where the function is not overridden.
    mio::SmootherCosine smoothcos(1.0);
    EXPECT_NEAR(smoothcos.get_mean(dt), 0.5, 1e-14);
    smoothcos.set_parameter(2.0);
    EXPECT_NEAR(smoothcos.get_mean(dt), 1.0, 1e-14);
}
...
TEST(TestStateAgeFunction, testSAFWrapperSpecialMember)
{
    ...
    EXPECT_EQ(wrapper.get_support_max(dt), 23.5);
    ...
    // Also test the constructor of StateAgeFunctionWrapper initialized with SmootherCosine and ConstantFunction.
    // This way we can be sure that clone_impl works for all derived classes of StateAgeFunction.
    mio::SmootherCosine smoothcos(1.0);
    mio::StateAgeFunctionWrapper wrapper_exp(smoothcos);
    EXPECT_NEAR(wrapper_exp.get_parameter(), 1., 1e-14);
    EXPECT_NEAR(wrapper_exp.get_support_max(dt), 1., 1e-14);
    EXPECT_NEAR(wrapper_exp.get_mean(dt), 0.5, 1e-14);
...
TEST(TestStateAgeFunction, testSAFWrapperSettersAndGetters)
{
    ...
    EXPECT_EQ(wrapper.get_mean(dt), testvalue_after * 0.5);
}

...
TEST(IdeSecir, checkProportionRecoveredDeath)
{
    ...
    // Check if the compartments E, C, I, H and U are empty at the equilibrium.
    EXPECT_NEAR(secihurd_simulated.get_last_value()[(Eigen::Index)mio::isecir::InfectionState::Exposed] ,0, 1e-18);
    EXPECT_NEAR(secihurd_simulated.get_last_value()[(Eigen::Index)mio::isecir::InfectionState::InfectedNoSymptoms]  ,0, 1e-8);
    EXPECT_NEAR(secihurd_simulated.get_last_value()[(Eigen::Index)mio::isecir::InfectionState::InfectedSymptoms]  ,0, 1e-8);
    EXPECT_NEAR(secihurd_simulated.get_last_value()[(Eigen::Index)mio::isecir::InfectionState::InfectedSevere]  ,0, 1e-8);
    EXPECT_NEAR(secihurd_simulated.get_last_value()[(Eigen::Index)mio::isecir::InfectionState::InfectedCritical] ,0, 1e-8);

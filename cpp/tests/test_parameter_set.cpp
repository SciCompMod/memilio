...
struct DoubleParamNoCheck
{
    using Type=double;
};

struct DoubleParamCheck
{
    using Type=double;
    static void check_constraints(Type const& val)
    {
        if (val < 0 ) {
            epi::log_warning("Constraint check: IntParamCheck value {:f} must be larger than zero", val);
        }
    }
};


TEST(TestParameterSet, check_constraints)
{
...
struct MyParams : public epi::ParameterSet<DoubleParam>
{
    template <typename... Args>
    MyParams(Args&&... args)
        : epi::ParameterSet<DoubleParam>(std::forward<Args>(args)...)
    {}

    void apply_constraints()
    {
        if (this->get<DoubleParam>() < 0 ) {
            epi::log_warning("Constraint check: IntParamCheck value {:d} will be changed to zero", this->get<DoubleParam>());
            this->set<DoubleParam>(0.0);
        }
    }
};

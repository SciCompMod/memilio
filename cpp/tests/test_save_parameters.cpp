    ...
    for (size_t node = 0; node < num_nodes; node++) {
        ...
        for (auto group = epi::AgeGroup(0); group < epi::AgeGroup(num_groups); group++) {
            ...
            check_distribution(
                ...
                *graph_read_model.parameters.get<epi::HomeToHospitalizedTime>()[group].get_distribution().get());
            check_distribution(
                *graph_model.parameters.get<epi::MaxRiskOfInfectionFromSympomatic>()[group].get_distribution().get(),
                *graph_read_model.parameters.get<epi::MaxRiskOfInfectionFromSympomatic>()[group]
                     .get_distribution()
                     .get());
            check_distribution(
...
TEST(TestSaveParameters, ExtrapolateRKI)
{
    ...
    auto extrapolate_result = epi::export_input_data_county_timeseries(model, TEST_DATA_DIR, results_dir, county, date,

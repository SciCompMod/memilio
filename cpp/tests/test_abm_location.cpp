...
TEST(TestLocation, CacheExposureRate)
{
    ...
    auto location_id = world.add_location(mio::abm::LocationType::PublicTransport, 3);
    auto& location = world.get_individualized_location(location_id);

    auto& infected1     = world.add_person(home_id, age);
    auto rng_infected1 = mio::abm::Person::RandomNumberGenerator(rng, infected1);
    infected1.add_new_infection(
            mio::abm::Infection(rng_infected1, variant, age, world.get_global_infection_parameters(), t, mio::abm::InfectionState::InfectedNoSymptoms));
    infected1.migrate_to(location, {0});
    auto& infected2     = world.add_person(home_id, age);
    auto rng_infected2 = mio::abm::Person::RandomNumberGenerator(rng, infected2);
    infected2.add_new_infection(
            mio::abm::Infection(rng_infected2, variant, age, world.get_global_infection_parameters(), t, mio::abm::InfectionState::InfectedNoSymptoms));

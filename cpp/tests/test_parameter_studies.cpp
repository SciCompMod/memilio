...
TEST(ParameterStudies, sample_edges)
{
    ...
    auto graph = epi::Graph<epi::SecirModel<epi::AgeGroup3>, epi::MigrationParameters>();
    graph.add_node(0, model);
    graph.add_node(1, model);
    graph.add_edge(0, 1, epi::MigrationParameters(Eigen::VectorXd::Constant(Eigen::Index(num_groups * 8), 1.0)));

    auto study = epi::ParameterStudy<epi::SecirModel<epi::AgeGroup3>>(graph, 0.0, 0.0, 0.5, 1);

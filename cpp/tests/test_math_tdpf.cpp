...
TEST_F(TestMathTdpf, zero)
{
    ...
    for (int i = 0; i < this->num_evals; i++) {
        auto random_t_eval = fuzzy_val(-std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
...
TEST_F(TestMathTdpf, linearInterpolation)
{
    ...
    const double t_min = -1, t_max = 1, t_mid = fuzzy_val(t_min, t_max);
    const double slope1 = fuzzy_val(min, max), slope2 = fuzzy_val(min, max), height = fuzzy_val(min, max);
    ...
    for (int i = 0; i < 10; i++) {
        const double t = fuzzy_val(-1.0, 1.0);
    ...
    for (int i = 0; i < this->num_evals; i++) {
        ...
        double random_t_eval = fuzzy_val(1.25 * t_min - 0.25 * t_max, 1.25 * t_max - 0.25 * t_min);

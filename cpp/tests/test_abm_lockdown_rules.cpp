...
TEST(TestLockdownRules, school_closure)
{
    ...
    auto p1_rng = mio::abm::Person<double>::RandomNumberGenerator(rng, p1);
    ASSERT_EQ(mio::abm::go_to_school(p1_rng, p1, t_morning, dt, params), mio::abm::LocationType::Home);
    auto p2_rng = mio::abm::Person<double>::RandomNumberGenerator(rng, p2);
...
TEST(TestLockdownRules, school_opening)
{
    ...
    EXPECT_CALL(mock_uniform_dist.get_mock(), invoke)
        ...
        .WillOnce(testing::Return(0.6))
        .WillRepeatedly(testing::Return(1.0));
    auto p = mio::abm::Person<double>(rng, home, age_group_5_to_14);
...
TEST(TestLockdownRules, home_office)
{
    ...
    auto p1_rng = mio::abm::Person<double>::RandomNumberGenerator(rng, person1);
    ASSERT_EQ(mio::abm::go_to_work(p1_rng, person1, t_morning, dt, params), mio::abm::LocationType::Work);
    auto p2_rng = mio::abm::Person<double>::RandomNumberGenerator(rng, person2);
...
TEST(TestLockdownRules, social_event_closure)
{
    ...
    mio::abm::Location<double> home(mio::abm::LocationType::Home, 0, num_age_groups);
    mio::abm::Location<double> event(mio::abm::LocationType::SocialEvent, 0, num_age_groups);

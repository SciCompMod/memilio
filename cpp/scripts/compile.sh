...
if [ -n "$build_type" ]; then
    ...
    cmake -S ${PWD}/.. -B ${PWD}/../build -DCMAKE_BUILD_TYPE="$build_type" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DMEMILIO_USE_BUNDLED_EIGEN:BOOL=ON .. || { echo "CMake configuration failed"; exit 1; }
fi

cmake --build ${PWD}/../build -j 8
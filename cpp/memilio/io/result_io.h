                                       ...
                                       const std::vector<int>& county_ids, const fs::path& result_dir, size_t run_idx)
{
    ...
    BOOST_OUTCOME_TRY(write_graph(create_graph_without_edges<Model, MovementParameters>(params, county_ids),
    ...
    if (save_percentiles) {
        ...
        // save percentiles of parameters
        {
            ...
            auto make_graph = [&county_ids](auto&& params) {
                return create_graph_without_edges<Model, MovementParameters>(params, county_ids);
            };
            BOOST_OUTCOME_TRY(

...
*/
#ifndef ADAPT_RK_ALL_H_
#define ADAPT_RK_ALL_H_
...
class RKAllIntegratorCore : public RKIntegratorCore {

public:
    RKAllIntegratorCore() : RKIntegratorCore() {}
    RKAllIntegratorCore(const double abs_tol, const double rel_tol, const double dt_min, const double dt_max) :
              ...
              Eigen::Ref<Eigen::VectorXd> ytp1) const
    {
        ...
        while (!converged && !failed_step_size_adapt) {
            ...
            converged = ((ytp1_low - ytp1_high).array().abs() <= dt * (m_abs_tol + ytp1_low.array().abs() * m_rel_tol)).all();
            ...
            if (converged || dt < 2 * m_dt_min + 1e-6) {
                ...
                bool crit2 = ((ytp1_low - ytp1_high).array().abs()  <=  0.03 * dt * (m_abs_tol + ytp1_low.array().abs() * m_rel_tol)).all();

                             ...
                             Eigen::Ref<Eigen::VectorXd> ytp1) const
{
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> kt_values(yt.size(),
                                                                                     m_tab_final.entries_low.size());
    ...
    while (!converged && !failed_step_size_adapt) {
        ...
        auto err  = (ytp1_low - ytp1_high).array().abs();
        auto eps  = dt * (m_abs_tol + ytp1_low.array().abs() * m_rel_tol);
        ...
        if (converged || dt < 2 * m_dt_min + 1e-6) {
            ...
            bool crit2 = (err <= 0.03 * eps).all();

            //if (max_err <= 0.03 * conv_crit &&
            if (crit2 && 2 * dt < m_dt_max) { // error of doubled step size is about 32 times as large
                dt = 2 * dt;
            }
        }
    }

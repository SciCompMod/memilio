...
bool VRKFastIntegratorCore::step(const DerivFunction& f, Eigen::Ref<const Eigen::VectorXd> yt, double& t, double& dt,
                                 Eigen::Ref<Eigen::VectorXd> ytp1) const
{
    ...
    while (max_err > conv_crit && !failed_step_size_adapt) {
        ...
        auto ytp1_high = yt + (dt * (m_kt_values * m_tab_final.entries_high));

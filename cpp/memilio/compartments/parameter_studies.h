 ...
 */
template <typename FP, class S, class EdgeT = mio::MobilityEdge<FP>>
...
public:
    ...
    */
    using SimulationGraph = mio::Graph<mio::SimulationNode<FP, Simulation>, EdgeT>;
...
public:
    ...
    run(CreateSimulationFunction&& create_simulation, ProcessSimulationResultFunction&& process_simulation_result)
    {
        ...
        std::vector<std::invoke_result_t<ProcessSimulationResultFunction, Simulation, size_t>> ensemble_result;
        ensemble_result.reserve(m_num_runs);

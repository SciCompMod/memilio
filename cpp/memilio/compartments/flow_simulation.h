...
public:
    ...
    Eigen::Ref<Eigen::VectorXd> advance(double tmax)
    {
        ...
        // as many entries in m_flow_result as in this->m_result
        assert(m_flow_result.get_num_time_points() == this->m_result.get_num_time_points());
        auto result = Base::m_integrator.advance(
            [this](auto&& flows, auto&& t, auto&& dflows_dt) {
                ...
                this->get_model().get_derivatives(flows -
    ...
    /**
     * @brief Returns the values describing the transition between compartments for each time step.
     *

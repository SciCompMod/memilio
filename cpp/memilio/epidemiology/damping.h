...
public:
    ...
    */
    ...
    auto get_matrix_at(SimulationTime<FP> t) const
    {
        ...
        auto damping =
            smoother_cosine(FP(t), FP(FP(std::get<SimulationTime<FP>>(*ub)) - 1), FP(std::get<SimulationTime<FP>>(*ub)),
    ...
    if (m_accumulated_dampings_cached.empty()) {
        ...
        for (auto& damping : m_dampings) {
            ...
            if (floating_point_equal<FP>(FP(get<SimulationTime<FP>>(damping)),
                                         FP(get<SimulationTime<FP>>(m_accumulated_dampings_cached.back())), FP(1e-15),
                                         FP(1e-15))) {

...
/**
 * @brief STL iterator for a MultiIndex.
...
{
    return {Index<Categories...>::Zero(), std::move(dimensions)};
}
...
MultiIndexIterator<Index<Categories...>> end(Index<Categories...> dimensions)
{
    ...
    auto end = Index<Categories...>::Zero();
    if constexpr (Index<Categories...>::size > 0) {
        mio::get<0>(end) = mio::get<0>(dimensions);
    }
...
{
    return {begin(dimensions), end(dimensions)};
}

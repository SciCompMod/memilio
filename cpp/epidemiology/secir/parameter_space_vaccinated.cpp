...
void set_params_distributions_normal(SecirModelV& model, double t0, double tmax, double dev_rel)
{
    ...
    for (auto i = AgeGroup(0); i < model.parameters.get_num_groups(); i++) {
        for (auto j = Index<InfectionStateV>(0); j < Index<InfectionStateV>(InfectionStateV::Count); j++) {

            // don't touch S and D
            if (j == InfectionStateV::Susceptible || j == InfectionStateV::Dead) {
                continue;
            }
...
void draw_sample_demographics(SecirModelV& model)
{
    ...
    for (auto i = AgeGroup(0); i < model.parameters.get_num_groups(); i++) {
        ...
        model.populations[{i, InfectionStateV::Exposed}].draw_sample();
        model.populations[{i, InfectionStateV::Carrier}].draw_sample();
        model.populations[{i, InfectionStateV::Infected}].draw_sample();
        model.populations[{i, InfectionStateV::Hospitalized}].draw_sample();
        model.populations[{i, InfectionStateV::ICU}].draw_sample();
        model.populations[{i, InfectionStateV::Recovered}].draw_sample();
...
void draw_sample_infection(SecirModelV& model)
{
...
void draw_sample(SecirModelV& model)
{

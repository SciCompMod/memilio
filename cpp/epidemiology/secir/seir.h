...
using SeirParameters = ParameterSet<TransmissionRisk, StageTimeIncubationInv, StageTimeInfectiousInv, ContactFrequency>;
...
public:
    SeirModel()
        : CompartmentalModel<SeirPopulations, SeirParameters>(Po({Index<SeirInfType>((size_t)SeirInfType::Count)}, 0.),
                                                              Pa())
    {
#if !USE_DERIV_FUNC
        //S to E
        this->add_flow(
            std::make_tuple(SeirInfType::S), std::make_tuple(SeirInfType::E),
            [](Pa const& p, Eigen::Ref<const Eigen::VectorXd> pop, Eigen::Ref<const Eigen::VectorXd> y, double t) {
                ...
                return cont_freq_eff * Po::get_from(y, SeirInfType::S) * Po::get_from(pop, SeirInfType::I) * divN;
            });

        //E to I
        this->add_flow(std::make_tuple(SeirInfType::E), std::make_tuple(SeirInfType::I),
                       [](Pa const& p, Eigen::Ref<const Eigen::VectorXd> /*pop*/, Eigen::Ref<const Eigen::VectorXd> y,
                          double /*t*/) {

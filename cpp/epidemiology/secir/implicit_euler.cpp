                                       ...
                                       double& dt, Eigen::Ref<Eigen::VectorXd> ytp1) const
{
    ...
    double dummy_R2 =
        1.0 / (2 * (params.get<SerialInterval>()[AgeGroup(0)]) - (params.get<IncubationTime>()[AgeGroup(0)])); // R2 = 1/(2SI-TINC)

    double dummy_R3 =
        0.5 / ((params.get<IncubationTime>()[AgeGroup(0)]) - (params.get<SerialInterval>()[AgeGroup(0)])); // R3 = 1/(2(TINC-SI))

    double dummy_C =
        1. / (1. + dt * ((1. - params.get<AsymptoticCasesPerInfectious>()[AgeGroup(0)]) * dummy_R3 +
                         params.get<AsymptoticCasesPerInfectious>()[AgeGroup(0)] / params.get<InfectiousTimeAsymptomatic>()[AgeGroup(0)]));

    double dummy_I =
        1. /
        (1. +
         dt * ((1 - params.get<HospitalizedCasesPerInfectious>()[AgeGroup(0)]) / params.get<InfectiousTimeMild>()[AgeGroup(0)] +
               params.get<HospitalizedCasesPerInfectious>()[AgeGroup(0)] / params.get<HomeToHospitalizedTime>()[AgeGroup(0)]));

    double dummy_H =
        1. /
        (1. +
         dt * ((1 - params.get<ICUCasesPerHospitalized>()[AgeGroup(0)]) / params.get<HospitalizedToHomeTime>()[AgeGroup(0)] +
    ...
    do {
        ...
        yt_tilde[(size_t)InfectionState::Infected] = dummy_I * (yt_eval[(size_t)InfectionState::Infected] +
                                                        dt * (1 - params.get<AsymptoticCasesPerInfectious>()[AgeGroup(0)]) *
    ...
    yt_tilde[(size_t)InfectionState::Hospitalized] =
        dummy_H * (yt_eval[(size_t)InfectionState::Hospitalized] + dt * params.get<HospitalizedCasesPerInfectious>()[AgeGroup(0)] /
    ...
    yt_tilde[(size_t)InfectionState::Recovered] =
        ...
        dt * (params.get<AsymptoticCasesPerInfectious>()[AgeGroup(0)] / params.get<InfectiousTimeAsymptomatic>()[AgeGroup(0)] *
                  yt_tilde[(size_t)InfectionState::Carrier] +
              (1 - params.get<HospitalizedCasesPerInfectious>()[AgeGroup(0)]) / params.get<InfectiousTimeMild>()[AgeGroup(0)] *
                  yt_tilde[(size_t)InfectionState::Infected] +

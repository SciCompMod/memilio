...
template <typename T> struct seirParam {
    ...
    seirParam(T tinc, T tinfmild, T tserint, T thosp2home, T thome2hosp, T thosp2icu, T ticu2home, T tinfasy,
              T ticu2death, T cont_freq_in, T alpha_in, T beta_in, T delta_in, T rho_in, T theta_in, T nb_total_t0_in,
              T nb_exp_t0_in, T nb_car_t0_in, T nb_inf_t0_in, T nb_hosp_t0_in, T nb_icu_t0_in, T nb_rec_t0_in,
              T nb_dead_t0_in)
    {
        ...
        base_reprod =
            cont_freq * ((1 - rho) * tinfmild_inv + dummy_R3 * beta * (1 - alpha) + rho * thome2hosp) /

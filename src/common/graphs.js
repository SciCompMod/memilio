...
class Graphs {
  constructor(node) {
    ...
    this.width = node.clientWidth - this.margin_horizontal;
    this.height = node.clientHeight - this.margin_vertical;
  ...
  visualize(data, actions) {
    ...
    function onMouseMove() {
      ...
      var index = bisectDate(self.data_read[1], self.xScale.invert(x0_pos));
      var selectedData = self.data_read[1][index - 1];

      x0_pos = self.xScale.invert(x0_pos + 0.5 * self.margin_horizontal);

      // set bar to move
      self.selectLine.style('opacity', 0.25).attr('x', self.xScale(x0_pos));
      ...
      for (let i = 0; i < self.data_read[1][0].cases.length; i++) {
    ...
    function onMouseMoveMini() {
      if (self.mouse_down_on_mini) {
        ...
        if (self.end_interval_x - self.begin_interval_x > 0) {
          self.overlay_mini.attr(
            'width',
            self.end_interval_x - self.begin_interval_x
          );
        }
      }
    }
    ...
    function onMouseReleaseMini() {
      ...
      if (self.end_interval_x - self.begin_interval_x > 0) {
        ...
        var i = bisectDate(
          self.data_read[0],
          self.xScaleMini.invert(self.end_interval_x)
        );
    ...
    if (this.logscale) {
      d3.select('#button_log')
        .attr('class', 'button_rescale_active')
        .attr('fill', '#73B0FF');
    } else {
      d3.select('#button_lin')
        .attr('class', 'button_rescale_active')
        .attr('fill', '#73B0FF');
    }
    ...
    this.g[1] = this.svg_graphs
      ...
      .attr(
        ...
        'translate(' +
          0.5 * this.margin_horizontal +
          ',' +
          (this.margin_top - 1.5 * this.heightMini) +
          ')'
      )
      .attr('pointer-events', 'all')
    ...
    this.g[2] = this.svg_graphs
      .append('g')
      .attr(
        'transform',
        'translate(' + 0.5 * this.margin_horizontal + ',' + 10 + ')'
      );

    this.overlay_mini = this.g[1]
      .append('rect')
    ...
    if (this.begin_interval_i === 0 && this.end_interval_i === 60) {
      this.filteredData = data.filter((d) => {
        return d.day < data[this.end_interval_i].day;
      });
      this.data_read.push(this.filteredData);
    } else {
      this.data_read.push(
        data.slice(this.begin_interval_i, this.end_interval_i)
      );
    }

    this.xScale.domain(
      d3.extent(this.data_read[1], function (d) {
        return d.day;
      })
    );
    this.yScale.domain([
      0,
      this.max_fact *
        d3.max(this.data_read[1], function (d) {
          return d.cases[0];
        })
    ]);
    ...
    if (this.begin_interval_i === 0 && this.end_interval_i === 60) {
      for (let i = 0; i < this.data_read[0][0].cases.length; i++) {
        this.g[0]
          .append('path')
    ...
    // mini graph
    this.xScaleMini.domain(
      d3.extent(
        this.data_read[0].map(function (d) {
          return d.day;
        })
      )
    );
    this.yScaleMini.domain([
      1,
      this.max_fact *
        d3.max(this.data_read[0], function (d) {
          return d.cases[0];
        })
    ]);

    this.overlay_mini.attr(
      'width',
      this.end_interval_x - this.begin_interval_x
    );
    ...
    this.selectLine = this.svg_graphs
      .append('rect')
      .style('opacity', 0)
      .attr('width', 2)
      .attr('height', (d) => {
    ...
    for (let i = 0; i < this.data_read[0][0].cases.length; i++) {
      this.selectLineCircle[i] = this.svg_graphs
        .append('g')
        .append('circle')
        .style('opacity', 0)
        .attr('r', 4)
    ...
    for (let i = 0; i < this.data_read[0][0].cases.length + 1; i++) {
      this.selectLineTooltip[i] = this.svg_graphs
        .append('g')
        .append('text')
        .style('opacity', 0)
        .attr('text-anchor', 'left')
  ...
  update_graph() {
    ...
    this.yScale.domain([
      ...
      this.max_fact *
        d3.max(this.data_read[1], function (d) {
          return Math.max(d.cases[0], d.cases[1], d.cases[2], d.cases[3]);
        })
    ]);
  ...
  subsample_data() {
    delete this.data_read[1];
    this.data_read[1] = this.data_read[0].slice(
      this.begin_interval_i,
      this.end_interval_i
    );
  }
    ...
    actions.forEach((action, index_i) => {
      action.intervals.forEach((interval, index_j) => {
        ...
        this.g[2]
          ...
          .attr('y', (d) => {
            return 30 + this.heightMini + 15 * index_i;
          })
